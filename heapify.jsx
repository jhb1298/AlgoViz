...
/* extracted from heapify.html */
        ...
        function generateSimulation(arr) {
            ...
            history.push({ stepId: 0, arr: [...data], status: 'START', algoLine: 1, desc: "Starting Max-Heap construction", speech: TREE_SCRIPT.HEAPIFY.START() });
            ...
            async function heapify(size, i) {
                ...
                if (l < size) {
                    history.push({ stepId: history.length, arr: [...data], activeNodes: [i, l], status: 'COMPARING', algoLine: 3, desc: `Comparing parent ${data[i]} with child ${data[l]}`, speech: TREE_SCRIPT.HEAPIFY.COMPARING(data[i], data[l]) });
                    if (data[l] > data[largest]) largest = l;
                }

                if (r < size) {
                    history.push({ stepId: history.length, arr: [...data], activeNodes: [i, r], status: 'COMPARING', algoLine: 3, desc: `Comparing parent ${data[i]} with child ${data[r]}`, speech: TREE_SCRIPT.HEAPIFY.COMPARING(data[i], data[r]) });
                    if (data[r] > data[largest]) largest = r;
                }

                if (largest !== i) {
                    history.push({ stepId: history.length, arr: [...data], activeNodes: [i, largest], status: 'SWAPPING', algoLine: 4, desc: `Swapping ${data[i]} and ${data[largest]}`, speech: TREE_SCRIPT.HEAPIFY.SWAPPING(data[i], data[largest]) });
                    [data[i], data[largest]] = [data[largest], data[i]];
                    await heapify(size, largest);
                } else {
                    history.push({ stepId: history.length, arr: [...data], activeNodes: [i], status: 'DONE', algoLine: 5, desc: `Node ${data[i]} satisfies heap property`, speech: TREE_SCRIPT.HEAPIFY.DONE() });
                }
            }
            ...
            history.push({ stepId: history.length, arr: [...data], status: 'SUCCESS', algoLine: 5, desc: "Max-Heap Built!", speech: "The Max-Heap is now fully constructed!" });
            return history;
        }

        const NodeComponent = ({  val, x, y, isActive, isSwap  }) => {
            ...
            return (
            <div className={`absolute w-14 h-14 rounded-full bg-slate-800 border-4 flex items-center justify-center font-black text-lg transition-all duration-500 z-20 ${isActive ? 'node-active' : (isSwap ? 'node-swap' : 'border-slate-700')}`} style={{ left: x - 28, top: y - 28, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                {val}
            </div>
        );
        };
        ...
        const App = () => {
            ...
            const [input, setInput] = useState("10,20,5,30,15,40,25");
            ...
            const state = history[currentStep] || { arr: [], activeNodes: [] };

            const getTreeCoords = (i, n) => {
                const level = Math.floor(Math.log2(i + 1));
                const posInLevel = i - (Math.pow(2, level) - 1);
                const numInLevel = Math.pow(2, level);
                const x = 500 + (posInLevel - (numInLevel - 1) / 2) * (1000 / numInLevel);
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <AlgorithmPanel algoKey="HEAPIFY" currentLine={state.algoLine} />
                        ...
                        <div className={`absolute left-0 top-0 w-full h-full ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            <svg className="absolute top-0 left-0 w-full h-full overflow-visible">
                                {state.arr.map((_, i) => {
                                    const l = 2 * i + 1;
                                    const r = 2 * i + 2;
                                    const p = getTreeCoords(i, state.arr.length);
                                    return (
                                        <React.Fragment key={i}>
                                            {l < state.arr.length && <line x1={p.x} y1={p.y} x2={getTreeCoords(l, state.arr.length).x} y2={getTreeCoords(l, state.arr.length).y} stroke={state.activeNodes?.includes(i) && state.activeNodes?.includes(l) ? "#6366f1" : "#1e293b"} strokeWidth={state.activeNodes?.includes(i) && state.activeNodes?.includes(l) ? 4 : 2} />}
                                            {r < state.arr.length && <line x1={p.x} y1={p.y} x2={getTreeCoords(r, state.arr.length).x} y2={getTreeCoords(r, state.arr.length).y} stroke={state.activeNodes?.includes(i) && state.activeNodes?.includes(r) ? "#6366f1" : "#1e293b"} strokeWidth={state.activeNodes?.includes(i) && state.activeNodes?.includes(r) ? 4 : 2} />}
                                        </React.Fragment>
                                    );
                                })}
                            </svg>
                            {state.arr.map((val, i) => {
                                const coords = getTreeCoords(i, state.arr.length);
                                return (
                                    <NodeComponent key={i} val={val} x={coords.x} y={coords.y} isActive={state.activeNodes?.includes(i)} isSwap={state.status === 'SWAPPING' && state.activeNodes?.includes(i)} />
                                );
                            })}
                        </div>
                    </div>
