...
import './mergesort_script.js';
import './settings_manager.js';
import './algorithm_data.js';
import './chatbot.js';

/* extracted from mergesort.html */
        ...
        function generateSimulation(arr) {
            let nodeIdCounter = 0;
            const history = [];
            function solve(data, depth, parentId = null) {
                const id = nodeIdCounter++;
                const node = { id, parentId, data, children: [], depth };
                history.push({ stepId: history.length, nodeId: id, data, line: 0, algoLine: 2, status: 'START', desc: `Split [${data.join(',')}]`, speech: MERGESORT_SCRIPT.START(data) });
                if (data.length <= 1) {
                    history.push({ stepId: history.length, nodeId: id, data, line: 1, algoLine: 1, status: 'BASE', desc: `Base case: [${data.join(',')}]`, result: data, speech: MERGESORT_SCRIPT.BASE(data[0]) });
                    return { node, result: data };
                }
                const mid = Math.floor(data.length / 2);
                const left = data.slice(0, mid); const right = data.slice(mid);
                history.push({ stepId: history.length, nodeId: id, data, line: 2, algoLine: 3, status: 'THINKING', desc: `Sorting left half`, speech: MERGESORT_SCRIPT.THINKING('left') });
                const { node: lNode, result: lRes } = solve(left, depth + 1, id);
                node.children.push(lNode);
                history.push({ stepId: history.length, nodeId: id, data, line: 2, algoLine: 4, status: 'THINKING', desc: `Sorting right half`, speech: MERGESORT_SCRIPT.THINKING('right') });
                const { node: rNode, result: rRes } = solve(right, depth + 1, id);
                node.children.push(rNode);
                const merged = [...lRes, ...rRes].sort((a,b) => a-b);
                history.push({ stepId: history.length, nodeId: id, data, line: 3, algoLine: 5, status: 'MERGE', desc: `Merged result`, result: merged, speech: MERGESORT_SCRIPT.MERGE(lRes, rRes, merged) });
                return { node, result: merged };
            }
            const { node: treeRoot } = solve(arr, 0);
        ...
        const NodeComponent = ({ nodeData, isActive, isBreadcrumb, latestState }) => {
            ...
            const arrToRender = latestState?.result || nodeData.data;
            ...
            if (latestState) {
                if (isActive) {
                    ...
                    if (latestState.status === 'START') { statusColor = "bg-blue-600"; statusText = "SPLITTING"; borderColor="border-blue-500"; }
                    if (latestState.status === 'BASE') { statusColor = "bg-emerald-600"; statusText = "BASE"; borderColor="border-emerald-500"; }
                    if (latestState.status === 'MERGE') { statusColor = "bg-rose-600"; statusText = "MERGING"; borderColor="border-rose-500"; }
                    if (latestState.status === 'THINKING') { statusColor = "bg-indigo-600"; statusText = "CALLING"; borderColor="border-indigo-500"; }
                } else {
                    if (latestState.status === 'MERGE') { statusColor = "bg-slate-600"; statusText = "SORTED"; borderColor = "border-slate-600"; opacity = 0.8; }
            ...
            return (
                <div className={`absolute rounded-xl shadow-2xl overflow-hidden transition-all duration-300 border-2 bg-slate-800 flex flex-col justify-between ${borderColor} ${shadow}`} style={{ left: nodeData.x, top: nodeData.y, width: NODE_WIDTH, height: NODE_HEIGHT, transform: isActive ? 'scale(1.05)' : 'scale(1)', zIndex: isActive ? 50 : (isBreadcrumb ? 40 : 10), opacity: opacity, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                    <div className="bg-slate-900/80 px-4 py-2 flex justify-between items-center border-b border-slate-700"><span className={`font-mono font-bold text-sm ${isActive || isBreadcrumb ? 'text-blue-200' : 'text-slate-400'}`}>ms(len={nodeData.data.length})</span><span className={`text-[10px] font-bold px-2 py-0.5 rounded ${statusColor} text-white shadow-sm`}>{statusText}</span></div>
                    <div className="px-3 py-1.5 text-[10px] font-mono text-slate-400 bg-slate-900 leading-relaxed border-b border-slate-700">
                        <div className={getCodeClass(1)}>1. if (arr.len &lt;= 1) return arr</div>
                        <div className={getCodeClass(2)}>2. left = ms(l), right = ms(r)</div>
                        <div className={getCodeClass(3)}>3. return merge(left, right)</div>
                    </div>
                    <div className="bg-slate-800 flex-1 relative p-2 flex flex-wrap gap-1 items-center justify-center content-center">{arrToRender.map((n, i) => (<div key={i} className="w-9 h-9 bg-slate-700 border border-slate-600 rounded flex items-center justify-center text-sm font-bold shadow-[0_0_10px_rgba(0,0,0,0.3)] text-blue-100">{n}</div>))}</div>
                </div>
            );
        };
        ...
        const App = () => {
            ...
            const [input, setInput] = useState("8,3,5,1");
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <AlgorithmPanel algoKey="MERGESORT" currentLine={currentState?.algoLine} />
