...
import './factorial_script.js';
import './settings_manager.js';
import './algorithm_data.js';
import './chatbot.js';

/* extracted from factorial.html */
        ...
        function generateSimulation(n) {
            ...
            function solve(val, depth, parentId = null) {
                ...
                history.push({ stepId: history.length, nodeId: id, val, line: 0, algoLine: 0, status: 'START', desc: `fact(${val}) called`, speech: FACTORIAL_SCRIPT.START(val) });
                if (val <= 1) {
                    history.push({ stepId: history.length, nodeId: id, val, line: 1, algoLine: 1, status: 'BASE', desc: `Base case: fact(1) = 1`, result: 1, speech: FACTORIAL_SCRIPT.BASE(val) });
                    return { node, result: 1 };
                }
                history.push({ stepId: history.length, nodeId: id, val, line: 2, algoLine: 2, status: 'THINKING', desc: `Recurse: ${val} * fact(${val-1})`, speech: FACTORIAL_SCRIPT.THINKING(val) });
                const { node: child, result: childResult } = solve(val - 1, depth + 1, id);
                node.children.push(child);
                const final = val * childResult;
                history.push({ stepId: history.length, nodeId: id, val, line: 3, algoLine: 3, status: 'RETURN', desc: `Return: ${val} * ${childResult} = ${final}`, result: final, speech: FACTORIAL_SCRIPT.RETURN(val, childResult) });
        ...
        const NodeComponent = ({ nodeData, isActive, isBreadcrumb, latestState }) => {
            ...
            if (latestState) {
                if (isActive) {
                    ...
                    if (latestState.status === 'START') { statusColor = "bg-emerald-600"; statusText = "START"; borderColor=`style={{borderColor: settings.nodeColor}}`; }
                    if (latestState.status === 'BASE') { statusColor = "bg-emerald-600"; statusText = "BASE"; borderColor=`style={{borderColor: settings.nodeColor}}`; }
                    if (latestState.status === 'RETURN') { statusColor = "bg-rose-600"; statusText = "RETURN"; borderColor=`style={{borderColor: settings.nodeColor}}`; }
                    if (latestState.status === 'THINKING') { statusColor = "bg-blue-600"; statusText = "CALLING"; borderColor=`style={{borderColor: settings.nodeColor}}`; }
            ...
            return (
                <div className={`absolute rounded-xl shadow-2xl overflow-hidden transition-all duration-300 border-2 bg-slate-800 flex flex-col justify-between ${shadow}`} style={{ left: nodeData.x, top: nodeData.y, width: NODE_WIDTH, height: NODE_HEIGHT, transform: isActive ? 'scale(1.05)' : 'scale(1)', zIndex: isActive ? 50 : (isBreadcrumb ? 40 : 10), opacity: opacity, borderColor: borderColor, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                    <div className="bg-slate-900/80 px-4 py-2 flex justify-between items-center border-b border-slate-700"><span className={`font-mono font-bold text-sm ${isActive || isBreadcrumb ? 'text-blue-200' : 'text-slate-400'}`}>fact({val})</span><span className={`text-[10px] font-bold px-2 py-0.5 rounded ${statusColor} text-white shadow-sm`}>{statusText}</span></div>
                    <div className="px-3 py-2 text-[11px] font-mono text-slate-400 bg-slate-900 leading-relaxed border-b border-slate-700">
                        <div className={getCodeClass(1)}>1. if (n &lt;= 1) return 1</div>
                        <div className={getCodeClass(2)}>2. res = fact(n - 1)</div>
                        <div className={getCodeClass(3)}>3. return n * res</div>
                    </div>
                    <div className="bg-slate-800 flex-1 relative p-3 flex flex-col items-center justify-center"><div className="font-black text-white" style={{fontSize: (val > 9 ? settings.fontSize * 2 : settings.fontSize * 2.5) + 'px'}}>{val}<span className="text-blue-500">!</span></div>{latestState?.result && <div className="text-emerald-400 font-bold font-mono animate-bounce text-lg mt-1">= {latestState.result}</div>}</div>
                </div>
            );
        };
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">Recursive Factorial</h1><span className="text-xs text-slate-400 font-mono">Visualizing n! calculation</span></div></div>
