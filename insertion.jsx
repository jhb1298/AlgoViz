...
/* extracted from insertion.html */
        ...
        function generateSimulation(arr) {
            ...
            history.push({ stepId: 0, arr: [...data], activeIdx: -1, sortedCount: 1, status: 'START', algoLine: 1, desc: "Starting Insertion Sort", speech: SORTING_SCRIPT.INSERTION.START() });

            for (let i = 1; i < n; i++) {
                let key = data[i];
                let j = i - 1;
                
                history.push({ stepId: history.length, arr: [...data], activeIdx: i, sortedCount: i, status: 'PICKING', algoLine: 2, desc: `Picking up ${key}`, speech: SORTING_SCRIPT.INSERTION.PICKING(key) });

                while (j >= 0 && data[j] > key) {
                    history.push({ stepId: history.length, arr: [...data], activeIdx: j, sortedCount: i, status: 'SLIDING', algoLine: 4, desc: `${data[j]} > ${key}, sliding right`, speech: SORTING_SCRIPT.INSERTION.SLIDING(key, data[j]) });
                    data[j + 1] = data[j];
                    j = j - 1;
                }
                data[j + 1] = key;
                history.push({ stepId: history.length, arr: [...data], activeIdx: j + 1, sortedCount: i + 1, status: 'INSERTING', algoLine: 6, desc: `Inserted ${key} at spot ${j + 2}`, speech: SORTING_SCRIPT.INSERTION.INSERTING(key) });
            }

            history.push({ stepId: history.length, arr: [...data], activeIdx: -1, sortedCount: n, status: 'SUCCESS', algoLine: 6, desc: "Sort Complete!", speech: SORTING_SCRIPT.INSERTION.SUCCESS() });
            return history;
        }

        const App = () => {
            const [input, setInput] = useState("34,12,89,5,67,20");
            ...
            const state = history[currentStep] || { arr: [], activeIdx: -1, sortedCount: 0 };
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">Insertion Sort Visualizer</h1><span className="text-xs text-slate-400 font-mono">Organizing cards in hand</span></div></div>
                    ...
                    <AlgorithmPanel algoKey="INSERTION" currentLine={state.algoLine} />
                            ...
                            {state.arr.map((val, i) => {
                                ...
                                return (
                                    <div key={i} className={`flex flex-col items-center gap-2 bar-transition`} style={{ width: 60, transform, opacity }}>
                                        <div className={`w-full rounded-t-lg rounded-b-sm border-2 border-white/20 shadow-lg`} style={{ height: val * 3, backgroundColor: barColor }}></div>
