...
/* extracted from selection.html */
        ...
        function generateSimulation(arr) {
            ...
            history.push({ stepId: 0, arr: [...data], sortedCount: 0, currentIdx: -1, minIdx: -1, status: 'START', algoLine: 1, desc: "Starting Selection Sort", speech: SORTING_SCRIPT.SELECTION.START() });

            for (let i = 0; i < n - 1; i++) {
                let minIdx = i;
                history.push({ stepId: history.length, arr: [...data], sortedCount: i, currentIdx: i, minIdx: i, status: 'NEW_MIN', algoLine: 2, desc: `Assume ${data[i]} is the minimum`, speech: SORTING_SCRIPT.SELECTION.NEW_MIN(data[i]) });

                for (let j = i + 1; j < n; j++) {
                    history.push({ stepId: history.length, arr: [...data], sortedCount: i, currentIdx: j, minIdx: minIdx, status: 'SCANNING', algoLine: 3, desc: `Scanning ${data[j]}`, speech: SORTING_SCRIPT.SELECTION.SCANNING(data[j]) });
                    
                    if (data[j] < data[minIdx]) {
                        minIdx = j;
                        history.push({ stepId: history.length, arr: [...data], sortedCount: i, currentIdx: j, minIdx: minIdx, status: 'NEW_MIN', algoLine: 5, desc: `New minimum found: ${data[j]}`, speech: SORTING_SCRIPT.SELECTION.NEW_MIN(data[j]) });
                    }
                }

                if (minIdx !== i) {
                    history.push({ stepId: history.length, arr: [...data], sortedCount: i, currentIdx: -1, minIdx: minIdx, swapWith: i, status: 'SWAPPING', algoLine: 6, desc: `Swapping ${data[minIdx]} with ${data[i]}`, speech: SORTING_SCRIPT.SELECTION.SWAPPING(data[minIdx], data[i]) });
                    [data[i], data[minIdx]] = [data[minIdx], data[i]];
                }
            }

            history.push({ stepId: history.length, arr: [...data], sortedCount: n, currentIdx: -1, minIdx: -1, status: 'SUCCESS', algoLine: 6, desc: "Sort Complete!", speech: SORTING_SCRIPT.SELECTION.SUCCESS() });
            return history;
        }
        ...
        const App = () => {
            ...
            const [input, setInput] = useState("64,25,12,22,11");
            ...
            const state = history[currentStep] || { arr: [], currentIdx: -1, minIdx: -1, sortedCount: 0 };

            const getColor = (val) => {
                const ratio = val / 100;
                // Interpolate between Indigo (6366f1) and Amber (f59e0b)
                const r = Math.floor(99 + (245 - 99) * ratio);
                const g = Math.floor(102 + (158 - 102) * ratio);
                const b = Math.floor(241 + (11 - 241) * ratio);
                return `rgb(${r}, ${g}, ${b})`;
            };
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <AlgorithmPanel algoKey="SELECTION" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.5, z-e.deltaY*0.001), 2))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 flex items-end justify-center gap-2 ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(calc(-50% + ${manualOffset.x}px), calc(-50% + ${manualOffset.y}px)) scale(${zoom})` }}>
                            {state.arr.map((val, i) => {
                                let barColor = getColor(val);
                                let opacity = 0.4;
                                let transform = "translateY(0)";
                                let borderWidth = "border-white/10";
                                
                                if (i < state.sortedCount) opacity = 1;
                                if (i === state.minIdx) {
                                    opacity = 1;
                                    transform = "scale(1.1) translateY(-5px)";
                                    borderWidth = "border-white";
                                }
                                if (i === state.currentIdx) {
                                    opacity = 1;
                                    transform = "translateY(-10px)";
                                }
                                if (state.status === 'SWAPPING' && (i === state.minIdx || i === state.swapWith)) {
                                    opacity = 1;
                                ...
                                return (
                                    <div key={i} className={`flex flex-col items-center gap-2 bar-transition`} style={{ width: 60, transform, opacity }}>
                                        <div className={`w-full rounded-t-lg rounded-b-sm border-2 ${borderWidth} shadow-lg`} style={{ height: val * 3, backgroundColor: barColor }}></div>
