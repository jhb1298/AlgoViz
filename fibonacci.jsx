...
import './fibonacci_script.js';
import './settings_manager.js';
import './algorithm_data.js';
import './chatbot.js';

/* extracted from fibonacci.html */
        ...
        function generateSimulation(n) {
            let nodeIdCounter = 0;
            const history = [];
            function solve(val, depth, parentId = null) {
                const id = nodeIdCounter++;
                const node = { id, parentId, val, children: [], depth };
                history.push({ stepId: history.length, nodeId: id, val, line: 0, status: 'START', desc: `fib(${val})`, speech: FIBONACCI_SCRIPT.START(val) });
                if (val <= 1) {
                    history.push({ stepId: history.length, nodeId: id, val, line: 1, status: 'BASE', desc: `fib(${val}) = ${val}`, result: val, speech: FIBONACCI_SCRIPT.BASE(val) });
                    return { node, result: val };
                }
                history.push({ stepId: history.length, nodeId: id, val, line: 2, status: 'THINKING', desc: `Calling fib(${val-1})`, speech: FIBONACCI_SCRIPT.THINKING(val, 'left') });
                const { node: leftNode, result: leftRes } = solve(val - 1, depth + 1, id);
                node.children.push(leftNode);
                history.push({ stepId: history.length, nodeId: id, val, line: 2, status: 'THINKING', desc: `Calling fib(${val-2})`, speech: FIBONACCI_SCRIPT.THINKING(val, 'right') });
                const { node: rightNode, result: rightRes } = solve(val - 2, depth + 1, id);
                node.children.push(rightNode);
                const final = leftRes + rightRes;
                history.push({ stepId: history.length, nodeId: id, val, line: 2, status: 'RETURN', desc: `${leftRes} + ${rightRes} = ${final}`, result: final, speech: FIBONACCI_SCRIPT.RETURN(leftRes, rightRes, final) });
                return { node, result: final };
            }
            const { node: treeRoot } = solve(n, 0);
        ...
        const NodeComponent = ({ nodeData, isActive, isBreadcrumb, latestState }) => {
            ...
            let highlightLine = -1, statusColor = "bg-slate-700", statusText = "IDLE", borderColor = "border-slate-700", shadow = "", opacity = 0.3, isWaiting = false;
            if (latestState) {
                if (isActive) {
                    opacity = 1; shadow = `shadow-[0_0_20px_${settings.nodeColor}80]`; highlightLine = latestState.line;
                    if (latestState.status === 'START') { statusColor = "bg-emerald-600"; statusText = "START"; borderColor=settings.nodeColor; }
                    if (latestState.status === 'BASE') { statusColor = "bg-emerald-600"; statusText = "BASE"; borderColor=settings.nodeColor; }
                    if (latestState.status === 'RETURN') { statusColor = "bg-rose-600"; statusText = "RETURN"; borderColor=settings.nodeColor; }
                    if (latestState.status === 'THINKING') { statusColor = "bg-blue-600"; statusText = "CALLING"; borderColor=settings.nodeColor; }
                    borderColor = settings.nodeColor;
                } else {
                    if (latestState.status === 'RETURN') { statusColor = "bg-slate-600"; statusText = "DONE"; borderColor = "#475569"; opacity = 0.8; }
                    else if (latestState.status === 'THINKING') { statusColor = "bg-indigo-600"; statusText = "WAITING"; borderColor = "#6366f1"; highlightLine = latestState.line; opacity = 1; isWaiting = true; }
                    else { statusColor = "bg-slate-600"; statusText = "PAUSED"; opacity = 0.9; borderColor = "#475569"; }
                }
            } else {
                borderColor = "#334155";
            }
            if (isBreadcrumb && !isActive) { borderColor = settings.nodeColor; shadow = `shadow-[0_0_15px_${settings.nodeColor}40]`; opacity = 1; }
            ...
            return (
                <div className={`absolute rounded-xl shadow-2xl overflow-hidden transition-all duration-300 border-2 bg-slate-800 flex flex-col justify-between ${shadow}`} style={{ left: nodeData.x, top: nodeData.y, width: NODE_WIDTH, height: NODE_HEIGHT, transform: isActive ? 'scale(1.05)' : 'scale(1)', zIndex: isActive ? 50 : (isBreadcrumb ? 40 : 10), opacity: opacity, borderColor: borderColor, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                    <div className="bg-slate-900/80 px-4 py-2 flex justify-between items-center border-b border-slate-700"><span className={`font-mono font-bold text-sm ${isActive || isBreadcrumb ? 'text-blue-200' : 'text-slate-400'}`}>fib({val})</span><span className={`text-[10px] font-bold px-2 py-0.5 rounded ${statusColor} text-white shadow-sm`}>{statusText}</span></div>
                    <div className="px-3 py-1.5 text-[10px] font-mono text-slate-400 bg-slate-900 leading-relaxed border-b border-slate-700">
                        <div className={getCodeClass(1)}>1. if (n &lt;= 1) return n</div>
                        <div className={getCodeClass(2)}>2. return fib(n-1) + fib(n-2)</div>
                    </div>
                    <div className="bg-slate-800 flex-1 relative p-2 flex flex-col items-center justify-center">
                        <div className="font-black text-white" style={{ fontSize: settings.fontSize + 'px' }}>n = {val}</div>
                        {latestState?.result !== undefined && <div className="text-emerald-400 font-bold font-mono text-xl animate-bounce mt-1">= {latestState.result}</div>}
                    </div>
                </div>
            );
        };
        ...
        const App = () => {
            ...
            const { history, flatNodes } = useMemo(() => {
                const { treeRoot, history } = generateSimulation(n);
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">Recursive Fibonacci</h1><span className="text-xs text-slate-400 font-mono">Branching subproblems</span></div></div>
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex items-center gap-2 bg-slate-800/80 p-1 pr-3 rounded-lg border border-slate-700/50"><span className="bg-slate-700/80 px-2 py-1 rounded text-slate-300 text-xs font-bold uppercase mr-1">Input N</span><input type="number" value={n} onChange={(e) => { setN(Math.min(7, Math.max(1, Number(e.target.value)))); setCurrentStep(0); setIsPlaying(false); }} className="bg-transparent font-bold outline-none text-blue-400 w-12 border-b border-blue-500/30" /></div>
                            </div>
                        </div>
                    </div>
                    <AlgorithmPanel algoKey="FIBONACCI_RECURSIVE" currentLine={currentState?.line} />
