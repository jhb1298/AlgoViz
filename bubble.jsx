...
/* extracted from bubble.html */

        

        // Audio helper
        const playBubbleSound = (isMuted) => {
            if (isMuted) return;
        ...
        function generateSimulation(arr) {
            ...
            history.push({ stepId: 0, arr: [...data], comparing: [], status: 'START', desc: "Starting Bubble Sort", speech: SORTING_SCRIPT.BUBBLE.START(), algoLine: 1 });

            for (let i = 0; i < n; i++) {
                for (let j = 0; j < n - i - 1; j++) {
                    history.push({ stepId: history.length, arr: [...data], comparing: [j, j + 1], status: 'COMPARING', desc: `Comparing ${data[j]} and ${data[j+1]}`, speech: SORTING_SCRIPT.BUBBLE.COMPARING(data[j], data[j+1]), algoLine: 3 });
                    
                    if (data[j] > data[j+1]) {
                        [data[j], data[j+1]] = [data[j+1], data[j]];
                        history.push({ stepId: history.length, arr: [...data], comparing: [j, j + 1], status: 'SWAPPING', desc: `Swapped ${data[j]} and ${data[j+1]}`, speech: SORTING_SCRIPT.BUBBLE.SWAPPING(data[j+1], data[j]), algoLine: 4 });
                    }
                }
                history.push({ stepId: history.length, arr: [...data], comparing: [], fixedIndex: n - i - 1, status: 'FIXED', desc: `${data[n-i-1]} is in its final position`, speech: SORTING_SCRIPT.BUBBLE.FIXED(data[n-i-1]), algoLine: 2 });
            }

            history.push({ stepId: history.length, arr: [...data], comparing: [], status: 'SUCCESS', desc: "Array Sorted!", speech: SORTING_SCRIPT.BUBBLE.SUCCESS(), algoLine: 4 });
            return history;
        }

        const App = () => {
            const { SettingsIcon, SettingsModal, AlgorithmPanel } = useMemo(() => window.initSettingsComponents(React), []);
            const [input, setInput] = useState("45,20,89,12,67,34");
                ...
                if (isMuted || !text) { 
                    setTimeout(() => { 
                        if(isPlayingRef.current) {
                            if (history[currentStep]?.status === 'SWAPPING') {
                                playBubbleSound(isMuted);
                                createParticles(history[currentStep].comparing);
                            }
                            nextStep(); 
                        }
                    }, delay); 
            ...
            const createParticles = (indices) => {
                const container = document.querySelector('.node-container');
                if (!container) return;
                indices.forEach(idx => {
                    const rect = container.children[idx].getBoundingClientRect();
                    for(let i=0; i<5; i++) {
                        const p = document.createElement('div');
                        p.className = 'bubble-particle';
                        p.style.left = `${Math.random() * BAR_WIDTH}px`;
                        p.style.width = `${Math.random() * 15 + 5}px`;
                        p.style.height = p.style.width;
                        container.children[idx].appendChild(p);
            ...
            const state = history[currentStep] || { arr: [], comparing: [] };
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">Bubble Sort Visualizer</h1><span className="text-xs text-slate-400 font-mono">Bubbling up the largest values</span></div></div>
                    ...
                    <AlgorithmPanel algoKey="BUBBLE" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.5, z-e.deltaY*0.001), 2))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center gap-2 ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(calc(-50% + ${manualOffset.x}px), calc(-50% + ${manualOffset.y}px)) scale(${zoom})` }}>
                            <div className="text-[10px] font-black text-slate-600 uppercase tracking-[0.5em] mb-4">Surface</div>
                            <div className="flex flex-col-reverse gap-2 p-8 bg-blue-500/5 rounded-3xl border border-blue-500/10 backdrop-blur-sm shadow-inner node-container">
                                {state.arr.map((val, i) => {
                                    let bgColor = "bg-slate-800";
                                    let borderColor = "border-white/10";
                                    if (state.comparing.includes(i)) {
                                        bgColor = state.status === 'SWAPPING' ? "bg-rose-500/20" : "bg-blue-500/20";
                                        borderColor = state.status === 'SWAPPING' ? "border-rose-500 shadow-[0_0_20px_rgba(244,63,94,0.3)]" : `${settings.nodeColor} shadow-[0_0_20px_${settings.nodeColor}80]`;
                                        if (state.status !== 'SWAPPING') borderColor = settings.nodeColor;
                                    }
                                    if (state.fixedIndex !== undefined && i >= state.fixedIndex) {
                                        bgColor = "bg-emerald-500/10";
                                        borderColor = "border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.1)]";
                                    }
                                    
                                    const isComparing = state.comparing.includes(i);
                                    const isFixed = state.fixedIndex !== undefined && i >= state.fixedIndex;

                                    return (
                                        <div key={i} className={`flex items-center gap-6 bar-transition px-8 py-4 rounded-3xl border-2 ${bgColor}`} style={{ width: 450, borderColor: isComparing ? (state.status === 'SWAPPING' ? '#f43f5e' : settings.nodeColor) : (isFixed ? '#10b981' : '#1e293b') }}>
                                            <span className="font-mono font-black text-sm text-slate-500 w-8">#{i}</span>
                                            
                                            {/* Bubble Representation */}
                                            <div className="flex-1 flex justify-center items-center h-24 relative">
                                                <div 
                                                    className={`rounded-full border-4 flex items-center justify-center font-black transition-all duration-500 shadow-2xl ${isComparing ? (state.status === 'SWAPPING' ? 'bg-rose-500/40 border-rose-400' : `bg-[${settings.nodeColor}40]`) : (isFixed ? 'bg-emerald-500/40 border-emerald-400' : 'bg-slate-700/40 border-slate-500')}`}
                                                    style={{ 
                                                        width: val + 20, 
                                                        height: val + 20,
                                                        fontSize: settings.fontSize + 'px',
                                                        borderColor: isComparing ? (state.status === 'SWAPPING' ? '#f43f5e' : settings.nodeColor) : (isFixed ? '#10b981' : '#64748b'),
                                                        backgroundColor: isComparing ? (state.status === 'SWAPPING' ? 'rgba(244,63,94,0.4)' : `${settings.nodeColor}40`) : (isFixed ? 'rgba(16,185,129,0.4)' : 'rgba(71,85,105,0.4)'),
                                                        boxShadow: isComparing ? `0 0 30px ${state.status === 'SWAPPING' ? '#f43f5e' : settings.nodeColor}44` : 'none'
                                                    }}
                                                >
                                            ...
                                            <span className={`font-mono font-black text-2xl w-12 text-right ${isComparing ? 'text-white' : 'text-slate-400'}`}>{String(val).padStart(2, '0')}</span>
                                        </div>
                                    );
                                })}
                            </div>
                            <div className="text-[10px] font-black text-slate-600 uppercase tracking-[0.5em] mt-4">Depth</div>
                        </div>
                    </div>
