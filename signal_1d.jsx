...
import './signal_script.js';
import './settings_manager.js';
import './algorithm_data.js';
import './chatbot.js';

/* extracted from signal_1d.html */
        ...
        function generateSimulation(signal, kernel, mode) {
            const history = [];
            const sig = [...signal];
            let ker = [...kernel];
            
            history.push({ stepId: 0, sig, ker, res: [], algoLine: 1, status: 'START', desc: `Initializing ${mode}...`, speech: SIGNAL_SCRIPT[mode].START() });

            if (mode === 'CONVOLUTION') {
                ker = ker.reverse();
                history.push({ stepId: 1, sig, ker, res: [], algoLine: 2, status: 'FLIP', desc: "Flipping kernel for convolution", speech: SIGNAL_SCRIPT.CONVOLUTION.FLIP() });
            }

            const result = [];
            const pad = ker.length - 1;
            const paddedSig = [...Array(pad).fill(0), ...sig, ...Array(pad).fill(0)];

            for (let i = 0; i < paddedSig.length - pad; i++) {
                let sum = 0;
                history.push({ stepId: history.length, sig, ker, res: [...result], slidePos: i, algoLine: 5, status: 'SLIDE', desc: `Calculating overlap at index ${i}`, speech: mode === 'CONVOLUTION' ? SIGNAL_SCRIPT.CONVOLUTION.SLIDE(i) : SIGNAL_SCRIPT.CORRELATION.SLIDE(i) });

                for (let j = 0; j < ker.length; j++) {
                    const prod = paddedSig[i + j] * ker[j];
                    sum += prod;
                    history.push({ stepId: history.length, sig, ker, res: [...result], slidePos: i, activeKerIdx: j, algoLine: 6, status: 'MULTIPLY', desc: `Multiplied ${paddedSig[i+j]} * ${ker[j]} = ${prod}`, speech: mode === 'CONVOLUTION' ? SIGNAL_SCRIPT.CONVOLUTION.MULTIPLY(paddedSig[i+j], ker[j], prod) : null });
                }

                result.push(sum);
                history.push({ stepId: history.length, sig, ker, res: [...result], slidePos: i, algoLine: 7, status: 'RESULT', desc: `Sum result: ${sum}`, speech: mode === 'CONVOLUTION' ? SIGNAL_SCRIPT.CONVOLUTION.RESULT(sum, i) : null });
            }

            history.push({ stepId: history.length, sig, ker, res: [...result], algoLine: 8, status: 'SUCCESS', desc: "Processing complete!", speech: SIGNAL_SCRIPT[mode].SUCCESS() });
            return history;
        }
        ...
        const App = () => {
            ...
            const [sigInput, setSigInput] = useState("1,2,1,0,1");
            const [kerInput, setKerInput] = useState("1,0,-1");
            ...
            const signal = useMemo(() => sigInput.split(',').map(Number), [sigInput]);
            const kernel = useMemo(() => kerInput.split(',').map(Number), [kerInput]);
            const history = useMemo(() => generateSimulation(signal, kernel, mode), [signal, kernel, mode]);
            ...
            const state = history[currentStep] || { sig: [], ker: [], res: [] };
            const pad = state.ker.length - 1;
            const paddedSig = [...Array(pad).fill(0), ...state.sig, ...Array(pad).fill(0)];
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-950 text-white select-none infinite-bg" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); }} onMouseUp={() => {setIsDraggingPanel(false);}}>
                    ...
                    <div className="absolute top-0 left-0 right-0 z-[60] p-6 flex justify-between items-center">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="font-black text-xl tracking-tighter uppercase italic">{mode} 1D</h1>
                                <div className="flex items-center gap-2">
                                    <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></span>
                                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Digital Signal Processing Lab</span>
                        ...
                        <div className="flex bg-slate-900 p-1.5 rounded-2xl border border-slate-800 gap-2 shadow-2xl">
                            {['CONVOLUTION', 'CORRELATION'].map(m => (
                                <button key={m} onClick={() => { setMode(m); setCurrentStep(0); setIsPlaying(false); }} className={`px-4 py-2 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all ${mode === m ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-slate-500 hover:text-white'}`}>{m}</button>
                            ))}
                        </div>
                    </div>
                    ...
                    <AlgorithmPanel algoKey="SIGNAL_1D" currentLine={state?.algoLine} />
                    <div className="flex-1 flex flex-col items-center justify-center gap-16 relative">
                        
                        {/* Signal Lane */}
                        <div className="flex flex-col items-center gap-4">
                            <span className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">Input Signal x[n] (Padded)</span>
                            <div className="flex gap-2">
                                {paddedSig.map((v, i) => (
                                    <div key={i} className={`w-16 h-16 flex items-center justify-center rounded-xl cell ${state.slidePos !== undefined && i >= state.slidePos && i < state.slidePos + state.ker.length ? 'cell-active border-indigo-500 bg-indigo-500/10' : 'opacity-40'}`}>
                        ...
                        {/* Kernel Lane (Floating / Sliding) */}
                        <div className="flex flex-col items-center gap-4 transition-all duration-500" style={{ transform: `translateX(${(state.slidePos * 72) - ((paddedSig.length-1)*36)}px)` }}>
                            <span className="text-[10px] font-bold text-amber-500 uppercase tracking-widest">Kernel h[n]</span>
                            <div className="flex gap-2">
                                {state.ker.map((v, i) => (
                                    <div key={i} className={`w-16 h-16 flex items-center justify-center rounded-xl cell kernel-cell ${state.activeKerIdx === i ? 'scale-110 shadow-2xl' : ''}`}>
                                        <span className="text-xl font-black">{v}</span>
                                    </div>
                                ))}
                            </div>
                        ...
                        {/* Result Lane */}
                        <div className="flex flex-col items-center gap-4">
                            <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">Output Result y[n]</span>
                            <div className="flex gap-2">
                                {Array.from({ length: paddedSig.length - pad }).map((_, i) => (
                                    <div key={i} className={`w-16 h-16 flex items-center justify-center rounded-xl cell result-cell ${state.res.length > i ? 'opacity-100' : 'opacity-20 border-dashed'} ${state.res.length === i + 1 ? 'scale-110 border-white' : ''}`}>
                                        <span className="text-xl font-black">{state.res[i] ?? '?'}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        {state.status === 'MULTIPLY' && (
                            <svg className="absolute inset-0 pointer-events-none w-full h-full overflow-visible">
                                {/* This would be complex to align perfectly without specific refs, but we can do a schematic representation */}
                            </svg>
                        )}
                    </div>

                    {/* Inputs */}
                    <div className="absolute left-10 bottom-32 flex flex-col gap-4 bg-slate-900/50 p-6 rounded-3xl border border-slate-800 backdrop-blur-xl">
                        <div className="flex flex-col gap-1">
                            <span className="text-[10px] font-bold text-slate-500 uppercase">Input Array</span>
                            <input value={sigInput} onChange={e => setSigInput(e.target.value)} className="bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-emerald-400 font-mono text-sm outline-none" />
                        </div>
                        <div className="flex flex-col gap-1">
                            <span className="text-[10px] font-bold text-slate-500 uppercase">Kernel Array</span>
                            <input value={kerInput} onChange={e => setKerInput(e.target.value)} className="bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-amber-400 font-mono text-sm outline-none" />
                        </div>
                    </div>
                    ...
                    <div className="absolute bottom-6 left-1/2 w-full max-w-4xl px-4 z-50 control-panel-wrapper" style={{ transform: `translate(calc(-50% + ${panelOffset.x}px), ${panelOffset.y}px)` }}>
                        <div className={`bg-slate-900/40 hover:bg-slate-900/85 backdrop-blur-md border border-slate-600/30 rounded-2xl shadow-2xl flex flex-col relative box-border transition-all duration-300 ${isMinimized ? 'w-fit min-w-[180px] mx-auto px-4 pb-3 pt-1' : 'px-5 pb-4 pt-2'}`}>
                            <div className="w-full h-6 cursor-grab active:cursor-grabbing flex justify-center items-center mb-1" onMouseDown={(e) => {e.preventDefault(); setIsDraggingPanel(true); panelDragStart.current={x:e.clientX-panelOffset.x, y:e.clientY-panelDragStart.current.y}; e.stopPropagation();}}>
                            ...
                            {isMinimized ? (
                                ...
                                <>
                                    ...
                                    <div className="flex items-center justify-between w-full">
                                        <div className="flex flex-col w-[30%] overflow-hidden pr-2"><span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-0.5">Operation Step {String(currentStep).padStart(3, '0')}</span><span className="text-sm font-mono text-blue-300 truncate tracking-tight">{state?.desc}</span></div>
