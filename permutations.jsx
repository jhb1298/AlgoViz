...
import './permutations_script.js';
import './settings_manager.js';
import './algorithm_data.js';
import './chatbot.js';

/* extracted from permutations.html */
        ...
        function generateSimulation(word) {
            let nodeIdCounter = 0;
            const treeRoot = { id: 0, current: '', remaining: word, children: [], depth: 0 };
            const history = [];
            function solve(node) {
                const { current, remaining, id } = node;
                history.push({ stepId: history.length, nodeId: id, line: 0, algoLine: 1, status: 'START', desc: `permute("${current}", "${remaining}")`, current, remaining, speech: PERMUTATIONS_SCRIPT.START(remaining) });
                if (remaining.length === 0) {
                    history.push({ stepId: history.length, nodeId: id, line: 1, algoLine: 3, status: 'SUCCESS', desc: `Found permutation: ${current}`, current, remaining, speech: PERMUTATIONS_SCRIPT.SUCCESS(current) });
                } else {
                    for (let i = 0; i < remaining.length; i++) {
                        const char = remaining[i];
                        const nextRemaining = remaining.slice(0, i) + remaining.slice(i + 1);
                        const childNode = { id: ++nodeIdCounter, parentId: id, current: current + char, remaining: nextRemaining, children: [], depth: node.depth + 1 };
                        node.children.push(childNode);
                        history.push({ stepId: history.length, nodeId: id, line: 2, algoLine: 6, status: 'THINKING', desc: `Loop i=${i}: Pick "${char}"`, current, remaining, loopIndex: i, speech: PERMUTATIONS_SCRIPT.THINKING(char) });
                        solve(childNode);
                    }
                }
                history.push({ stepId: history.length, nodeId: id, line: 3, algoLine: null, status: 'RETURNING', desc: `Return from permute("${current}")`, current, remaining, speech: PERMUTATIONS_SCRIPT.RETURNING(current) });
            }
            solve(treeRoot);
        ...
        const WordVisualizer = ({ current, remaining, loopIndex }) => (
            <div className="flex flex-col items-center w-full gap-4 py-2">
                <div className="flex flex-col items-center gap-1"><span className="text-[10px] uppercase font-bold text-slate-500 tracking-widest">Built String</span><div className="flex gap-1.5">{current.split('').map((char, i) => (<div key={i} className="w-8 h-10 bg-emerald-500/20 border border-emerald-500/50 rounded flex items-center justify-center text-emerald-400 font-bold text-lg shadow-[0_0_10px_rgba(16,185,129,0.2)]">{char}</div>))}{current.length === 0 && <div className="w-8 h-10 border-2 border-dashed border-slate-700 rounded flex items-center justify-center text-slate-600 font-bold">_</div>}</div></div>
                <div className="w-full h-px bg-slate-700/50 relative"><div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-slate-800 px-2 text-[9px] font-bold text-slate-500">POOL</div></div>
                <div className="flex flex-col items-center gap-1"><div className="flex gap-1.5">{remaining.split('').map((char, i) => (<div key={i} className={`w-8 h-10 border rounded flex items-center justify-center font-bold text-lg transition-all duration-300 ${loopIndex === i ? 'bg-yellow-500 border-yellow-400 text-slate-900 scale-110 shadow-[0_0_15px_rgba(234,179,8,0.4)]' : 'bg-slate-700 border-slate-600 text-slate-300'}`}>{char}</div>))}{remaining.length === 0 && <div className="text-emerald-400 text-xs font-bold uppercase tracking-widest">Complete!</div>}</div></div>
            </div>
        );
        ...
        const NodeComponent = ({ nodeData, isActive, isBreadcrumb, latestState }) => {
            ...
            const { current, remaining } = nodeData;
            let highlightLine = -1, statusColor = "bg-slate-700", statusText = "IDLE", borderColor = "border-slate-700", shadow = "", opacity = 0.3, isWaiting = false, loopIndex = -1;
            if (latestState) {
                if (isActive) {
                    opacity = 1; shadow = 'shadow-yellow-500/20'; highlightLine = latestState.line; loopIndex = latestState.loopIndex !== undefined ? latestState.loopIndex : -1;
                    if (latestState.status === 'START') { statusColor = "bg-emerald-600"; statusText = "START"; borderColor="border-emerald-500"; }
                    if (latestState.status === 'SUCCESS') { statusColor = "bg-emerald-600"; statusText = "PERM!"; borderColor="border-emerald-500"; }
                    if (latestState.status === 'RETURNING') { statusColor = "bg-rose-600"; statusText = "RETURN"; borderColor="border-rose-500"; }
                    if (latestState.status === 'THINKING') { statusColor = "bg-blue-600"; statusText = "PICKING"; borderColor="border-blue-500"; }
                } else {
                    if (latestState.status === 'RETURNING') { statusColor = "bg-slate-600"; statusText = "DONE"; borderColor = "border-slate-600"; opacity = 0.8; }
            ...
            return (
                <div className={`absolute rounded-xl shadow-2xl overflow-hidden transition-all duration-300 border-2 bg-slate-800 flex flex-col justify-between ${borderColor} ${shadow}`} style={{ left: nodeData.x, top: nodeData.y, width: NODE_WIDTH, height: NODE_HEIGHT, transform: isActive ? 'scale(1.05)' : 'scale(1)', zIndex: isActive ? 50 : (isBreadcrumb ? 40 : 10), opacity: opacity, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                    <div className="bg-slate-900/80 px-4 py-2 flex justify-between items-center border-b border-slate-700"><span className={`font-mono font-bold text-sm ${isActive || isBreadcrumb ? 'text-blue-200' : 'text-slate-400'}`}>permute(len={current.length})</span><span className={`text-[10px] font-bold px-2 py-0.5 rounded ${statusColor} text-white shadow-sm`}>{statusText}</span></div>
                    <div className="px-3 py-2 text-[10px] font-mono text-slate-400 bg-slate-900 leading-relaxed border-b border-slate-700">
                        <div className={`px-2 rounded ${highlightLine === 0 ? 'code-highlight text-yellow-200' : ''}`}>1. if (rem == "") print(curr)</div>
                        <div className={`px-2 rounded ${highlightLine === 2 ? (isWaiting ? 'code-highlight-waiting text-indigo-300' : 'code-highlight text-yellow-200') : ''}`}>2. for each char in rem:</div>
                        <div className={`px-2 rounded ${highlightLine === 2 ? (isWaiting ? 'code-highlight-waiting text-indigo-300' : 'code-highlight text-yellow-200') : ''}`}>&nbsp;&nbsp;permute(curr+c, rem-c)</div>
                        <div className={`px-2 rounded ${highlightLine === 3 ? 'code-highlight text-yellow-200' : ''}`}>3. // return</div>
                    </div>
                    <div className="bg-slate-800 flex-1 relative p-3 flex flex-col justify-center"><WordVisualizer current={current} remaining={remaining} loopIndex={loopIndex} /></div>
                </div>
            );
        };
        ...
        const App = () => {
            ...
            const { history, flatNodes } = useMemo(() => {
                const { treeRoot, history } = generateSimulation(word);
            ...
            const handleGlobalMouseUp = () => {setIsDraggingCanvas(false); setIsDraggingPanel(false);};
            const handleCanvasMouseDown = (e) => {if(!e.target.closest('.control-panel-wrapper') && !e.target.closest('.top-nav-area')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}};
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={handleGlobalMouseMove} onMouseUp={handleGlobalMouseUp} onMouseLeave={handleGlobalMouseUp}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">Recursive Word Permutations</h1><span className="text-xs text-slate-400 font-mono">Letter shuffling visualizer</span><button onClick={() => setIsSettingsOpen(true)} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors text-slate-400 hover:text-white shadow-lg"><SettingsIcon size={20}/></button></div></div>
                            <div className="flex items-center gap-6"><div className="flex items-center gap-2 text-sm bg-slate-800/80 p-1 pr-3 rounded-lg border border-slate-700/50"><span className="bg-slate-700/80 px-2 py-1 rounded text-slate-300 text-xs font-bold uppercase mr-1">Word</span><input type="text" value={word} onChange={e => { setWord(e.target.value.toUpperCase()); setCurrentStep(0); setIsPlaying(false); }} className="bg-transparent font-bold outline-none text-blue-400 w-20 border-b border-blue-500/30" maxLength="4" /></div></div>
                        </div>
                    </div>
                    
                    <SettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} />
                    <AlgorithmPanel algoKey="PERMUTATIONS" currentLine={currentState?.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.1, z-e.deltaY*0.001), 1.5))} onMouseDown={handleCanvasMouseDown}>
