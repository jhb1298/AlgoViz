...
E F`;
        ...
        function generateSimulation(graph) {
            ...
            history.push({ 
                ...
                status: 'START', 
                algoLine: 1,
                desc: "Checking if Bipartite...", 
                speech: BIPARTITE_SCRIPT.START() 
            });
            ...
            for (const startNode of nodes) {
                ...
                history.push({ 
                    ...
                    status: 'COLORING', 
                    algoLine: 2,
                    desc: `Starting at ${startNode}, color: Blue`, 
                    speech: BIPARTITE_SCRIPT.COLORING(startNode, 0) 
                });
                    ...
                    for (const v of adj[u]) {
                        ...
                        if (!(v in colors)) {
                            ...
                            history.push({ 
                                ...
                                status: 'COLORING', 
                                algoLine: 5,
                                desc: `Coloring ${v} with ${colors[v] === 0 ? 'Blue' : 'Pink'}`, 
                                speech: BIPARTITE_SCRIPT.COLORING(v, colors[v]) 
                            });
                            ...
                            history.push({ 
                                ...
                                status: 'IMBALANCE', 
                                algoLine: 7,
                                desc: `Conflict found between ${u} and ${v}!`, 
                                speech: BIPARTITE_SCRIPT.IMBALANCE(u, v) 
                            });
                            return history;
                        }
                    }
            ...
            if (isBipartite) {
                // Final step: rearrange into bipartite form
                history.push({ 
                    stepId: history.length, 
                    colors: { ...colors }, 
                    status: 'REARRANGING', 
                    algoLine: 8,
                    desc: "Redrawing in Bipartite Form...", 
                    speech: BIPARTITE_SCRIPT.BIPARTITE_FORM() 
                });
                ...
                history.push({ 
                    ...
                    status: 'SUCCESS', 
                    algoLine: 8,
                    isBipartiteForm: true,
                    desc: "Bipartite Detection Complete!", 
                    speech: BIPARTITE_SCRIPT.SUCCESS() 
                });
            }
        ...
        const NodeComponent = ({ node, color, isActive, isBipartiteForm, setA, setB }) => {
            ...
            if (isBipartiteForm) {
                if (color === 0) {
                    const index = setA.indexOf(node.id);
                    x = 300;
                    y = 200 + index * 120;
                } else {
                    const index = setB.indexOf(node.id);
                    x = 700;
                    y = 200 + index * 120;
                }
            }
            ...
            return (
                <div className={`absolute w-20 h-20 rounded-full bg-slate-800 border-4 flex flex-col items-center justify-center node-transition z-20 ${isActive ? 'node-active' : (color === 0 ? 'node-color-0' : (color === 1 ? 'node-color-1' : 'border-slate-700'))}`} style={{ left: x - 40, top: y - 40, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                    <span className="text-xl font-black text-white">{node.id}</span>
                </div>
            );
        };

        const EdgeComponent = ({ edge, nodes, isActive, isConflict, isBipartiteForm, colors, setA, setB }) => {
            const u = nodes.find(n => n.id === edge.from);
            const v = nodes.find(n => n.id === edge.to);
            if(!u || !v) return null;

            let uX = u.x, uY = u.y, vX = v.x, vY = v.y;

            if (isBipartiteForm) {
                if (colors[u.id] === 0) {
                    uX = 300; uY = 200 + setA.indexOf(u.id) * 120;
                } else {
                    uX = 700; uY = 200 + setB.indexOf(u.id) * 120;
                }

                if (colors[v.id] === 0) {
                    vX = 300; vY = 200 + setA.indexOf(v.id) * 120;
                } else {
                    vX = 700; vY = 200 + setB.indexOf(v.id) * 120;
                }
            }

            return (
                <line x1={uX} y1={uY} x2={vX} y2={vY} stroke={isConflict ? "#f43f5e" : (isActive ? "#6366f1" : "#1e293b")} strokeWidth={isConflict ? 6 : (isActive ? 4 : 2)} className={`node-transition ${isActive ? 'edge-active' : (isConflict ? 'edge-conflict' : '')}`} />
            );
        };
        ...
        const App = () => {
            ...
            const state = history[currentStep] || { colors: {} };
            const isBipartiteForm = state.isBipartiteForm;

            const setA = useMemo(() => Object.keys(state.colors).filter(id => state.colors[id] === 0).sort(), [state.colors]);
            const setB = useMemo(() => Object.keys(state.colors).filter(id => state.colors[id] === 1).sort(), [state.colors]);
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <AlgorithmPanel algoKey="BIPARTITE" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper') && !e.target.closest('.top-nav-area')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`node-container absolute left-0 top-0 w-full h-full ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            <svg className="absolute top-0 left-0 w-full h-full overflow-visible">
                                {graph.edges.map((edge, i) => {
                                    const isConflict = state.status === 'IMBALANCE' && ((edge.from === state.activeNode && edge.to === state.targetNode) || (edge.from === state.targetNode && edge.to === state.activeNode));
                                    const isActive = state.activeEdge && ((edge.from === state.activeEdge.from && edge.to === state.activeEdge.to) || (edge.from === state.activeEdge.to && edge.to === state.activeEdge.from));
                                    return <EdgeComponent key={i} edge={edge} nodes={graph.nodes} isActive={isActive} isConflict={isConflict} isBipartiteForm={isBipartiteForm} colors={state.colors} setA={setA} setB={setB} />;
                                })}
                            </svg>
                            {graph.nodes.map(node => <NodeComponent key={node.id} node={node} color={state.colors[node.id]} isActive={state.activeNode === node.id} isBipartiteForm={isBipartiteForm} setA={setA} setB={setB} />)}
                            
                            {isBipartiteForm && (
                                <React.Fragment>
                                    <text x="300" y="100" textAnchor="middle" className="text-2xl font-black fill-blue-400 uppercase tracking-widest">Set A</text>
                                    <text x="700" y="100" textAnchor="middle" className="text-2xl font-black fill-pink-400 uppercase tracking-widest">Set B</text>
                                </React.Fragment>
                            )}
                        </div>
                    </div>
