...
/* extracted from knapsack.html */
        ...
        const ITEMS = [
            { id: 0, name: "Gem", weight: 2, value: 3 },
            { id: 1, name: "Gold", weight: 3, value: 4 },
            { id: 2, name: "Statue", weight: 4, value: 5 },
            { id: 3, name: "Crown", weight: 5, value: 8 },
            { id: 4, name: "Sword", weight: 1, value: 2 },
        ];
        const CAPACITY = 7;
        ...
        function generateSimulation() {
            ...
            history.push({
                ...
                status: 'START',
                desc: 'Initialization',
                speech: DP_SCRIPT.KNAPSACK.START()
            });
            ...
            for (let i = 1; i <= n; i++) {
                for (let w = 1; w <= W; w++) {
                    ...
                    if (item.weight <= w) {
                        ...
                        if (include > exclude) {
                            dp[i][w] = include;
                            decision = `Include ${item.name}: ${include} vs ${exclude}`;
                            speech = DP_SCRIPT.KNAPSACK.INCLUDE(include, exclude);
                        } else {
                            dp[i][w] = exclude;
                            decision = `Exclude ${item.name}: ${include} vs ${exclude}`;
                            speech = DP_SCRIPT.KNAPSACK.EXCLUDE(exclude);
                        }
                    } else {
                        dp[i][w] = dp[i-1][w];
                        decision = `${item.name} is too heavy`;
                        speech = DP_SCRIPT.KNAPSACK.EXCLUDE(dp[i-1][w]);
                    }
                    ...
                    history.push({
                        ...
                        activeItem: i,
                        activeCap: w,
                        compareCells: item.weight <= w ? [[i-1, w-item.weight], [i-1, w]] : [[i-1, w]],
                        algoLine: item.weight <= w ? 4 : 6,
                        status: 'EVAL',
                        desc: decision,
                        speech: i % 2 === 0 && w === 1 ? DP_SCRIPT.KNAPSACK.CELL_EVAL(item.name, w) : null // Reduced speech frequency
                    });
                }
            }
            ...
            history.push({
                ...
                algoLine: null,
                status: 'BACKTRACK_START',
                desc: 'Backtracking to find items...',
                speech: DP_SCRIPT.KNAPSACK.BACKTRACK_START()
            });
            ...
            for (let i = n; i > 0 && res > 0; i--) {
                if (res !== dp[i - 1][w]) {
                    ...
                    history.push({
                        ...
                        algoLine: null,
                        status: 'SELECTED',
                        desc: `Selected ${item.name}`,
                        speech: DP_SCRIPT.KNAPSACK.ITEM_SELECTED(item.name)
                    });
                    res -= item.value;
                    w -= item.weight;
                }
            ...
            history.push({
                ...
                algoLine: null,
                status: 'SUCCESS',
                desc: `Optimal Value: ${dp[n][W]}`,
                speech: DP_SCRIPT.KNAPSACK.SUCCESS(dp[n][W])
            });
        ...
        const App = () => {
            ...
            const speak = (text) => {
                const delay = (100 / speedMultiplier);
                if (isMuted || !text) { 
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none">
                    <div className="absolute top-0 left-0 right-0 z-[60]">
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="font-bold text-lg leading-none tracking-tight">0/1 Knapsack</h1>
                                    <span className="text-xs text-slate-400 font-mono">Dynamic Programming Table</span>
                            ...
                            <div className="flex items-center gap-6">
                                <div className="text-right">
                                    <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Max Capacity</div>
                                    <div className="font-mono text-emerald-400 font-bold">{CAPACITY}kg</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <AlgorithmPanel algoKey="KNAPSACK" currentLine={state.algoLine} />
                    <div className="flex-1 relative overflow-hidden flex items-center justify-center p-20">
                        {/* Table */}
                        <div className="flex flex-col gap-1 items-center" style={{ transform: `scale(${zoom})` }}>
                            <div className="flex gap-1">
                                <div className="w-16 h-12 flex items-center justify-center text-[10px] font-bold text-slate-500 uppercase">Cap ?</div>
                                {Array.from({length: CAPACITY + 1}).map((_, i) => (
                                    <div key={i} className="w-16 h-12 flex items-center justify-center font-bold text-slate-500">{i}</div>
                                ))}
                            </div>
                            {state.dp.map((row, i) => (
                                <div key={i} className="flex gap-1">
                                    <div className="w-16 h-12 flex items-center justify-center text-xs font-bold text-slate-500">{i === 0 ? "Empty" : ITEMS[i-1].name}</div>
                                    {row.map((val, w) => {
                                        const isActive = state.activeItem === i && state.activeCap === w;
                                        const isCompare = state.compareCells?.some(c => c[0] === i && c[1] === w);
                                        const isSelected = state.selectedItems?.has(i-1) && state.activeItem === i && state.activeCap === w;
                                        return (
                                            <div key={w} className={`w-16 h-12 flex items-center justify-center border border-slate-700 rounded-lg font-mono text-lg dp-cell ${isActive ? 'dp-cell-active' : ''} ${isCompare ? 'dp-cell-compare' : ''} ${val > 0 && !isActive ? 'dp-cell-best' : ''}`}>
                                                {val}
                                            </div>
                                        );
                                    })}
                        ...
                        {/* Item Sidebar */}
                        <div className="absolute left-6 top-24 w-48 flex flex-col gap-2">
                            <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Available Items</h3>
                            {ITEMS.map((item, i) => {
                                const isActive = state.activeItem === i + 1;
                                const isSelected = state.selectedItems?.has(i);
                                return (
                                    <div key={item.id} className={`p-3 rounded-xl border border-slate-700 bg-slate-800 transition-all ${isActive ? 'item-row-active shadow-lg shadow-blue-500/10' : ''} ${isSelected ? 'item-row-selected border-emerald-500/50' : ''}`}>
                            ...
                            {isMinimized ? (
                                ...
                                <>
                                    ...
                                    <div className="flex items-center justify-between w-full">
                                        <div className="flex flex-col w-[30%]"><span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Decision</span><span className="text-sm font-mono text-emerald-300 truncate">{state.desc}</span></div>
