...
/* extracted from bellmanford.html */
...
D B -2`;
        ...
        function parseGraph(text) {
            ...
            const nodes = nodesArr.map((id, i) => {
                ...
                return { id, x: 500 + 220 * Math.cos(angle), y: 500 + 220 * Math.sin(angle) };
            });
            return { nodes, edges };
        }
        ...
        function generateSimulation(graph) {
            ...
            nodes.forEach(n => { distances[n] = Infinity; parents[n] = null; });
            const source = nodes[0];
            distances[source] = 0;

            const n = nodes.length;
            history.push({ stepId: 0, iteration: 0, distances: { ...distances }, parents: { ...parents }, status: 'START', algoLine: 1, desc: `Starting at Source ${source}`, speech: BELLMANFORD_SCRIPT.START(n-1) });

            for (let i = 1; i < n; i++) {
                history.push({ stepId: history.length, iteration: i, distances: { ...distances }, parents: { ...parents }, status: 'PASS', algoLine: 2, desc: `Pass ${i}`, speech: BELLMANFORD_SCRIPT.PASS_START(i) });
                for (const edge of graph.edges) {
                    const u = edge.from;
                    const v = edge.to;
                    history.push({ stepId: history.length, iteration: i, activeEdge: edge, activeNode: v, distances: { ...distances }, parents: { ...parents }, status: 'CHECKING', algoLine: 3, desc: `Checking edge ${u}->${v}` });

                    if (distances[u] !== Infinity && distances[u] + edge.weight < distances[v]) {
                        distances[v] = distances[u] + edge.weight;
                        parents[v] = u;
                        history.push({ stepId: history.length, iteration: i, activeEdge: edge, activeNode: v, distances: { ...distances }, parents: { ...parents }, status: 'RELAXING', algoLine: 5, desc: `Updated ${v} to ${distances[v]}`, speech: BELLMANFORD_SCRIPT.RELAXING(u, v, distances[v]) });
                    }
                }
            }

            history.push({ stepId: history.length, iteration: n, distances: { ...distances }, parents: { ...parents }, status: 'PASS', algoLine: 6, desc: "Final cycle check", speech: BELLMANFORD_SCRIPT.CYCLE_CHECK() });
            for (const edge of graph.edges) {
                if (distances[edge.from] !== Infinity && distances[edge.from] + edge.weight < distances[edge.to]) {
                    history.push({ stepId: history.length, iteration: n, activeEdge: edge, activeNode: edge.to, distances: { ...distances }, parents: { ...parents }, status: 'CYCLE', algoLine: 8, desc: "Negative cycle detected!", speech: BELLMANFORD_SCRIPT.CYCLE_FOUND() });
                    return history;
                }
            }

            history.push({ stepId: history.length, iteration: n, status: 'SUCCESS', algoLine: 9, distances: { ...distances }, parents: { ...parents }, desc: "Bellman-Ford Complete", speech: BELLMANFORD_SCRIPT.SUCCESS() });
            return history;
        }

        const NodeComponent = ({  node, dist, isActive, isUpdated  }) => {
            ...
            return (
            <div className={`absolute w-20 h-20 rounded-xl bg-slate-800 border-4 flex flex-col items-center justify-center transition-all duration-500 z-20 ${isActive ? 'border-blue-500 scale-110 shadow-lg' : (isUpdated ? 'node-updated' : 'border-slate-700')}`} style={{ left: node.x - 40, top: node.y - 40, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                <span className="text-xl font-black text-white">{node.id}</span>
                <span className={`text-xs font-mono font-bold ${dist === Infinity ? 'text-slate-500' : 'text-rose-400'}`}>{dist === Infinity ? '8' : dist}</span>
            </div>
        );
        };

        const EdgeComponent = ({ edge, nodes, isActive, isShortest }) => {
            const [settings, setSettings] = useState(JSON.parse(localStorage.getItem('simulation_settings') || '{"fontSize": 16, "nodeColor": "#3b82f6"}'));
            useEffect(() => {
                const handleUpdate = () => setSettings(JSON.parse(localStorage.getItem('simulation_settings') || '{"fontSize": 16, "nodeColor": "#3b82f6"}'));
                window.addEventListener('simulation_settings_updated', handleUpdate);
            ...
            const headlen = 15;
            const x2 = v.x - 45 * Math.cos(angle);
            const y2 = v.y - 45 * Math.sin(angle);
            let strokeColor = isActive ? "#fb7185" : (isShortest ? "#10b981" : "#1e293b");
            return (
                <g>
                    <line x1={u.x} y1={u.y} x2={x2} y2={y2} stroke={strokeColor} strokeWidth={isActive ? 4 : (isShortest ? 5 : 2)} className={isActive ? 'edge-active' : (isShortest ? 'edge-shortest' : '')} style={{ transition: 'stroke 0.3s, stroke-width 0.3s' }} />
                    <polygon points={`${x2},${y2} ${x2-headlen*Math.cos(angle-Math.PI/6)},${y2-headlen*Math.sin(angle-Math.PI/6)} ${x2-headlen*Math.cos(angle+Math.PI/6)},${y2-headlen*Math.sin(angle+Math.PI/6)}`} fill={strokeColor} />
                    <rect x={midX-12} y={midY-10} width="24" height="20" rx="4" fill="#0f172a" />
                    <text x={midX} y={midY} textAnchor="middle" dy=".3em" fontSize={settings.fontSize} fontWeight="bold" fill={isShortest ? "#10b981" : "#94a3b8"}>{edge.weight}</text>
                </g>
            );
        };
        ...
        const App = () => {
            ...
            const state = history[currentStep] || { distances: {}, parents: {}, iteration: 0 };
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">Bellman-Ford Algorithm</h1><span className="text-xs text-slate-400 font-mono">Distance Update Table</span><button onClick={() => setIsSettingsOpen(true)} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors text-slate-400 hover:text-white shadow-lg"><SettingsIcon size={20}/></button></div></div>
                            <div className="flex items-center gap-4">
                                <div className="bg-rose-500/10 border border-rose-500/30 px-3 py-1.5 rounded-lg flex items-center gap-2">
                                    <span className="text-[10px] font-black uppercase text-rose-400 tracking-wider">Iteration</span>
                                    <span className="text-lg font-mono font-black text-white leading-none">{state.iteration}</span>
                                </div>
                                <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 bg-rose-600 hover:bg-rose-500 px-4 py-2 rounded-lg font-bold transition-all shadow-lg shadow-rose-900/20"><Edit size={18}/> Edit Graph</button>
                            </div>
                        </div>
                    </div>
                    ...
                    {isEditing && (
                        <div className="absolute inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-6">
                            <div className="bg-slate-900 border border-slate-700 rounded-3xl p-8 max-w-lg w-full shadow-2xl">
                                ...
                                <textarea value={inputText} onChange={(e) => setInputText(e.target.value)} className="w-full h-64 bg-slate-950 border border-rose-400 outline-none focus:border-rose-500 mb-6" />
                                <div className="flex justify-end gap-4"><button onClick={() => setIsEditing(false)} className="px-6 py-2 rounded-xl font-bold text-slate-400 hover:bg-slate-800">Cancel</button><button onClick={() => { setIsEditing(false); setCurrentStep(0); setIsPlaying(false); }} className="px-8 py-2 rounded-xl font-bold bg-rose-600 hover:bg-rose-500 shadow-lg shadow-rose-900/40">Apply Graph</button></div>
                            </div>
                        </div>
                    )}

                    {/* DISTANCE TABLE */}
                    <div className="absolute top-20 right-6 z-40 bg-slate-800/80 backdrop-blur-md p-4 rounded-2xl border border-slate-700 shadow-xl w-48 transition-all duration-300">
                        <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">Distances</h3>
                        <div className="flex flex-col gap-1">
                            {Object.keys(state.distances).sort().map(nodeId => (
                                <div key={nodeId} className={`flex justify-between items-center px-2 py-1 rounded transition-colors ${state.activeNode === nodeId ? 'table-row-active' : ''}`}>
                                    <span className="font-bold text-slate-300">{nodeId}</span>
                                    <span className={`font-mono ${state.distances[nodeId] === Infinity ? 'text-slate-600' : 'text-emerald-400 font-bold'}`}>
                                        {state.distances[nodeId] === Infinity ? '8' : state.distances[nodeId]}
                                    </span>
                                </div>
                            ))}
                        </div>
                    ...
                    <AlgorithmPanel algoKey="BELLMANFORD" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper') && !e.target.closest('.top-nav-area')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`node-container absolute origin-top-left ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            <svg className="absolute top-0 left-0 overflow-visible" style={{ width: 1000, height: 1000 }}>
                                {graph.edges.map((edge, i) => <EdgeComponent key={i} edge={edge} nodes={graph.nodes} isActive={state.activeEdge === edge} isShortest={state.parents[edge.to] === edge.from} />)}
                            </svg>
                            {graph.nodes.map(node => <NodeComponent key={node.id} node={node} dist={state.distances[node.id]} isActive={state.activeNode === node.id} isUpdated={state.status === 'RELAXING' && state.activeNode === node.id} />)}
                        </div>
                    </div>
