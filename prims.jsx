...
D F 4`;
        ...
        function generateSimulation(graph) {
            ...
            history.push({ stepId: 0, activeNode: startNode, visited: new Set(visited), mstEdges: [], status: 'START', algoLine: 1, desc: `Starting at node ${startNode}`, speech: PRIMS_SCRIPT.START(startNode) });

            while (visited.size < nodes.length) {
                let minEdge = null;
                let minWeight = Infinity;

                history.push({ stepId: history.length, visited: new Set(visited), mstEdges: [...mstEdges], status: 'SEARCH', algoLine: 5, desc: "Looking for cheapest edge...", speech: PRIMS_SCRIPT.EXPLORING(Array.from(visited).join(', ')) });

                for (const edge of graph.edges) {
                    const uIn = visited.has(edge.from);
                    const vIn = visited.has(edge.to);
                ...
                if (minEdge) {
                    ...
                    history.push({ 
                        ...
                        status: 'PICKING', 
                        algoLine: 6,
                        desc: `Added edge ${minEdge.from}-${minEdge.to}`, 
                        speech: PRIMS_SCRIPT.PICKING(minEdge.from, minEdge.to, minEdge.weight) 
                    });
                } else {
            ...
            history.push({ stepId: history.length, status: 'SUCCESS', algoLine: 8, visited: new Set(visited), mstEdges: [...mstEdges], desc: "MST Complete", speech: PRIMS_SCRIPT.SUCCESS(total) });
            return history;
        }

        const NodeComponent = ({  node, isActive, isVisited  }) => {
            ...
            return (
            <div className={`absolute w-20 h-20 rounded-full bg-slate-800 border-4 flex flex-col items-center justify-center transition-all duration-500 z-20 ${isActive ? 'node-active' : (isVisited ? 'node-in-mst' : 'border-slate-700')}`} style={{ left: node.x - 40, top: node.y - 40, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                <span className="text-xl font-black text-white">{node.id}</span>
            </div>
        );
        };

        const EdgeComponent = ({ edge, nodes, isActive, isMst }) => {
            const [settings, setSettings] = useState(JSON.parse(localStorage.getItem('simulation_settings') || '{"fontSize": 16, "nodeColor": "#3b82f6"}'));
            useEffect(() => {
                const handleUpdate = () => setSettings(JSON.parse(localStorage.getItem('simulation_settings') || '{"fontSize": 16, "nodeColor": "#3b82f6"}'));
                window.addEventListener('simulation_settings_updated', handleUpdate);
            ...
            return (
                <g>
                    <line x1={u.x} y1={u.y} x2={v.x} y2={v.y} stroke={isMst ? "#10b981" : (isActive ? "#6366f1" : "#1e293b")} strokeWidth={isMst ? 6 : (isActive ? 4 : 2)} className={isMst ? 'edge-mst' : (isActive ? 'edge-active' : '')} style={{ transition: 'stroke 0.3s, stroke-width 0.3s' }} />
                    <circle cx={(u.x+v.x)/2} cy={(u.y+v.y)/2} r="12" fill="#0f172a" />
                    <text x={(u.x+v.x)/2} y={(u.y+v.y)/2} textAnchor="middle" dy=".3em" fontSize={settings.fontSize} fontWeight="bold" fill={isMst ? "#10b981" : "#94a3b8"}>{edge.weight}</text>
                </g>
            );
        };
        ...
        const App = () => {
            ...
            const state = history[currentStep] || { visited: new Set(), mstEdges: [] };
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">Prim's Algorithm</h1><span className="text-xs text-slate-400 font-mono">Minimum Spanning Tree</span><button onClick={() => setIsSettingsOpen(true)} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors text-slate-400 hover:text-white shadow-lg"><SettingsIcon size={20}/></button></div></div>
                            <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 px-4 py-2 rounded-lg font-bold transition-all shadow-lg shadow-emerald-900/20"><Edit size={18}/> Edit Graph</button>
                        </div>
                    </div>
                    ...
                    {isEditing && (
                        <div className="absolute inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-6">
                            <div className="bg-slate-900 border border-slate-700 rounded-3xl p-8 max-w-lg w-full shadow-2xl">
                                ...
                                <p className="text-slate-400 text-sm mb-4">Format: <code className="bg-slate-800 px-1.5 py-0.5 rounded text-emerald-300">Node1 Node2 Weight</code></p>
                                <textarea value={inputText} onChange={(e) => setInputText(e.target.value)} className="w-full h-64 bg-slate-950 border border-slate-700 rounded-xl p-4 font-mono text-emerald-400 outline-none focus:border-emerald-500 mb-6" />
                                <div className="flex justify-end gap-4"><button onClick={() => setIsEditing(false)} className="px-6 py-2 rounded-xl font-bold text-slate-400">Cancel</button><button onClick={() => { setIsEditing(false); setCurrentStep(0); setIsPlaying(false); }} className="px-8 py-2 rounded-xl font-bold bg-emerald-600 hover:bg-emerald-500 shadow-lg shadow-emerald-900/40">Apply Graph</button></div>
                            </div>
                        </div>
                    )}
                    ...
                    <AlgorithmPanel algoKey="PRIMS" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper') && !e.target.closest('.top-nav-area')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`node-container absolute origin-top-left ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            <svg className="absolute top-0 left-0 overflow-visible" style={{ width: 1000, height: 1000 }}>
                                {graph.edges.map((edge, i) => <EdgeComponent key={i} edge={edge} nodes={graph.nodes} isActive={state.activeEdge === edge} isMst={state.mstEdges.includes(edge)} />)}
                            </svg>
                            {graph.nodes.map(node => <NodeComponent key={node.id} node={node} isActive={state.activeNode === node.id} isVisited={state.visited.has(node.id)} />)}
                        </div>
                    </div>
