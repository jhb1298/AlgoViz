...
E F 5`;
        ...
        function parseGraph(text) {
            ...
            const nodes = nodesArr.map((id, i) => {
                // For A*, let's place nodes more spread out to make heuristic meaningful
                const angle = (i / nodesArr.length) * 2 * Math.PI;
                return { id, x: 500 + 350 * Math.cos(angle), y: 500 + 350 * Math.sin(angle) };
            });
            return { nodes, edges };
        }

        function getHeuristic(nodeId, goalId, nodes) {
            const u = nodes.find(n => n.id === nodeId);
            const v = nodes.find(n => n.id === goalId);
            if (!u || !v) return 0;
            // Euclidean distance scaled down to be comparable to edge weights
            return Math.floor(Math.sqrt(Math.pow(u.x - v.x, 2) + Math.pow(u.y - v.y, 2)) / 50);
        }
        ...
        function generateSimulation(graph) {
            ...
            gScore[source] = 0;
            fScore[source] = getHeuristic(source, goal, graph.nodes);
            ...
            history.push({ 
                ...
                visited: new Set(), 
                status: 'START', 
                desc: `Starting A* Search from ${source} to ${goal}`, 
                speech: SEARCH_SCRIPT.ASTAR.START(),
                algoLine: 1
            });
            ...
            while (openSet.length > 0) {
                ...
                if (u === goal) {
                    ...
                    history.push({ 
                        ...
                        status: 'TARGET_FOUND', 
                        desc: `Goal ${goal} reached!`, 
                        speech: SEARCH_SCRIPT.ASTAR.TARGET_FOUND(),
                        algoLine: 4
                    });
                    break;
                }
                ...
                history.push({ 
                    ...
                    visited: new Set(visited), 
                    status: 'VISITING', 
                    desc: `Visiting ${u} (f=${fScore[u]})`, 
                    speech: SEARCH_SCRIPT.ASTAR.VISITING(u, fScore[u], gScore[u], fScore[u] - gScore[u]),
                    algoLine: 3
                });
                ...
                for (const edge of neighbors) {
                    ...
                    const tentativeGScore = gScore[u] + edge.weight;
                    if (tentativeGScore < gScore[v]) {
                        parents[v] = u;
                        gScore[v] = tentativeGScore;
                        const h = getHeuristic(v, goal, graph.nodes);
                        fScore[v] = gScore[v] + h;
                        ...
                        history.push({ 
                            ...
                            visited: new Set(visited), 
                            status: 'RELAXING', 
                            desc: `Updating ${v}: g=${gScore[v]}, f=${fScore[v]}`, 
                            speech: SEARCH_SCRIPT.ASTAR.RELAXING(v, fScore[v]),
                            algoLine: 8
                        });
                    } else {
                        history.push({ 
            ...
            history.push({ 
                ...
                speech: SEARCH_SCRIPT.ASTAR.SUCCESS(),
                algoLine: 2
            });
            return history;
        }

        const NodeComponent = ({ node, g, f, isActive, isVisited, isGoal }) => {
            const [settings, setSettings] = useState(JSON.parse(localStorage.getItem('simulation_settings') || '{"fontSize": 16, "nodeColor": "#3b82f6"}'));
            useEffect(() => {
                const handleUpdate = () => {
                    setSettings(JSON.parse(localStorage.getItem('simulation_settings') || '{"fontSize": 16, "nodeColor": "#3b82f6"}'));
                };
            ...
            return (
                <div className={`absolute w-20 h-20 rounded-full bg-slate-800 border-4 flex flex-col items-center justify-center transition-all duration-500 z-20 ${isActive ? 'node-active' : (isGoal ? 'node-target' : (isVisited ? 'node-visited' : 'border-slate-700'))}`} style={{ left: node.x - 40, top: node.y - 40, borderColor: isActive ? settings.nodeColor : (isGoal ? '#f59e0b' : (isVisited ? '#10b981' : '#334155')), boxShadow: isActive ? `0 0 25px ${settings.nodeColor}80` : '', fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                    <span className="font-black text-white" style={{ fontSize: settings.fontSize + 'px' }}>{node.id}</span>
                    <div className="flex flex-col items-center leading-none">
                        <span className={`text-[10px] font-mono font-bold ${g === Infinity ? 'text-slate-500' : 'text-blue-400'}`}>g: {g === Infinity ? '8' : g}</span>
                        <span className={`text-[10px] font-mono font-bold ${f === Infinity ? 'text-slate-500' : 'text-amber-400'}`}>f: {f === Infinity ? '8' : f}</span>
                    </div>
                </div>
            );
        };
        ...
        const EdgeComponent = ({ edge, nodes, isActive, isShortest }) => {
            ...
            let strokeColor = isActive ? "#6366f1" : (isShortest ? "#10b981" : "#1e293b");
            return (
                <g>
                    <line x1={u.x} y1={u.y} x2={v.x} y2={v.y} stroke={strokeColor} strokeWidth={isActive ? 4 : (isShortest ? 5 : 2)} className={isActive ? 'edge-active' : (isShortest ? 'edge-shortest' : '')} style={{ transition: 'stroke 0.3s, stroke-width 0.3s' }} />
                    <circle cx={midX} cy={midY} r="12" fill="#0f172a" />
        ...
        const App = () => {
            ...
            const state = history[currentStep] || { gScore: {}, fScore: {}, visited: new Set(), parents: {} };
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">A* Search Algorithm</h1><span className="text-xs text-slate-400 font-mono">Heuristic-based pathfinding</span></div></div>
                            <div className="flex items-center gap-3">
                                <button onClick={() => setIsSettingsOpen(true)} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors text-slate-400 hover:text-white shadow-lg"><SettingsIcon size={20}/></button>
                                <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 px-4 py-2 rounded-lg font-bold transition-all shadow-lg shadow-blue-900/20"><Edit size={18}/> Edit Graph</button>
                            </div>
                        </div>
                    </div>

                    <SettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} />
                    <AlgorithmPanel algoKey="ASTAR" currentLine={state.algoLine} />
                    ...
                    {isEditing && (
                        <div className="absolute inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-6">
                            <div className="bg-slate-900 border border-slate-700 rounded-3xl p-8 max-w-lg w-full shadow-2xl">
                                ...
                                <div className="flex justify-end gap-4"><button onClick={() => setIsEditing(false)} className="px-6 py-2 rounded-xl font-bold text-slate-400">Cancel</button><button onClick={() => { setIsEditing(false); setCurrentStep(0); setIsPlaying(false); }} className="px-8 py-2 rounded-xl font-bold bg-blue-600">Apply Graph</button></div>
                            </div>
                        </div>
                    )}

                    {/* SCORE TABLE */}
                    <div className="absolute top-20 right-6 z-40 bg-slate-800/80 backdrop-blur-md p-4 rounded-2xl border border-slate-700 shadow-xl w-56 transition-all duration-300">
                        <div className="flex justify-between items-center mb-3">
                            <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">A* Scores (f = g + h)</h3>
                        </div>
                        <div className="flex flex-col gap-1 max-h-64 overflow-y-auto pr-1">
                            {Object.keys(state.fScore).sort().map(nodeId => (
                                <div key={nodeId} className={`flex justify-between items-center px-2 py-1 rounded transition-colors ${state.targetNode === nodeId ? 'table-row-active' : ''}`}>
                                    <span className="font-bold text-slate-300">{nodeId}</span>
                                    <div className="flex gap-2">
                                        <span className="text-[10px] text-slate-500 font-mono">g:{state.gScore[nodeId] === Infinity ? '8' : state.gScore[nodeId]}</span>
                                        <span className={`font-mono text-xs ${state.fScore[nodeId] === Infinity ? 'text-slate-600' : 'text-amber-400 font-bold'}`}>
                                            f:{state.fScore[nodeId] === Infinity ? '8' : state.fScore[nodeId]}
                                        </span>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className={`node-container absolute origin-top-left ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            <svg className="absolute top-0 left-0 overflow-visible" style={{ width: 1000, height: 1000 }}>
                                {graph.edges.map((edge, i) => {
                                    const isShortest = state.parents[edge.to] === edge.from || state.parents[edge.from] === edge.to;
                                    return <EdgeComponent key={i} edge={edge} nodes={graph.nodes} isActive={state.activeEdge === edge} isShortest={isShortest} />;
                                })}
                            </svg>
                            {graph.nodes.map(node => <NodeComponent key={node.id} node={node} g={state.gScore[node.id]} f={state.fScore[node.id]} isActive={state.activeNode === node.id} isVisited={state.visited.has(node.id)} isGoal={state.goalNode === node.id} />)}
                        </div>
                    </div>
