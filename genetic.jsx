...
/* extracted from genetic.html */
        ...
        const TARGET = "GENETIC ALGORITHM";
        ...
        function createIndividual() {
            let genes = "";
            for (let i = 0; i < TARGET.length; i++) genes += randomChar();
            return genes;
        }
        ...
        function generateSimulation() {
            ...
            history.push({
                stepId: 0,
                generation,
                population: population.map(p => ({ genes: p, fitness: getFitness(p) })),
                algoLine: 1,
                status: 'START',
                desc: 'Initializing Population',
                speech: OPTIMIZATION_SCRIPT.GENETIC.START()
            });

            while (!finished && generation < 200) {
                generation++;
                let popWithFitness = population.map(genes => ({ genes, fitness: getFitness(genes) }));
                popWithFitness.sort((a, b) => b.fitness - a.fitness);

                const best = popWithFitness[0];
                history.push({
                    ...
                    algoLine: 3,
                    status: 'NEW_GEN',
                    desc: `Generation ${generation}: Best Fitness ${Math.round(best.fitness * 100)}%`,
                    speech: generation % 10 === 0 ? OPTIMIZATION_SCRIPT.GENETIC.NEW_GENERATION(generation, best.fitness) : null
                });
                ...
                let nextPop = [best.genes]; // Elitism
                while (nextPop.length < POP_SIZE) {
                    let parentA = select(popWithFitness);
                    let parentB = select(popWithFitness);
            ...
            history.push({
                stepId: history.length,
                generation,
                population: population.map(genes => ({ genes, fitness: getFitness(genes) })),
                algoLine: 8,
                status: 'SUCCESS',
                desc: 'Target Reached!',
                speech: OPTIMIZATION_SCRIPT.GENETIC.SUCCESS(generation)
            });
        ...
        function mutate(genes) {
            let mutated = "";
            for (let i = 0; i < genes.length; i++) {
                mutated += Math.random() < MUTATION_RATE ? randomChar() : genes[i];
            }
            return mutated;
        }

        const IndividualCard = ({ individual, target }) => (
            <div className={`pop-card p-3 rounded-xl border border-slate-700 bg-slate-800 flex flex-col items-center gap-1 ${individual.fitness === 1 ? 'pop-card-best' : ''}`}>
                <div className="flex font-mono text-xs tracking-widest uppercase">
                    {individual.genes.split('').map((char, i) => (
                        <span key={i} className={char === target[i] ? 'dna-correct' : 'dna-wrong'}>{char}</span>
                    ))}
                </div>
                <div className="text-[9px] font-bold text-slate-500 uppercase tracking-tighter">Fitness: {Math.round(individual.fitness * 100)}%</div>
            </div>
        );
        ...
        const App = () => {
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none">
                    <div className="absolute top-0 left-0 right-0 z-[60]">
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="font-bold text-lg leading-none tracking-tight">Genetic Algorithm</h1>
                                    <span className="text-xs text-slate-400 font-mono">Survival of the Fittest</span>
                            ...
                            <div className="flex items-center gap-6">
                                <div className="text-right">
                                    ...
                                    <div className="font-mono text-emerald-400 font-bold tracking-widest">{TARGET}</div>
                                </div>
                                <div className="w-px h-8 bg-slate-800"></div>
                                <div className="text-center">
                                    <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Gen</div>
                                    <div className="font-mono text-blue-400 font-bold">{state.generation}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <AlgorithmPanel algoKey="GENETIC" currentLine={state?.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden flex items-center justify-center">
                        <div className="camera-layer grid grid-cols-4 lg:grid-cols-5 gap-4 p-10 max-w-6xl" style={{ transform: `scale(${zoom})` }}>
                            {state.population.map((ind, i) => (
                                <IndividualCard key={i} individual={ind} target={TARGET} />
                            ))}
                        </div>
                    </div>
                                ...
                                <>
                                    ...
                                    <div className="flex items-center justify-between w-full">
                                        ...
                                        <div className="flex items-center justify-end gap-4 w-[30%]">
                                            <button onClick={() => setZoom(z => Math.max(0.5, z - 0.1))} className="text-slate-400 hover:text-white">-</button>
