...
/* extracted from bst.html */
        ...
        const Trash = (props) => <IconWrapper {...props}><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></IconWrapper>;
        ...
        function generateSimulation(initialArr, opType, opVal) {
            ...
            function insert(node, val) {
                ...
                record('COMPARING', `Comparing ${val} with ${node.val}`, TREE_SCRIPT.BST.COMPARING(val, node.val), node.id);
            ...
            function search(node, val) {
                if (!node) { record('NOT_FOUND', `${val} not found`, TREE_SCRIPT.BST.NOT_FOUND(val)); return null; }
                record('COMPARING', `Comparing ${val} with ${node.val}`, TREE_SCRIPT.BST.SEARCH(val, node.val), node.id);
                if (val === node.val) { record('FOUND', `Found ${val}!`, TREE_SCRIPT.BST.FOUND(val), node.id); return node; }
            ...
            function remove(node, val) {
                ...
                if (val < node.val) node.left = remove(node.left, val);
                    ...
                    record('DELETE', `Deleting ${val}`, TREE_SCRIPT.BST.DELETE(val), node.id);
            ...
            function serializeTree(node) {
                if (!node) return null;
                return { val: node.val, id: node.id, left: serializeTree(node.left), right: serializeTree(node.right) };
            }

            // Build initial tree
            initialArr.forEach(v => { root = insert(root, v); });
            history.length = 0; // Clear setup history
            record('START', "Tree Ready", TREE_SCRIPT.BST.START(), null, 0);

            if (opType === 'INSERT') {
                const insertBST = (node, val) => {
                    if (!node) {
                        record('SUCCESS', `Created new node ${val}`, TREE_SCRIPT.BST.START(), null, 1);
                        return new Node(val);
                    }
                    record('COMPARING', `Comparing ${val} with ${node.val}`, TREE_SCRIPT.BST.COMPARING(val, node.val), node.id, 2);
                    if (val < node.val) {
                        record('LEFT', `Going left from ${node.val}`, null, node.id, 2);
                        node.left = insertBST(node.left, val);
                    } else if (val > node.val) {
                        record('RIGHT', `Going right from ${node.val}`, null, node.id, 3);
                        node.right = insertBST(node.right, val);
                    }
                    return node;
                };
                root = insertBST(root, opVal);
            }
            ...
            if (opType === 'SEARCH') {
                const searchBST = (node, val) => {
                    ...
                    record('RIGHT', `Checking right subtree`, null, node.id, 4);
                    return searchBST(node.right, val);
                };
                searchBST(root, opVal);
            }
            
            if (opType === 'DELETE') {
                const deleteBST = (node, val) => {
                    if (!node) return null;
                    record('COMPARING', `Searching for ${val}, at ${node.val}`, TREE_SCRIPT.BST.COMPARING(val, node.val), node.id, 1);
                    if (val < node.val) node.left = deleteBST(node.left, val);
                    else if (val > node.val) node.right = deleteBST(node.right, val);
                    else {
                        record('DELETE_TARGET', `Found node ${val} to delete`, TREE_SCRIPT.BST.DELETE(val), node.id, 2);
                        if (!node.left || !node.right) {
                            record('SUCCESS', `Removing node ${val}`, null, node.id, 2);
                            return node.left || node.right;
                        }
                        record('SUCCESSOR', `Finding in-order successor for ${val}`, null, node.id, 3);
                        let temp = findMin(node.right);
                        node.val = temp.val;
                        record('SWAP', `Swapping ${val} with successor ${temp.val}`, null, node.id, 4);
                        node.right = deleteBST(node.right, temp.val);
                    }
                    return node;
                };
                root = deleteBST(root, opVal);
            }

            return history;
        }

        const NodeComponent = ({  node, x, y, isActive, isFound, isDelete  }) => {
            ...
            return (
            <div className={`absolute w-14 h-14 rounded-full bg-slate-800 border-4 flex items-center justify-center font-black text-lg transition-all duration-500 z-20 ${isActive ? 'node-active' : (isFound ? 'node-found' : (isDelete ? 'node-delete' : 'border-slate-700'))}`} style={{ left: x - 28, top: y - 28, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                {node.val}
            </div>
        );
        };
        ...
        const App = () => {
            ...
            const [treeData, setTreeData] = useState([50, 30, 70, 20, 40, 60, 80]);
            ...
            const renderTree = (node, x, y, level) => {
                if (!node) return null;
                const spread = 600 / Math.pow(2, level);
                return (
                    <React.Fragment key={node.id}>
                        ...
                        <NodeComponent node={node} x={x} y={y} isActive={state.activeId === node.id} isFound={state.status === 'FOUND' && state.activeId === node.id} isDelete={state.status === 'DELETE' && state.activeId === node.id} />
                        {renderTree(node.left, x - spread, y + 100, level + 1)}
                        {renderTree(node.right, x + spread, y + 100, level + 1)}
                    </React.Fragment>
                );
            };
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button onClick={() => { setCurrentStep(0); setIsPlaying(true); }} className="bg-blue-600 hover:bg-blue-500 p-2 rounded-lg transition-all"><Play fill="white" size={18} /></button>
                            </div>
                        </div>
                    </div>
                    ...
                    <AlgorithmPanel algoKey={`BST_${opType}`} currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`absolute left-0 top-0 w-full h-full ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            <svg className="absolute top-0 left-0 w-full h-full overflow-visible">
                                {state.tree && renderTree(state.tree, 500, 100, 1)}
                            </svg>
                        </div>
                    </div>
