...
/* extracted from shellsort.html */
        ...
        function generateSimulation(arr) {
            ...
            history.push({ stepId: 0, arr: [...data], status: 'START', algoLine: 1, desc: "Starting Shell Sort", speech: SORTING_SCRIPT.SHELL.START() });

            for (let gap = Math.floor(n / 2); gap > 0; gap = Math.floor(gap / 2)) {
                history.push({ stepId: history.length, arr: [...data], gap, status: 'GAP', algoLine: 2, desc: `Gap set to ${gap}`, speech: SORTING_SCRIPT.SHELL.GAP(gap) });

                for (let i = gap; i < n; i++) {
                    let temp = data[i];
                    let j;
                    
                    history.push({ stepId: history.length, arr: [...data], gap, active: i, status: 'INSERT', algoLine: 3, desc: `Gap-inserting ${temp}`, speech: SORTING_SCRIPT.SHELL.INSERTION(temp, gap) });

                    for (j = i; j >= gap && data[j - gap] > temp; j -= gap) {
                        data[j] = data[j - gap];
                        history.push({ stepId: history.length, arr: [...data], gap, compare: [j, j - gap], status: 'SLIDE', algoLine: 5, desc: `Sliding ${data[j]} forward` });
                    }
                    data[j] = temp;
                    history.push({ stepId: history.length, arr: [...data], gap, placed: j, status: 'PLACE', algoLine: 6, desc: `Placed ${temp} at index ${j}` });
                }
            }

            history.push({ stepId: history.length, arr: [...data], status: 'SUCCESS', algoLine: 7, desc: "Sort Complete!", speech: SORTING_SCRIPT.SHELL.SUCCESS() });
            return history;
        }
        ...
        const App = () => {
            ...
            const [input, setInput] = useState("35,15,80,25,45,10,65,50,20,70");
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <AlgorithmPanel algoKey="SHELLSORT" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.5, z-e.deltaY*0.001), 2))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 flex items-end justify-center gap-2 ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(calc(-50% + ${manualOffset.x}px), calc(-50% + ${manualOffset.y}px)) scale(${zoom})` }}>
                            {state.arr.map((val, i) => {
                                ...
                                if (state.active === i) bgColor = "bg-indigo-500 shadow-[0_0_20px_rgba(99,102,241,0.5)]";
                                if (state.compare && state.compare.includes(i)) bgColor = "bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.4)]";
                                if (state.placed === i) bgColor = "bg-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.5)]";
                                
                                return (
                                    <div key={i} className={`flex flex-col items-center gap-2 bar-transition`} style={{ width: 50 }}>
                                        <div className={`w-full rounded-t-lg rounded-b-sm border border-white/10 ${bgColor}`} style={{ height: val * 2.5 }}></div>
                                        <span className="font-mono font-bold text-xs text-slate-400">{val}</span>
                                    </div>
                                );
                            })}
                        </div>
