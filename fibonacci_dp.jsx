...
/* extracted from fibonacci_dp.html */
        ...
        function generateSimulation(n) {
            ...
            function solve(val, depth, parentId = null) {
                ...
                const node = { id, parentId, val, children: [], depth, isMemo: false };
                
                history.push({ stepId: history.length, nodeId: id, val, memo: { ...memo }, line: 0, algoLine: 1, status: 'START', desc: `fib(${val}) call`, speech: val === n ? FIBONACCI_SCRIPT.DP_START(val) : FIBONACCI_SCRIPT.MEMO_CHECK(val) });

                if (memo[val] !== undefined) {
                    node.isMemo = true;
                    node.result = memo[val];
                    history.push({ stepId: history.length, nodeId: id, val, memo: { ...memo }, line: 1, algoLine: 2, status: 'MEMO_FOUND', desc: `Found in Memo: ${memo[val]}`, speech: FIBONACCI_SCRIPT.MEMO_FOUND(val, memo[val]) });
                    return { node, result: memo[val] };
                }

                if (val <= 1) {
                    memo[val] = val;
                    history.push({ stepId: history.length, nodeId: id, val, memo: { ...memo }, line: 2, algoLine: 3, status: 'BASE', result: val, desc: `Base case: ${val}`, speech: FIBONACCI_SCRIPT.BASE(val) });
                    return { node, result: val };
                }

                history.push({ stepId: history.length, nodeId: id, val, memo: { ...memo }, status: 'THINKING', algoLine: 4, desc: `Solving subproblems...` });
                const { node: left, result: r1 } = solve(val - 1, depth + 1, id);
                node.children.push(left);
                
                const { node: right, result: r2 } = solve(val - 2, depth + 1, id);
                node.children.push(right);

                const final = r1 + r2;
                memo[val] = final;
                history.push({ stepId: history.length, nodeId: id, val, memo: { ...memo }, line: 3, algoLine: 5, status: 'STORE', result: final, desc: `Storing fib(${val}) = ${final}`, speech: FIBONACCI_SCRIPT.MEMO_STORE(val, final) });
                
                return { node, result: final };
            }

            const { node: treeRoot, result: finalRes } = solve(n, 0);
            history.push({ stepId: history.length, status: 'SUCCESS', val: n, result: finalRes, memo: { ...memo }, algoLine: 6, desc: "DP Complete!", speech: FIBONACCI_SCRIPT.DP_SUCCESS(finalRes) });
        ...
        const NodeComponent = ({ nodeData, isActive, isBreadcrumb, latestState }) => {
            ...
            const { val } = nodeData;
            let statusColor = "bg-slate-700", borderColor = "border-slate-700", opacity = 0.3, statusText = "IDLE";
            
            if (latestState) {
                opacity = 1;
                if (latestState.status === 'START') { statusColor = "bg-blue-600"; statusText = "CHECKING"; borderColor="border-blue-500"; }
                if (latestState.status === 'MEMO_FOUND') { statusColor = "bg-emerald-600"; statusText = "MEMO HIT"; borderColor="border-emerald-500"; }
                if (latestState.status === 'BASE') { statusColor = "bg-emerald-600"; statusText = "BASE"; borderColor="border-emerald-500"; }
                if (latestState.status === 'STORE') { statusColor = "bg-rose-600"; statusText = "STORED"; borderColor="border-rose-500"; }
            }
            if (isBreadcrumb && !isActive) { borderColor = 'border-indigo-500/70'; opacity = 1; }

            return (
                <div className={`absolute rounded-xl shadow-2xl overflow-hidden transition-all duration-300 border-2 bg-slate-800 flex flex-col justify-between ${borderColor}`} style={{ left: nodeData.x, top: nodeData.y, width: NODE_WIDTH, height: NODE_HEIGHT, transform: isActive ? 'scale(1.05)' : 'scale(1)', zIndex: isActive ? 50 : 10, opacity, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                    <div className="bg-slate-900/80 px-3 py-1.5 flex justify-between items-center border-b border-slate-700">
                        <span className="font-mono font-bold text-xs text-blue-300">fib({val})</span>
                    ...
                    <div className="bg-slate-800 flex-1 flex flex-col items-center justify-center p-2">
                        <div className="text-xl font-black text-white">{latestState?.result !== undefined ? latestState.result : '?'}</div>
                        {nodeData.isMemo && <div className="text-[9px] text-emerald-400 font-bold uppercase tracking-tighter mt-1">From Memo</div>}
                    </div>
                </div>
            );
        };
        ...
        const App = () => {
            ...
            const { treeRoot, history, flatNodes } = useMemo(() => {
                ...
                const flat = []; assignCoordinates(treeRoot, 0, 0, flat);
            ...
            const currentState = history[currentStep] || { memo: {} };
            const activePathIds = new Set();
            const latestNodeStates = {};
            if (currentState) {
                for(let i = 0; i <= currentStep; i++) latestNodeStates[history[i].nodeId] = history[i];
                let curr = flatNodes.find(n => n.id === history[currentStep]?.nodeId);
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <div className="absolute top-20 right-6 z-40 bg-slate-800/80 backdrop-blur-md p-4 rounded-2xl border border-slate-700 shadow-xl w-48 transition-all duration-300">
                        <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">Memo Table</h3>
                        <div className="flex flex-col gap-1">
                            {Array.from({length: n + 1}).map((_, i) => (
                                <div key={i} className={`flex justify-between items-center px-2 py-1 rounded transition-colors ${currentState.status === 'STORE' && currentState.val === i ? 'memo-row-active' : ''}`}>
                                    <span className="font-bold text-slate-300">[{i}]</span>
                                    <span className={`font-mono ${currentState.memo[i] === undefined ? 'text-slate-600' : 'text-emerald-400 font-bold'}`}>
                                        {currentState.memo[i] ?? 'null'}
                                    </span>
                                </div>
                            ))}
                        </div>
                    ...
                    <AlgorithmPanel algoKey="FIBONACCI_DP" currentLine={currentState?.algoLine} />
                        ...
                        <div className={`node-container absolute origin-top-left ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${camTarget.x + manualOffset.x}px, ${camTarget.y + manualOffset.y}px) scale(${zoom})` }}>
                            <svg className="absolute top-0 left-0 overflow-visible" style={{ width: 1, height: 1 }}>
                                {flatNodes.map(node => node.children.map(child => {
                                    const isPruned = child.isMemo && !latestNodeStates[child.id];
                                    return (
                                        <path key={`${node.id}-${child.id}`} d={`M ${node.x+NODE_WIDTH/2} ${node.y+NODE_HEIGHT} C ${node.x+NODE_WIDTH/2} ${(node.y+child.y)/2}, ${child.x+NODE_WIDTH/2} ${(node.y+child.y)/2}, ${child.x+NODE_WIDTH/2} ${child.y}`} fill="none" stroke={activePathIds.has(child.id) ? "#818cf8" : (latestNodeStates[child.id] ? "#475569" : "#1e293b")} strokeWidth={activePathIds.has(child.id) ? 3 : 2} className={`edge-path ${activePathIds.has(child.id) ? 'edge-waiting' : ''} ${isPruned ? 'pruned-branch' : ''}`} style={{ opacity: activePathIds.has(child.id) ? 1 : (latestNodeStates[child.id] ? 0.8 : 0.2) }} />
                                    );
                                }))}
                            </svg>
