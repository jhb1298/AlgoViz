...
/* extracted from bucketsort.html */
        ...
        function generateSimulation(arr) {
            ...
            history.push({ 
                ...
                status: 'START', 
                algoLine: 1,
                desc: "Starting Bucket Sort", 
                speech: SORTING_SCRIPT.BUCKET.START() 
            });

            // 1. Distribute into buckets
            for (let i = 0; i < data.length; i++) {
                let bucketIdx = Math.floor((data[i] - min) / range);
                ...
                history.push({ 
                    ...
                    status: 'BUCKETING', 
                    algoLine: 2,
                    desc: `Placing ${data[i]} into bucket ${bucketIdx}`, 
                    speech: SORTING_SCRIPT.BUCKET.BUCKETING(data[i], bucketIdx)
                });
            }
            ...
            for (let i = 0; i < numBuckets; i++) {
                if (buckets[i].length > 0) {
                    ...
                    history.push({ 
                        ...
                        buckets: buckets.map(b => [...b]), 
                        activeBucket: i,
                        status: 'SORTING_BUCKET', 
                        algoLine: 3,
                        desc: `Sorting contents of bucket ${i}`, 
                        speech: SORTING_SCRIPT.BUCKET.SORTING_BUCKET(i)
                    });
                }
            }

            // 3. Collect from buckets
            let k = 0;
            const output = [...data];
            for (let i = 0; i < numBuckets; i++) {
                for (let j = 0; j < buckets[i].length; j++) {
                    ...
                    history.push({ 
                        ...
                        status: 'COLLECTING', 
                        algoLine: 4,
                        desc: `Collecting from bucket ${i}`, 
                        speech: SORTING_SCRIPT.BUCKET.COLLECTING(i)
                    });
                }
            }
            ...
            history.push({ 
                ...
                algoLine: 5,
                desc: "Sort Complete!", 
                speech: SORTING_SCRIPT.BUCKET.SUCCESS() 
            });
            return history;
        }
        ...
        const App = () => {
            ...
            const [input, setInput] = useState("29,25,3,49,9,37,21,43");
            ...
            const state = history[currentStep] || { arr: [], buckets: Array.from({length: 5}, () => []) };
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <AlgorithmPanel algoKey="BUCKETSORT" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`absolute left-0 top-0 w-full h-full flex flex-col items-center justify-center gap-32 ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            
                            {/* Main Array Display */}
                            <div className="flex flex-col items-center gap-4">
                                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Main Array</h3>
                                <div className="flex gap-4">
                                    {state.arr.map((val, i) => (
                                        <div key={i} className={`w-16 h-12 bg-slate-800 border-2 rounded-xl flex items-center justify-center font-bold text-lg num-transition ${state.activeIndex === i ? 'cell-active border-blue-500 bg-blue-500/10' : 'border-slate-700'}`}>
                                            {val}
                                        </div>
                                    ))}
                                </div>
                            ...
                            {/* Buckets Display */}
                            <div className="flex flex-col items-center gap-4">
                                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Buckets (Range-based)</h3>
                                <div className="flex gap-8">
                                    {state.buckets.map((bucket, i) => (
                                        <div key={i} className={`w-24 min-h-[160px] bucket rounded-2xl flex flex-col items-center gap-2 p-3 ${state.activeBucket === i ? 'bucket-active' : ''}`}>
                                            <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-700 w-full text-center pb-1.5 mb-1.5">B{i}</div>
                                            {bucket.map((val, idx) => (
                                                <div key={idx} className="w-16 h-10 bg-slate-700 border border-slate-600 rounded-lg flex items-center justify-center font-bold text-sm text-blue-200 shadow-sm num-transition">
                                                    {val}
                                                </div>
                                            ))}
                                        </div>
