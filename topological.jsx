...
import './topo_script.js';
import './settings_manager.js';
import './algorithm_data.js';
import './chatbot.js';

/* extracted from topological.html */
...
E F`;
        ...
        function generateSimulation(graph) {
            ...
            history.push({ 
                stepId: 0, inDegree: { ...inDegree }, result: [], status: 'START', algoLine: 1,
                desc: "Calculating initial in-degrees", speech: TOPO_SCRIPT.START() 
            });

            history.push({ 
                stepId: history.length, inDegree: { ...inDegree }, result: [], status: 'IN_DEGREE', algoLine: 2,
                desc: "Every node counts its incoming prerequisites.", speech: TOPO_SCRIPT.IN_DEGREE() 
            });
            ...
            while (queue.length > 0) {
                ...
                result.push(u);
                history.push({ 
                    stepId: history.length, inDegree: { ...inDegree }, activeNode: u, 
                    result: [...result], status: 'VISITING', algoLine: 4,
                    desc: `Task ${u} has no dependencies left. Heading to sorted list!`, speech: TOPO_SCRIPT.ZERO_IN_DEGREE(u) 
                });
                ...
                for (const edge of outgoing) {
                    ...
                    history.push({ 
                        stepId: history.length, inDegree: { ...inDegree }, activeNode: u, 
                        activeEdge: edge, result: [...result], status: 'REMOVING', algoLine: 6,
                        desc: `Reducing dependency for ${edge.to}`, speech: TOPO_SCRIPT.REMOVING(u, edge.to) 
                    });
            ...
            history.push({ stepId: history.length, inDegree: { ...inDegree }, result: [...result], status: 'SUCCESS', algoLine: 8, desc: `Sorted ${total} nodes successfully!`, speech: TOPO_SCRIPT.SUCCESS() });
            return history;
        }

        const NodeComponent = ({ node, inDegree, isActive, isCompleted, isReady }) => {
            ...
            return (
                <div 
                    className={`absolute w-20 h-20 rounded-2xl bg-slate-800 border-4 flex flex-col items-center justify-center transition-all duration-500 z-20 ${isActive ? 'node-active' : (isCompleted ? 'node-completed border-slate-700' : (isReady ? 'node-ready' : 'border-slate-700'))}`} 
                    style={{ 
                        left: node.x - 40, top: node.y - 40, 
                        fontSize: settings.fontSize + "px", 
                        borderColor: isActive ? settings.nodeColor : (isReady ? '#10b981' : undefined),
                        boxShadow: isActive ? `0 0 25px ${settings.nodeColor}` : undefined
                    }}
                >
                    <span className="text-2xl font-black text-white">{node.id}</span>
                    <div className="mt-1 px-2 py-0.5 bg-slate-900/50 rounded-full">
                        <span className="text-[10px] font-black text-indigo-400">IN: {inDegree}</span>
                    </div>
                </div>
            );
        };

        const EdgeComponent = ({ edge, nodes, isActive, isCompleted }) => {
            ...
            return (
                <g className={isCompleted ? 'edge-faded' : ''}>
                    <line x1={u.x} y1={u.y} x2={x2} y2={y2} stroke={isActive ? "#6366f1" : "#1e293b"} strokeWidth={isActive ? 4 : 2} className={isActive ? 'edge-active' : ''} style={{ transition: 'stroke 0.3s' }} />
                    <polygon points={`${x2},${y2} ${x2-headlen*Math.cos(angle-Math.PI/6)},${y2-headlen*Math.sin(angle-Math.PI/6)} ${x2-headlen*Math.cos(angle+Math.PI/6)},${y2-headlen*Math.sin(angle+Math.PI/6)}`} fill={isActive ? "#6366f1" : "#1e293b"} style={{ transition: 'fill 0.3s' }} />
                </g>
            );
        };
        ...
        const App = () => {
            ...
            const state = history[currentStep] || { inDegree: {}, result: [] };
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="font-bold text-lg leading-none tracking-tight">Topological Sort</h1>
                                    <span className="text-xs text-slate-400 font-mono">Directed Acyclic Graph Ordering</span>
                    ...
                    {isEditing && (
                        <div className="absolute inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-6 text-white pointer-events-auto">
                            <div className="bg-slate-900 border border-slate-700 rounded-3xl p-8 max-w-lg w-full shadow-2xl">
                                <h2 className="text-2xl font-bold mb-2">Edit Task Dependencies</h2>
                                <p className="text-slate-400 text-sm mb-4">Format: <code className="bg-slate-800 px-1.5 py-0.5 rounded text-blue-300">Prerequisite Target</code></p>
                                ...
                                <div className="flex justify-end gap-4">
                                    ...
                                    <button onClick={() => { setIsEditing(false); setCurrentStep(0); setIsPlaying(false); }} className="px-8 py-2 rounded-xl font-bold bg-blue-600 hover:bg-blue-500 shadow-lg transition-all text-white">Apply Graph</button>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <AlgorithmPanel algoKey="TOPOLOGICAL" currentLine={state.algoLine} />
                    ...
                    <div className="flex flex-1 overflow-hidden relative">
                            ...
                            <div className={`node-container absolute origin-top-left ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                                <svg className="absolute top-0 left-0 overflow-visible" style={{ width: 1000, height: 1000 }}>
                                    {graph.edges.map((edge, i) => (
                                        <EdgeComponent key={i} edge={edge} nodes={graph.nodes} isActive={state.activeEdge === edge} isCompleted={state.result.includes(edge.from)} />
                                    ))}
                                </svg>
                                {graph.nodes.map(node => (
                                    <NodeComponent key={node.id} node={node} inDegree={state.inDegree[node.id] || 0} isActive={state.activeNode === node.id} isCompleted={state.result.includes(node.id)} isReady={state.inDegree[node.id] === 0 && !state.result.includes(node.id)} />
                                ))}
                            </div>
                        </div>
                        ...
                        {/* List Sidebar */}
                        {showSidebar && (
                            <div className="w-[300px] bg-slate-900/50 backdrop-blur-xl border-l border-slate-700/50 flex flex-col p-8 overflow-y-auto gap-10 z-40 relative shadow-2xl">
                                <div>
                                    <div className="flex items-center justify-between mb-6">
                                        <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-400">ORDERED SEQUENCE</h3>
