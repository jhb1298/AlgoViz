...
/* extracted from heap.html */
        ...
        function generateSimulation(arr) {
            ...
            history.push({ stepId: 0, arr: [...data], status: 'START', algoLine: 1, desc: "Starting Heap Sort", speech: SORTING_SCRIPT.HEAP.START() });
            ...
            async function heapify(size, i) {
                ...
                if (l < size) {
                    history.push({ stepId: history.length, arr: [...data], activeNodes: [i, l], size, status: 'COMPARING', algoLine: 4, desc: `Comparing parent ${data[i]} with left child ${data[l]}`, speech: SORTING_SCRIPT.HEAP.HEAPIFY(data[i], data[l]) });
                    if (data[l] > data[largest]) largest = l;
                }

                if (r < size) {
                    history.push({ stepId: history.length, arr: [...data], activeNodes: [i, r], size, status: 'COMPARING', algoLine: 4, desc: `Comparing parent ${data[i]} with right child ${data[r]}`, speech: SORTING_SCRIPT.HEAP.HEAPIFY(data[i], data[r]) });
                    if (data[r] > data[largest]) largest = r;
                }

                if (largest !== i) {
                    history.push({ stepId: history.length, arr: [...data], activeNodes: [i, largest], size, status: 'SWAPPING', algoLine: 3, desc: `Swapping ${data[i]} and ${data[largest]}` });
            ...
            // Extract elements
            for (let i = n - 1; i > 0; i--) {
                history.push({ stepId: history.length, arr: [...data], activeNodes: [0, i], size: i + 1, status: 'EXTRACTING', algoLine: 2, desc: `Extracting ${data[0]}`, speech: SORTING_SCRIPT.HEAP.EXTRACTING(data[0]) });
                [data[0], data[i]] = [data[i], data[0]];
                history.push({ stepId: history.length, arr: [...data], size: i, status: 'REBUILDING', algoLine: 4, desc: `Rebuilding heap`, speech: SORTING_SCRIPT.HEAP.REBUILDING() });
                heapify(i, 0);
            }

            history.push({ stepId: history.length, arr: [...data], status: 'SUCCESS', algoLine: 5, desc: "Sort Complete!", speech: SORTING_SCRIPT.HEAP.SUCCESS() });
            return history;
        }
        ...
        const NodeComponent = ({  val, x, y, isActive, isSwap  }) => {
            ...
            return (
            <div className={`absolute w-12 h-12 rounded-full bg-slate-800 border-2 flex items-center justify-center font-bold transition-all duration-500 z-20 ${isActive ? 'node-active' : (isSwap ? 'node-swap' : 'border-slate-600')}`} style={{ left: x - 24, top: y - 24, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                {val}
            </div>
        );
        };
        ...
        const App = () => {
            ...
            const [input, setInput] = useState("15,40,10,30,50,20");
            ...
            const getTreeCoords = (i, n) => {
                ...
                const x = 500 + (posInLevel - (numInLevel - 1) / 2) * (800 / numInLevel);
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <AlgorithmPanel algoKey="HEAPSORT" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`absolute left-0 top-0 w-full h-full ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            <svg className="absolute top-0 left-0 w-full h-full overflow-visible">
                                {state.arr.map((_, i) => {
                                    if (i >= (state.size || state.arr.length)) return null;
                                    const l = 2 * i + 1;
                                    const r = 2 * i + 2;
                                    const p = getTreeCoords(i, state.arr.length);
                                    return (
                                        <React.Fragment key={i}>
                                            {l < (state.size || state.arr.length) && <line x1={p.x} y1={p.y} x2={getTreeCoords(l, state.arr.length).x} y2={getTreeCoords(l, state.arr.length).y} stroke="#1e293b" strokeWidth="2" />}
                                            {r < (state.size || state.arr.length) && <line x1={p.x} y1={p.y} x2={getTreeCoords(r, state.arr.length).x} y2={getTreeCoords(r, state.arr.length).y} stroke="#1e293b" strokeWidth="2" />}
                                        </React.Fragment>
                                    );
                                })}
                            </svg>
                            {state.arr.map((val, i) => {
                                const isHidden = i >= (state.size || state.arr.length);
                                const coords = isHidden ? { x: 200 + i * 60, y: 600 } : getTreeCoords(i, state.arr.length);
                                return (
