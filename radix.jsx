...
/* extracted from radix.html */
        ...
        function generateSimulation(arr) {
            ...
            history.push({ stepId: 0, arr: [...data], buckets: Array.from({length: 10}, () => []), status: 'START', algoLine: 1, desc: "Starting Radix Sort", speech: SORTING_SCRIPT.RADIX.START() });

            for (let exp = 1; Math.floor(maxVal / exp) > 0; exp *= 10) {
                const buckets = Array.from({length: 10}, () => []);
                const place = exp === 1 ? "one" : (exp === 10 ? "ten" : "hundred");
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    history.push({ 
                        ...
                        status: 'BUCKETING', 
                        algoLine: 3,
                        desc: `Placing ${data[i]} in bucket ${digit}`, 
                        speech: SORTING_SCRIPT.RADIX.BUCKETING(data[i], digit, place)
                    });
                }
                ...
                history.push({ stepId: history.length, arr: [...data], buckets: buckets.map(b => [...b]), status: 'COLLECTING', algoLine: 6, desc: `Collected from buckets for ${place}s place`, speech: SORTING_SCRIPT.RADIX.COLLECTING(place) });
            }

            history.push({ stepId: history.length, arr: [...data], buckets: Array.from({length: 10}, () => []), status: 'SUCCESS', algoLine: 6, desc: "Sort Complete!", speech: SORTING_SCRIPT.RADIX.SUCCESS() });
            return history;
        }
        ...
        const App = () => {
            ...
            const state = history[currentStep] || { arr: [], buckets: Array.from({length: 10}, () => []) };
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <AlgorithmPanel algoKey="RADIX" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`absolute left-0 top-0 w-full h-full ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            {/* Main Array Display */}
                            <div className="absolute left-1/2 top-40 transform -translate-x-1/2 flex gap-4">
                                {state.arr.map((val, i) => (
                                    <div key={i} className={`w-16 h-12 bg-slate-800 border-2 rounded-lg flex items-center justify-center font-bold text-lg num-transition ${state.activeNum === val ? 'border-blue-500 shadow-lg scale-110' : 'border-slate-700'}`}>
                                        {val}
                                    </div>
                                ))}
                            </div>
                            ...
                            {/* Buckets Display */}
                            <div className="absolute left-1/2 top-[400px] transform -translate-x-1/2 flex gap-4">
                                {state.buckets.map((bucket, i) => (
                                    <div key={i} className={`w-20 min-h-[150px] bucket rounded-xl flex flex-col items-center gap-2 p-2 ${state.activeBucket === i ? 'bucket-active' : ''}`}>
                                        <div className="text-xs font-bold text-slate-500 uppercase tracking-widest border-b border-slate-700 w-full text-center pb-1 mb-1">B{i}</div>
                                        {bucket.map((val, idx) => (
                                            <div key={idx} className="w-14 h-10 bg-slate-700 border border-slate-600 rounded flex items-center justify-center font-bold text-sm text-blue-200 shadow-sm num-transition">
                                                {val}
                                            </div>
                                        ))}
                                    </div>
