            ...
            return (
                <div className="absolute inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-6">
                    <div className="bg-slate-900 border border-slate-700 rounded-3xl p-8 max-w-sm w-full shadow-2xl">
                        <h2 className="text-2xl font-bold mb-6 flex items-center gap-2"><Settings size={24}/> Global Settings</h2>
                        <div className="space-y-6">
                            <div>
                                <label className="block text-sm font-bold text-slate-400 uppercase tracking-widest mb-3">Font Size ({settings.fontSize}px)</label>
                                <input type="range" min="10" max="32" value={settings.fontSize} onChange={(e) => handleSave('fontSize', parseInt(e.target.value))} className="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-blue-500" />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-slate-400 uppercase tracking-widest mb-3">Node Accent Color</label>
                                <div className="flex gap-3">
                                    {['#3b82f6', '#10b981', '#f43f5e', '#f59e0b', '#8b5cf6'].map(color => (
                                        <button key={color} onClick={() => handleSave('nodeColor', color)} className={`w-10 h-10 rounded-full border-2 transition-transform active:scale-90 ${settings.nodeColor === color ? 'border-white scale-110' : 'border-transparent'}`} style={{ backgroundColor: color }} />
                                    ))}
                                </div>
                            </div>
                        </div>
                        <button onClick={onClose} className="w-full mt-8 py-3 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold transition-all shadow-lg">Close</button>
                    </div>
                </div>
            );
        };
...
E F`;
        ...
        function generateSimulation(graph) {
            ...
            history.push({ stepId: 0, activeNode: source, visited: new Set(), queued: new Set(queued), queue: [...queue], status: 'START', desc: `Starting BFS at ${source}`, speech: SEARCH_SCRIPT.BFS.START(source), algoLine: 1 });
            ...
            while (queue.length > 0) {
                ...
                history.push({ 
                    ...
                    queued: new Set(queued), queue: [...queue], status: 'VISITING', desc: `Visiting ${u}`, 
                    speech: SEARCH_SCRIPT.BFS.VISITING(u), algoLine: 4
                });
                ...
                for (const v of adj[u]) {
                    if (!visited.has(v) && !queued.has(v)) {
                        ...
                        history.push({ 
                            ...
                            queued: new Set(queued), queue: [...queue], status: 'ENQUEUE', desc: `Enqueuing ${v}`, 
                            speech: SEARCH_SCRIPT.BFS.ENQUEUE(v), algoLine: 8
                        });
                    }
                }
            }

            history.push({ stepId: history.length, status: 'SUCCESS', visited: new Set(visited), queued: new Set(queued), queue: [], desc: "BFS Complete", speech: SEARCH_SCRIPT.BFS.SUCCESS(), algoLine: 3 });
            return history;
        }

        const NodeComponent = ({ node, isActive, isVisited, isQueued }) => {
            const [settings, setSettings] = useState(JSON.parse(localStorage.getItem('simulation_settings') || '{"fontSize": 16, "nodeColor": "#3b82f6"}'));
            useEffect(() => {
                const handleSettingsUpdate = () => { setSettings(JSON.parse(localStorage.getItem('simulation_settings') || '{"fontSize": 16, "nodeColor": "#3b82f6"}')); };
                window.addEventListener('storage', handleSettingsUpdate);
                const interval = setInterval(handleSettingsUpdate, 1000);
                return () => { window.removeEventListener('storage', handleSettingsUpdate); clearInterval(interval); };
            }, []);
            return (
                <div className={`absolute w-20 h-20 rounded-full bg-slate-800 border-4 flex flex-col items-center justify-center transition-all duration-500 z-20 ${isActive ? 'scale-110' : (isVisited ? 'node-visited' : (isQueued ? 'node-queued' : 'border-slate-700'))}`} style={{ left: node.x - 40, top: node.y - 40, borderColor: isActive ? settings.nodeColor : (isVisited ? '#10b981' : (isQueued ? '#3b82f6' : '#334155')), boxShadow: isActive ? `0 0 25px ${settings.nodeColor}80` : '', fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
        ...
        const QueueComponent = ({ queue }) => (
            <div className="absolute right-6 top-24 w-28 flex flex-col gap-2 z-40 bg-slate-900/80 backdrop-blur-md p-4 rounded-2xl border border-slate-700 shadow-xl max-h-[60vh] overflow-y-auto">
                <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 text-center w-full">Queue</div>
                {queue.map((id, i) => (
                    <div key={i} className={`flex items-center justify-center w-full h-10 bg-slate-800 border-2 rounded-lg font-bold animate-in fade-in slide-in-from-right-2 duration-300 ${i === 0 ? 'border-emerald-500 text-emerald-400' : 'border-blue-500/50 text-blue-400'}`}>
                        {id}
                    </div>
                ))}
                {queue.length === 0 && <div className="text-[10px] text-slate-600 text-center py-2 italic">Empty</div>}
            </div>
        );
        ...
        const App = () => {
            ...
            const state = history[currentStep] || { visited: new Set(), queued: new Set() };
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">BFS Visualizer</h1><span className="text-xs text-slate-400 font-mono">Breadth First Search Traversal</span></div></div>
                    ...
                    <AlgorithmPanel algoKey="BFS" currentLine={state.algoLine} />
                    ...
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`node-container absolute origin-top-left ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            ...
                            {graph.nodes.map(node => <NodeComponent key={node.id} node={node} isActive={state.activeNode === node.id} isVisited={state.visited.has(node.id)} isQueued={state.queued.has(node.id)} />)}
                        </div>
                    </div>

                    <QueueComponent queue={state.queue || []} />
