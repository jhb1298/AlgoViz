...
E F 5`;
        ...
        function generateSimulation(graph) {
            ...
            history.push({ stepId: 0, activeNode: source, distances: { ...distances }, parents: { ...parents }, visited: new Set(), status: 'START', algoLine: 1, desc: `Starting at Source ${source}`, speech: DIJKSTRA_SCRIPT.START(source) });
            ...
            while (pq.length > 0) {
                ...
                visited.add(u);
                history.push({ stepId: history.length, activeNode: u, distances: { ...distances }, parents: { ...parents }, visited: new Set(visited), status: 'VISITING', algoLine: 4, desc: `Visiting ${u}`, speech: DIJKSTRA_SCRIPT.VISITING(u) });
                ...
                for (const edge of neighbors) {
                    ...
                    if (newDist < distances[v]) {
                        ...
                        parents[v] = u;
                        pq.push(v);
                        history.push({ stepId: history.length, activeNode: u, targetNode: v, activeEdge: edge, distances: { ...distances }, parents: { ...parents }, visited: new Set(visited), status: 'RELAXING', algoLine: 9, desc: `Shortcut to ${v} via ${u}`, speech: DIJKSTRA_SCRIPT.RELAXING(u, v, newDist) });
                    } else {
                        history.push({ stepId: history.length, activeNode: u, targetNode: v, activeEdge: edge, distances: { ...distances }, parents: { ...parents }, visited: new Set(visited), status: 'CHECKING', algoLine: 8, desc: `No shortcut to ${v}` });
                    }
                }
            }

            history.push({ stepId: history.length, status: 'SUCCESS', algoLine: 3, distances: { ...distances }, parents: { ...parents }, visited: new Set(visited), desc: "Search Complete", speech: DIJKSTRA_SCRIPT.SUCCESS() });
            return history;
        }

        const NodeComponent = ({  node, dist, isActive, isVisited  }) => {
            ...
            return (
            <div className={`absolute w-20 h-20 rounded-full bg-slate-800 border-4 flex flex-col items-center justify-center transition-all duration-500 z-20 ${isActive ? 'node-active' : (isVisited ? 'node-visited' : 'border-slate-700')}`} style={{ left: node.x - 40, top: node.y - 40, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                <span className="text-xl font-black text-white">{node.id}</span>
                <span className={`text-xs font-mono font-bold ${dist === Infinity ? 'text-slate-500' : 'text-blue-400'}`}>{dist === Infinity ? '8' : dist}</span>
            </div>
        );
        };
        ...
        const App = () => {
            ...
            const state = history[currentStep] || { distances: {}, visited: new Set(), parents: {} };
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <AlgorithmPanel algoKey="DIJKSTRA" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper') && !e.target.closest('.top-nav-area')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`node-container absolute origin-top-left ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            <svg className="absolute top-0 left-0 overflow-visible" style={{ width: 1000, height: 1000 }}>
                                {graph.edges.map((edge, i) => <EdgeComponent key={i} edge={edge} nodes={graph.nodes} isActive={state.activeEdge === edge} isShortest={state.parents[edge.to] === edge.from || state.parents[edge.from] === edge.to} />)}
                            </svg>
                            {graph.nodes.map(node => <NodeComponent key={node.id} node={node} dist={state.distances[node.id]} isActive={state.activeNode === node.id} isVisited={state.visited.has(node.id)} />)}
                        </div>
                    </div>
