...
/* extracted from countingsort.html */
        ...
        function generateSimulation(arr) {
            ...
            history.push({ 
                ...
                status: 'START', 
                algoLine: 1,
                desc: "Starting Counting Sort", 
                speech: SORTING_SCRIPT.COUNTING.START() 
            });
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                history.push({ 
                    ...
                    status: 'COUNTING', 
                    algoLine: 3,
                    desc: `Counting ${data[i]}`, 
                    speech: SORTING_SCRIPT.COUNTING.COUNTING(data[i])
                });
            }
            ...
            for (let i = 1; i <= max; i++) {
                ...
                history.push({ 
                    ...
                    status: 'CUMULATIVE', 
                    algoLine: 4,
                    desc: `Calculating cumulative count for ${i}`, 
                    speech: SORTING_SCRIPT.COUNTING.CUMULATIVE(i, count[i])
                });
            }
            ...
            for (let i = data.length - 1; i >= 0; i--) {
                ...
                history.push({ 
                    ...
                    status: 'PLACING', 
                    algoLine: 5,
                    desc: `Placing ${val} at index ${pos}`, 
                    speech: SORTING_SCRIPT.COUNTING.PLACING(val, pos)
                });
            }
            ...
            history.push({ 
                ...
                algoLine: 6,
                desc: "Sort Complete!", 
                speech: SORTING_SCRIPT.COUNTING.SUCCESS() 
            });
            return history;
        }
        ...
        const App = () => {
            ...
            const state = history[currentStep] || { input: [], count: [], output: [] };
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <AlgorithmPanel algoKey="COUNTINGSORT" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`absolute left-0 top-0 w-full h-full flex flex-col items-center justify-center gap-20 ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            
                            {/* Input Array Section */}
                            <div className="flex flex-col items-center gap-4">
                                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Input Array</h3>
                                <div className="flex gap-2">
                                    {state.input.map((val, i) => (
                                        <div key={i} className={`w-12 h-12 bg-slate-800 border-2 rounded-lg flex items-center justify-center font-bold text-lg cell ${state.activeIndex === i ? 'cell-active border-blue-500' : 'border-slate-700'}`}>
                                            {val}
                                        </div>
                                    ))}
                                </div>
                            ...
                            {/* Count Array Section */}
                            <div className="flex flex-col items-center gap-4">
                                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Count Array (Frequency/Cumulative)</h3>
                                <div className="flex gap-2">
                                    {state.count.map((val, i) => (
                                        <div key={i} className="flex flex-col items-center gap-1">
                                            <div className={`w-12 h-12 bg-slate-800 border-2 rounded-lg flex items-center justify-center font-bold text-lg cell ${state.activeCount === i ? 'cell-active cell-counting' : 'border-slate-700 opacity-60'}`}>
                                                {val}
                                            </div>
                                            <span className="text-[10px] font-mono text-slate-500">v:{i}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col items-center gap-4">
                                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Sorted Output</h3>
                                <div className="flex gap-2">
                                    {state.output.map((val, i) => (
                                        <div key={i} className={`w-12 h-12 bg-slate-800 border-2 rounded-lg flex items-center justify-center font-bold text-lg cell ${state.activeOutput === i ? 'cell-active cell-placing' : (val !== null ? 'border-emerald-500/50 text-emerald-400' : 'border-slate-700/30 text-transparent')}`}>
                                            {val ?? 0}
                                        </div>
                                    ))}
                                </div>
