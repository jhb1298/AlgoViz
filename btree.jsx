...
/* extracted from btree.html */
        ...
        function generateSimulation(initialKeys, opVals, maxKeys) {
            ...
            function cloneTree(node) {
                if (!node) return null;
                const newNode = new BNode(node.isLeaf);
            ...
            function split(node, path, val, valIdx, total) {
                record('SPLIT', `(${valIdx}/${total}) Node full! Splitting node [${node.keys.join(', ')}]`, TREE_SCRIPT.BTREE.SPLIT(), node.id, 4);
                ...
                node.keys.pop(); // Remove the promoted middle key from the current node
                ...
                if (node === root) {
                    ...
                    root = newRoot;
                    record('NEW_ROOT', `(${valIdx}/${total}) Promoted ${midKey} to new root`, TREE_SCRIPT.BTREE.PROMOTE(midKey), root.id, 5);
                } else {
                    const parent = path.pop();
                    const idx = parent.children.indexOf(node);
                    parent.keys.splice(idx, 0, midKey);
                    parent.children.splice(idx + 1, 0, newNode);
                    record('PROMOTED', `(${valIdx}/${total}) Promoted ${midKey} to parent`, TREE_SCRIPT.BTREE.PROMOTE(midKey), parent.id, 5);
            ...
            function insert(val, valIdx, total) {
                let curr = root;
                const path = [];
                record('TRAVERSING', `(${valIdx}/${total}) Finding correct leaf for ${val}`, TREE_SCRIPT.BTREE.INSERT(val), curr.id, 0);
                ...
                while (!curr.isLeaf) {
                    ...
                    record('TRAVERSING', `(${valIdx}/${total}) Traversing to child node [${curr.keys.join(', ')}]`, TREE_SCRIPT.BTREE.NODE_REACHED(curr.keys), curr.id, 0);
                }

                curr.keys.push(val);
                curr.keys.sort((a, b) => a - b);
                record('INSERTED', `(${valIdx}/${total}) Inserted ${val} into leaf`, null, curr.id, 2);
            ...
            initialKeys.forEach(k => {
                ...
                if (curr.keys.length > maxKeys) {
                    const midIdx = Math.floor(maxKeys / 2);
                    const midKey = curr.keys[midIdx];
                    const newNode = new BNode(curr.isLeaf);
                    newNode.keys = curr.keys.splice(midIdx + 1);
                    curr.keys.pop();
                    if (curr === root) {
            ...
            record('SUCCESS', `B-Tree insertions complete`, TREE_SCRIPT.BTREE.SUCCESS(), null, 3);
        ...
        const NodeComponent = ({ node, x, y, activeNodeId }) => {
            ...
            return (
                <div 
                    className={`absolute flex flex-col items-center glass rounded-2xl overflow-hidden transition-all duration-500 border-2 ${isActive ? 'node-active' : 'border-white/10'} bg-slate-800/40`}
                    style={{ left: x - totalWidth / 2 - 10, top: y, width: totalWidth + 20, height: NODE_HEIGHT }}
                >
                    <div className="w-full bg-white/5 px-2 py-0.5 text-[8px] font-black text-slate-500 border-b border-white/5 flex justify-between uppercase tracking-widest leading-none">
                    ...
                    <div className="flex-1 flex items-center justify-between px-2 w-full">
                        {Array.from({ length: keysCount + 1 }).map((_, i) => (
                            <React.Fragment key={`slot-wrap-${i}`}>
                                <div className={`w-5 h-5 rounded-full border border-dashed flex items-center justify-center transition-colors ${!node.isLeaf ? 'border-indigo-500/50 bg-indigo-500/10' : 'border-white/10 bg-white/5'}`}>
                                    <div className={`w-1.5 h-1.5 rounded-full ${!node.isLeaf ? 'bg-indigo-400' : 'bg-slate-600'}`}></div>
                                </div>
                                {i < keysCount && (
                                    <div className="flex-1 flex items-center justify-center font-black text-lg text-indigo-400 key-enter min-w-[40px]">
                                        {node.keys[i]}
                                    </div>
                                )}
                            </React.Fragment>
        ...
        const App = () => {
            ...
            const [initialData, setInitialData] = useState([15, 30, 45]);
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-[#0f172a] text-white select-none" 
                        ...
                        <div className="relative h-20 glass border-b border-white/5 flex items-center justify-between px-8 shadow-2xl transition-all duration-500 -translate-y-[85%] peer-hover:translate-y-0 group-hover/nav:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-6">
                                ...
                                <div className="flex flex-col">
                                    <h1 className="font-extrabold text-xl leading-tight tracking-tight bg-gradient-to-r from-indigo-400 to-blue-400 bg-clip-text text-transparent">B-Tree Visualizer</h1>
                                    <div className="flex gap-2 items-center">
                                        <span className="text-[10px] text-slate-500 font-mono uppercase tracking-[0.2em]">Balanced Search Structure</span>
                                        <div className="w-1 h-1 bg-slate-700 rounded-full"></div>
                                        <button onClick={() => setIsSettingsOpen(true)} className="text-[10px] text-indigo-400 hover:underline">SETTINGS</button>
                                    </div>
                                </div>
                            </div>
                             ...
                             <div className="flex items-center gap-6">
                                <div className="flex flex-col items-end">
                                    ...
                                    <input type="number" min="2" max="6" value={maxKeys} onChange={e => { setMaxKeys(Number(e.target.value)); setCurrentStep(0); setIsPlaying(false); }} 
                                        className="w-12 bg-transparent font-black text-lg outline-none text-indigo-400 text-right border-b border-white/10" />
                                </div>
                                <div className="flex flex-col items-end">
                                    ...
                                    <input type="text" value={inputString} onChange={e => { setInputString(e.target.value); setCurrentStep(0); setIsPlaying(false); }} 
                                        className="w-32 bg-transparent font-black text-lg outline-none text-indigo-400 text-right border-b border-white/10" placeholder="25, 35..." />
                                </div>
                                <button onClick={() => { setCurrentStep(0); setIsPlaying(true); }} 
                                    className="bg-indigo-600 hover:bg-indigo-500 w-12 h-12 rounded-2xl transition-all shadow-xl shadow-indigo-900/40 flex items-center justify-center group active:scale-95">
                    ...
                    <AlgorithmPanel algoKey="BTREE" currentLine={state.line} />
                                        ...
                                        return (
                                            <g key={`${item.node.id}-${child.id}`}>
                                                ...
                                                <line 
                                                    x1={startX} y1={item.y + 40} 
                                                    x2={childItem.x} y2={childItem.y} 
                                                    stroke={state.activeNodeId === child.id ? '#6366f1' : 'rgba(255,255,255,0.2)'} 
                        ...
                        <div className={`glass rounded-[2rem] shadow-2xl flex flex-col relative overflow-hidden transition-all duration-300 ${isMinimized ? 'w-fit mx-auto px-6 py-4' : 'px-8 pb-6 pt-2'}`}>
                            ...
                            {!isMinimized && (
                                <div className="flex flex-col gap-6">
                                    <div className="relative w-full h-2 bg-slate-950/50 rounded-full overflow-hidden">
                                        <div className="absolute top-0 left-0 h-full bg-gradient-to-r from-indigo-600 to-blue-500 transition-all duration-300" 
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex flex-col w-[35%]">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="text-[10px] font-black text-indigo-500 tracking-[0.2em] uppercase">Step {currentStep + 1}</span>
                                        ...
                                        <div className="flex items-center justify-center gap-4 w-[30%]">
                                            ...
                                            <button onClick={() => setIsPlaying(!isPlaying)} 
                                                className={`w-16 h-16 flex items-center justify-center rounded-3xl shadow-2xl transform transition-all active:scale-90 border border-white/10 ${isPlaying ? 'bg-amber-500' : 'bg-indigo-600'}`}>
                                        ...
                                        <div className="flex items-center justify-end gap-6 w-[35%]">
                                            <div className="flex flex-col gap-2 items-end w-32">
                                                <div className="flex justify-between w-full">
                                                    ...
                                                    <span className="text-[10px] font-bold text-indigo-400 font-mono">{speedMultiplier.toFixed(1)}x</span>
                                                </div>
                                            ...
                                            <button onClick={() => setIsMuted(!isMuted)} 
                                                className={`p-3 rounded-2xl transition-all shadow-lg ${isMuted ? 'text-rose-400 bg-rose-400/5' : 'text-indigo-400 bg-indigo-400/5'}`}>
                            ...
                            {isMinimized && (
                                <div className="flex items-center justify-center gap-6">
                                    ...
                                    <button onClick={() => setIsPlaying(!isPlaying)} 
                                        className={`w-14 h-14 flex items-center justify-center rounded-2xl shadow-xl transform transition-all active:scale-90 border border-white/10 ${isPlaying ? 'bg-amber-500' : 'bg-indigo-600'}`}>
