...
G H`;
        ...
        function parseGraph(text) {
            ...
            const nodes = nodesArr.map((id, i) => {
                ...
                return { id, x: 500 + 300 * Math.cos(angle), y: 500 + 300 * Math.sin(angle) };
            });
            return { nodes, edges };
        }

        function generateSimulation(graph, targetId) {
            const history = [];
            ...
            const target = graph.nodes.find(n => n.id === targetId) || graph.nodes[graph.nodes.length - 1];
            if (!source || !target) return [];

            const getHeuristic = (nodeId) => {
                const n = graph.nodes.find(node => node.id === nodeId);
                return Math.sqrt(Math.pow(n.x - target.x, 2) + Math.pow(n.y - target.y, 2)) / 50;
            };

            gScores[source] = 0;
            const openList = [{ id: source, h: getHeuristic(source) }];
            queued.add(source);
            
            history.push({ 
                stepId: 0, activeNode: source, visited: new Set(), 
                queued: new Set(queued), openList: [...openList], 
                gScores: { ...gScores },
                status: 'START', algoLine: 1, desc: `Starting Greedy Search for ${target.id}`, 
                speech: SEARCH_SCRIPT.GREEDY.START(source, target.id) 
            });
            ...
            while (openList.length > 0) {
                ...
                history.push({ 
                    stepId: history.length, activeNode: u, visited: new Set(visited), 
                    queued: new Set(queued), openList: [...openList], 
                    gScores: { ...gScores },
                    status: 'VISITING', algoLine: 3, desc: `Visiting ${u} (h=${current.h.toFixed(1)})`, 
                    speech: SEARCH_SCRIPT.GREEDY.VISITING(u, current.h) 
                });
                ...
                if (u === target.id) {
                    history.push({ 
                        ...
                        queued: new Set(queued), openList: [...openList], 
                        gScores: { ...gScores },
                        status: 'SUCCESS', algoLine: 4, desc: `Target ${u} found!`, 
                        speech: SEARCH_SCRIPT.GREEDY.TARGET_FOUND(u) 
                    });
                    return history;
                }
                ...
                for (const v of adj[u]) {
                    if (!visited.has(v) && !queued.has(v)) {
                        ...
                        gScores[v] = gScores[u] + 1; // Simplistic node distance (edge count)
                        queued.add(v);
                        openList.push({ id: v, h });
                        history.push({ 
                            stepId: history.length, activeNode: u, targetNode: v,
                            activeEdge: {from: u, to: v}, visited: new Set(visited), 
                            queued: new Set(queued), openList: [...openList], 
                            gScores: { ...gScores },
                            status: 'ENQUEUE', algoLine: 8, desc: `Enqueuing ${v} (h=${h.toFixed(1)})`, 
                            speech: SEARCH_SCRIPT.GREEDY.ENQUEUE(v, h) 
                        });
                    }
                }
            }

            history.push({ stepId: history.length, status: 'FAIL', algoLine: 2, visited: new Set(visited), queued: new Set(queued), openList: [], gScores: { ...gScores }, desc: "Target not reachable", speech: "I couldn't find a path to the target node." });
            return history;
        }

        const NodeComponent = ({ node, isActive, isVisited, isQueued, isTarget, onSetTarget, g, targetNode }) => {
            ...
            const h = targetNode ? Math.sqrt(Math.pow(node.x - targetNode.x, 2) + Math.pow(node.y - targetNode.y, 2)) / 50 : null;
            const showStats = isVisited || isQueued || isActive;

            return (
                <div 
                    className={`absolute w-20 h-20 rounded-full bg-slate-800 border-4 flex flex-col items-center justify-center transition-all duration-500 z-20 cursor-pointer ${isActive ? 'node-active' : (isTarget ? 'node-target' : (isVisited ? 'node-visited' : (isQueued ? 'node-queued' : 'border-slate-700')))}`} 
                    style={{ left: node.x - 40, top: node.y - 40, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}
                    onClick={() => onSetTarget(node.id)}
                >
                    <span className="text-xl font-black text-white">{node.id}</span>
                    {isTarget && <Target size={14} className="text-rose-400 mt-0.5" />}
                    
                    {showStats && !isTarget && (
                        <div className="absolute -right-16 top-0 flex flex-col gap-0.5 bg-slate-900/90 backdrop-blur-sm border border-slate-700 p-1.5 rounded-lg shadow-xl z-30 pointer-events-none scale-90 origin-left">
                            <div className="flex items-center gap-1.5">
                                <span className="text-[8px] font-bold text-slate-500 uppercase">Dist (g)</span>
                                <span className="text-[10px] font-mono font-bold text-blue-400">{g === Infinity ? '8' : g}</span>
                            </div>
                            <div className="flex items-center gap-1.5 border-t border-slate-800 pt-0.5 mt-0.5">
                                <span className="text-[8px] font-bold text-slate-500 uppercase">Heur (h)</span>
                                <span className="text-[10px] font-mono font-bold text-emerald-400">{h?.toFixed(1)}</span>
                            </div>
                        </div>
                    )}
                </div>
        ...
        const EdgeComponent = ({ edge, nodes, isActive, isVisited }) => {
            const u = nodes.find(n => n.id === edge.from);
            const v = nodes.find(n => n.id === edge.to);
            if(!u || !v) return null;
            return (
                <line x1={u.x} y1={u.y} x2={v.x} y2={v.y} stroke={isVisited ? "#10b981" : (isActive ? "#6366f1" : "#1e293b")} strokeWidth={isVisited ? 4 : (isActive ? 4 : 2)} className={isActive ? 'edge-active' : (isVisited ? 'edge-visited' : '')} />
            );
        };

        const OpenListComponent = ({ list }) => {
            const sortedList = [...list].sort((a, b) => a.h - b.h);
            return (
                <div className="absolute right-6 top-24 w-32 flex flex-col gap-2 z-40 bg-slate-900/80 backdrop-blur-md p-4 rounded-2xl border border-slate-700 shadow-xl max-h-[60vh] overflow-y-auto">
                    <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 text-center w-full">Open List (h)</div>
                    {sortedList.map((item, i) => (
                        <div key={i} className={`flex flex-col items-center justify-center w-full py-1 bg-slate-800 border-2 rounded-lg font-bold animate-in fade-in slide-in-from-right-2 duration-300 ${i === 0 ? 'border-emerald-500 text-emerald-400' : 'border-blue-500/50 text-blue-400'}`}>
                            <span className="text-sm">{item.id}</span>
                            <span className="text-[9px] opacity-70">h={item.h.toFixed(1)}</span>
                        </div>
                    ))}
                    {list.length === 0 && <div className="text-[10px] text-slate-600 text-center py-2 italic">Empty</div>}
                </div>
            );
        };
        ...
        const App = () => {
            ...
            useEffect(() => {
                if (!targetNodeId && graph.nodes.length > 0) {
                    setTargetNodeId(graph.nodes[graph.nodes.length - 1].id);
                }
            }, [graph, targetNodeId]);

            const history = useMemo(() => generateSimulation(graph, targetNodeId), [graph, targetNodeId]);
            ...
            const state = history[currentStep] || { visited: new Set(), queued: new Set(), openList: [] };
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">Greedy Search Visualizer</h1><span className="text-xs text-slate-400 font-mono">Greedy Best-First Search</span><button onClick={() => setIsSettingsOpen(true)} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors text-slate-400 hover:text-white shadow-lg"><SettingsIcon size={20}/></button></div></div>
                            <div className="flex gap-3">
                                <div className="hidden md:flex items-center px-3 py-1 bg-rose-500/10 border border-rose-500/20 rounded-full text-[10px] text-rose-400 font-bold uppercase tracking-wider">Click any node to set Target</div>
                    ...
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper') && !e.target.closest('.node-container')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`node-container absolute origin-top-left ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            <svg className="absolute top-0 left-0 overflow-visible" style={{ width: 1000, height: 1000 }}>
                                {graph.edges.map((edge, i) => {
                                    const isEdgeVisited = state.visited.has(edge.from) && state.visited.has(edge.to);
                                    const isActive = (state.activeEdge?.from === edge.from && state.activeEdge?.to === edge.to) || (state.activeEdge?.from === edge.to && state.activeEdge?.to === edge.from);
                                    return <EdgeComponent key={i} edge={edge} nodes={graph.nodes} isActive={isActive} isVisited={isEdgeVisited} />;
                                })}
                            </svg>
                            {graph.nodes.map(node => (
                                <NodeComponent 
                                    key={node.id} 
                                    node={node} 
                                    isActive={state.activeNode === node.id} 
                                    isVisited={state.visited.has(node.id)} 
                                    isQueued={state.queued.has(node.id)} 
                                    isTarget={targetNodeId === node.id}
                                    onSetTarget={(id) => { setTargetNodeId(id); setCurrentStep(0); setIsPlaying(false); }}
                                    g={state.gScores ? state.gScores[node.id] : Infinity}
                                    targetNode={graph.nodes.find(n => n.id === targetNodeId)}
                                />
                            ))}
                        </div>
                    </div>

                    <OpenListComponent list={state.openList || []} />
