...
/* extracted from postfix.html */
        ...
        function generateSimulation(expression) {
            ...
            history.push({ stepId: 0, stack: [], currentTokenIdx: -1, algoLine: null, status: 'START', desc: "Let's begin!", speech: EXPRESSION_SCRIPT.POSTFIX.START(expression) });

            for (let i = 0; i < tokens.length; i++) {
                const token = tokens[i];
                if (!isNaN(token)) {
                    stack.push(Number(token));
                    history.push({ stepId: history.length, stack: [...stack], currentTokenIdx: i, algoLine: 3, status: 'PUSH', desc: `Push ${token}`, speech: EXPRESSION_SCRIPT.POSTFIX.NUMBER(token) });
                } else {
                    const b = stack.pop();
                    const a = stack.pop();
                    let res = 0;
                    ...
                    history.push({ stepId: history.length, stack: [...stack, a, b], currentTokenIdx: i, algoLine: 5, status: 'OPERATOR', desc: `Operator ${token}`, speech: EXPRESSION_SCRIPT.POSTFIX.OPERATOR(token) });
                    history.push({ stepId: history.length, stack: [...stack], currentTokenIdx: i, algoLine: 6, status: 'POP', desc: `Popped ${a} and ${b}`, speech: EXPRESSION_SCRIPT.POSTFIX.CALC(a, b, token, res) });
                    stack.push(res);
                    history.push({ stepId: history.length, stack: [...stack], currentTokenIdx: i, algoLine: 7, status: 'PUSH_RES', desc: `Push result ${res}` });
                }
            }

            history.push({ stepId: history.length, stack: [...stack], currentTokenIdx: tokens.length, algoLine: 8, status: 'SUCCESS', desc: "Final Result!", speech: EXPRESSION_SCRIPT.POSTFIX.SUCCESS(stack[0]) });
            return history;
        }
        ...
        const App = () => {
            ...
            const [input, setInput] = useState("5 3 + 8 2 * -");
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <AlgorithmPanel algoKey="POSTFIX" currentLine={state?.algoLine} />
                        ...
                        <div className={`absolute inset-0 flex flex-col items-center justify-center gap-12 ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            ...
                            <div className="flex flex-col-reverse items-center gap-2 min-h-[300px] w-48 p-4 border-x-4 border-b-4 border-slate-700 rounded-b-3xl bg-slate-800/20">
                                {state.stack.map((val, i) => (
                                    <div key={i} className="w-full h-12 bg-blue-600 border-2 border-blue-400 rounded-lg flex items-center justify-center font-black text-xl shadow-lg stack-item">
                                        {val}
                                    </div>
                                ))}
