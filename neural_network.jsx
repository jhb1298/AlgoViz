...
/* extracted from neural_network.html */
        ...
        function generateSimulation() {
            ...
            let w1 = Array.from({ length: 2 }, () => Array.from({ length: 3 }, () => Math.random() * 2 - 1));
            let w2 = Array.from({ length: 3 }, () => Array.from({ length: 1 }, () => Math.random() * 2 - 1));
            let b1 = Array(3).fill(0);
            let b2 = Array(1).fill(0);
            ...
            history.push({
                ...
                algoLine: 1,
                status: 'START',
                desc: 'Initializing Neural Network weights...',
                speech: ML_SCRIPT.NEURAL.START()
            });
            ...
            for (let epoch = 0; epoch < 50; epoch++) {
                ...
                for (const data of DATASET) {
                    // Forward Pass
                    let hidden = b1.map((b, i) => sigmoid(data.input.reduce((sum, val, j) => sum + val * w1[j][i], 0) + b));
                    let output = b2.map((b, i) => sigmoid(hidden.reduce((sum, val, j) => sum + val * w2[j][i], 0) + b));
                    
                    const loss = Math.pow(data.output[0] - output[0], 2);
                    totalLoss += loss;
                    ...
                    if (epoch % 10 === 0) {
                        history.push({
                            ...
                            algoLine: 4,
                            status: 'FORWARD',
                            desc: `Forward pass: Input [${data.input}] -> Output ${output[0].toFixed(3)}`,
                            speech: epoch === 0 ? ML_SCRIPT.NEURAL.FORWARD() : null
                        });
                    }

                    // Backward Pass
                    let outputErrors = output.map((o, i) => (data.output[i] - o) * dSigmoid(o));
                    let hiddenErrors = hidden.map((h, i) => outputErrors.reduce((sum, err, j) => sum + err * w2[i][j], 0) * dSigmoid(h));

                    // Update Weights
                    for (let i = 0; i < 3; i++) {
                        for (let j = 0; j < 1; j++) w2[i][j] += lr * outputErrors[j] * hidden[i];
                    }
                    for (let i = 0; i < 2; i++) {
                        for (let j = 0; j < 3; j++) w1[i][j] += lr * hiddenErrors[j] * data.input[i];
                    }
                    b2 = b2.map((b, i) => b + lr * outputErrors[i]);
                    b1 = b1.map((b, i) => b + lr * hiddenErrors[i]);
                    ...
                    if (epoch % 10 === 0) {
                        history.push({
                            ...
                            algoLine: 6,
                            status: 'BACKWARD',
                            desc: `Backpropagation: Error signal flowing back. Loss: ${loss.toFixed(4)}`,
                            speech: epoch === 0 ? ML_SCRIPT.NEURAL.BACKWARD() : null
                        });
                    }
                }
            }
            ...
            history.push({
                ...
                algoLine: 8,
                status: 'SUCCESS',
                desc: 'Training complete! XOR logic mastered.',
                speech: ML_SCRIPT.NEURAL.SUCCESS(0.01)
            });

            return history;
        }

        const NeuronComponent = ({ x, y, value, label, isActive }) => {
            ...
            <g transform={`translate(${x}, ${y})`}>
                <circle r="25" className={`neuron ${isActive ? 'neuron-active' : ''}`} fill="#334155" />
                <text textAnchor="middle" dy=".3em" fill="white" fontSize="12" fontWeight="bold">{value !== undefined ? value.toFixed(2) : label}</text>
                <text textAnchor="middle" dy="2.5em" fill="#cbd5e1" fontSize={settings.fontSize}>{label}</text>
            </g>
        
            );
        };

        const Connection = ({ x1, y1, x2, y2, weight, isActive }) => {
            const opacity = Math.min(1, Math.abs(weight) + 0.4);
            const color = weight > 0 ? "#10b981" : "#f43f5e";
            return (
                <g>
                    <line x1={x1} y1={y1} x2={x2} y2={y2} stroke={color} strokeWidth={Math.abs(weight) * 5} opacity={opacity} className="weight-line" />
                    {isActive && (
        ...
        const App = () => {
            ...
            const history = useMemo(() => generateSimulation(), []);
            ...
            const LAYOUT = {
                input: [{x: 200, y: 400}, {x: 200, y: 600}],
                hidden: [{x: 500, y: 300}, {x: 500, y: 500}, {x: 500, y: 700}],
                output: [{x: 800, y: 500}]
            };
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none">
                    <div className="absolute top-0 left-0 right-0 z-[60]">
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="font-bold text-lg leading-none tracking-tight">Neural Network</h1>
                                    <span className="text-xs text-slate-400 font-mono">Multilayer Perceptron (XOR)</span>
                            ...
                            <div className="flex items-center gap-8">
                                <div className="text-right">
                                    <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Loss</div>
                                    <div className="font-mono text-rose-400 font-bold">{state.loss ? state.loss.toFixed(4) : "---"}</div>
                                </div>
                                <div className="text-right">
                                    <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Target</div>
                                    <div className="font-mono text-emerald-400 font-bold">{state.target ? state.target[0] : "-"}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <AlgorithmPanel algoKey="NEURAL_NETWORK" currentLine={state?.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden flex items-center justify-center">
                        <svg width="1000" height="1000" style={{ transform: `scale(${zoom})` }} className="camera-layer overflow-visible">
                            ...
                            {LAYOUT.input.map((inp, i) => 
                                LAYOUT.hidden.map((hid, j) => (
                                    <Connection key={`w1-${i}-${j}`} x1={inp.x} y1={inp.y} x2={hid.x} y2={hid.y} weight={state.w1[i][j]} isActive={state.status === 'FORWARD'} />
                                ))
                            )}
                            {/* Weights Hidden -> Output */}
                            {LAYOUT.hidden.map((hid, i) => 
                                LAYOUT.output.map((out, j) => (
                                    <Connection key={`w2-${i}-${j}`} x1={hid.x} y1={hid.y} x2={out.x} y2={out.y} weight={state.w2[i][j]} isActive={state.status === 'FORWARD'} />
                                ))
                            )}
                            
                            {/* Neurons */}
                            {LAYOUT.input.map((p, i) => <NeuronComponent key={`i-${i}`} {...p} value={state.input ? state.input[i] : 0} label={`In ${i+1}`} isActive={state.status === 'FORWARD'} />)}
                            {LAYOUT.hidden.map((p, i) => <NeuronComponent key={`h-${i}`} {...p} value={state.hidden ? state.hidden[i] : 0} label={`Hid ${i+1}`} isActive={state.status === 'FORWARD'} />)}
                            {LAYOUT.output.map((p, i) => <NeuronComponent key={`o-${i}`} {...p} value={state.output ? state.output[i] : 0} label="Output" isActive={state.status === 'FORWARD'} />)}
                        </svg>
                    </div>
                            ...
                            {isMinimized ? (
                                ...
                                <>
                                    ...
                                    <div className="flex items-center justify-between w-full">
                                        <div className="flex flex-col w-[30%]"><span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Step {currentStep}</span><span className="text-sm font-mono text-emerald-300 truncate">{state.desc}</span></div>
