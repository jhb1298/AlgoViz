...
/* extracted from hanoi.html */
        ...
        const ZoomOut = (props) => <IconWrapper {...props}><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line><line x1="11" y1="8" x2="11" y2="14"></line><line x1="8" y1="11" x2="14" y2="11"></line></IconWrapper>;
        ...
        const clonePegs = (pegs) => JSON.parse(JSON.stringify(pegs));

        function generateSimulation(n) {
            let nodeIdCounter = 0;
            const treeRoot = { id: 0, n, src: 'A', aux: 'B', dest: 'C', children: [], depth: 0 };
            const initialPegs = { 'A': Array.from({length: n}, (_, i) => n - i), 'B': [], 'C': [] };
            const history = []; 
            function solve(node, pegs) {
                const { n, src, aux, dest, id } = node;
                history.push({ stepId: history.length, nodeId: id, line: 0, algoLine: 1, pegs: clonePegs(pegs), status: 'START', desc: `Called hanoi(${n}, ${src}, ${dest}, ${aux})`, speech: HANOI_SCRIPT.START(n, src, dest) });
                if (n === 1) {
                    history.push({ stepId: history.length, nodeId: id, line: 2, algoLine: 2, pegs: clonePegs(pegs), status: 'MOVING', moveDetails: { disk: 1, from: src, to: dest }, desc: `Move disk 1 from ${src} to ${dest}`, speech: HANOI_SCRIPT.MOVING(1, src, dest) });
                    const disk = pegs[src].pop(); pegs[dest].push(disk);
                    history.push({ stepId: history.length, nodeId: id, line: 2, algoLine: 2, pegs: clonePegs(pegs), status: 'MOVED', desc: `Disk 1 moved.` });
                } else {
                    const leftNode = { id: ++nodeIdCounter, parentId: id, n: n - 1, src: src, aux: dest, dest: aux, children: [], depth: node.depth + 1 };
                    node.children.push(leftNode);
                    history.push({ stepId: history.length, nodeId: id, line: 1, algoLine: 4, pegs: clonePegs(pegs), status: 'THINKING', desc: `Recursive call for n-1`, speech: HANOI_SCRIPT.THINKING(n-1) });
                    solve(leftNode, pegs);
                    history.push({ stepId: history.length, nodeId: id, line: 2, algoLine: 5, pegs: clonePegs(pegs), status: 'MOVING', moveDetails: { disk: n, from: src, to: dest }, desc: `Move disk ${n} from ${src} to ${dest}`, speech: HANOI_SCRIPT.MOVING(n, src, dest) });
                    const disk = pegs[src].pop(); pegs[dest].push(disk);
                    history.push({ stepId: history.length, nodeId: id, line: 2, algoLine: 5, pegs: clonePegs(pegs), status: 'MOVED', desc: `Disk ${n} moved.` });
                    const rightNode = { id: ++nodeIdCounter, parentId: id, n: n - 1, src: aux, aux: src, dest: dest, children: [], depth: node.depth + 1 };
                    node.children.push(rightNode);
                    history.push({ stepId: history.length, nodeId: id, line: 3, algoLine: 6, pegs: clonePegs(pegs), status: 'THINKING', desc: `Recursive call for n-1`, speech: HANOI_SCRIPT.THINKING(n-1) });
                    solve(rightNode, pegs);
                }
                history.push({ stepId: history.length, nodeId: id, line: 4, algoLine: null, pegs: clonePegs(pegs), status: 'RETURNING', desc: `Return from hanoi(${n})`, speech: HANOI_SCRIPT.RETURNING(n-1 > 0 ? n-1 : 0) });
            }
            solve(treeRoot, initialPegs);
        ...
        function calculateLayout(node) {
            ...
            node.width = totalWidth; return totalWidth;
        }
        ...
        const PegSystem = ({ pegs, currentN, srcName, destName, auxName, moveDetails }) => {
            const pegIds = ['A', 'B', 'C'];
            const isMoving = moveDetails !== null;
            const getDiskWidth = (val) => 20 + (val * 12);
            const getDiskColor = (val) => val > currentN ? "bg-slate-700 opacity-40 border-slate-600" : "bg-slate-300 border-slate-200 shadow-[0_0_8px_rgba(255,255,255,0.4)]";
            const themeMap = {
                src: { pole: "bg-emerald-500/80 shadow-[0_0_8px_rgba(16,185,129,0.5)]", base: "bg-emerald-500", text: "text-emerald-400" },
                dest: { pole: "bg-blue-500/80 shadow-[0_0_8px_rgba(59,130,246,0.5)]", base: "bg-blue-500", text: "text-blue-400" },
                aux: { pole: "bg-purple-500/80 shadow-[0_0_8px_rgba(168,85,247,0.5)]", base: "bg-purple-500", text: "text-purple-400" },
                none: { pole: "bg-slate-600", base: "bg-slate-600", text: "text-slate-500" }
            };
            return (
                <div className="relative flex justify-between items-end h-[140px] px-2 select-none">
                    {pegIds.map((pid) => {
                        const role = pid === srcName ? "src" : (pid === destName ? "dest" : (pid === auxName ? "aux" : null));
                        const theme = role ? themeMap[role] : themeMap.none;
                        let disks = pegs[pid] || [];
                        if (isMoving && moveDetails.from === pid) disks = disks.slice(0, -1);
                        return (
                            <div key={pid} className="relative flex flex-col items-center justify-end w-[80px] h-full">
                                <div className={`absolute bottom-[26px] w-2 rounded-t-lg h-[90px] transition-colors duration-300 ${theme.pole}`}></div>
                                <div className="z-10 flex flex-col-reverse items-center justify-start w-full h-[90px] mb-1">{disks.map((d, i) => (<div key={i} className={`h-4 rounded-sm border border-b-2 mb-[1px] transition-all duration-300 ${getDiskColor(d)}`} style={{ width: `${getDiskWidth(d)}px`, flexShrink: 0 }}></div>))}</div>
                                <div className={`w-full h-1.5 rounded-full transition-colors duration-300 ${theme.base}`}></div>
                                <span className={`font-mono font-bold mt-1 h-4 leading-none transition-colors duration-300 ${theme.text}`}>{pid}</span>
                            </div>
                        );
                    })}
                    {isMoving && (<div className="absolute z-50 h-4 rounded-sm bg-yellow-400 border border-yellow-200 border-b-2 shadow-[0_0_15px_rgba(250,204,21,0.8)]" style={{ width: `${getDiskWidth(moveDetails.disk)}px`, animation: 'fly-disk-linear 0.3s linear forwards', '--startX': `${(pegIds.indexOf(moveDetails.from) * 33.33) + 16.66}%`, '--endX': `${(pegIds.indexOf(moveDetails.to) * 33.33) + 16.66}%`, '--startY': `${(pegs[moveDetails.from].length - 1) * 17 + 30}px`, '--endY': `${(pegs[moveDetails.to].length) * 17 + 30}px` }} />)}
                </div>
            );
        };
        ...
        const NodeComponent = ({ nodeData, isActive, isBreadcrumb, latestState }) => {
            ...
            const { n, src, aux, dest } = nodeData;
            let highlightLine = -1, statusColor = "bg-slate-700", statusText = "IDLE", borderColor = "border-slate-700", shadow = "", opacity = 0.3, isWaiting = false, pegsToRender = { 'A':[], 'B':[], 'C':[] }, moveDetails = null;
            if (latestState) {
                pegsToRender = latestState.pegs;
                if (isActive) {
                    opacity = 1; shadow = 'shadow-yellow-500/20'; highlightLine = latestState.line;
                    if (latestState.status === 'START') { statusColor = "bg-emerald-600"; statusText = "START"; borderColor="border-emerald-500"; }
                    if (latestState.status === 'MOVING') { statusColor = "bg-yellow-600"; statusText = "MOVING"; borderColor="border-yellow-500"; moveDetails = latestState.moveDetails; }
                    if (latestState.status === 'MOVED') { statusColor = "bg-yellow-600"; statusText = "MOVED"; borderColor="border-yellow-500"; }
                    if (latestState.status === 'RETURNING') { statusColor = "bg-rose-600"; statusText = "RETURN"; borderColor="border-rose-500"; }
                    if (latestState.status === 'THINKING') { statusColor = "bg-blue-600"; statusText = "CALLING"; borderColor="border-blue-500"; }
                } else {
                    if (latestState.status === 'RETURNING') { statusColor = "bg-slate-600"; statusText = "COMPLETED"; borderColor = "border-slate-600"; opacity = 0.8; }
                    else if (latestState.status === 'THINKING') { statusColor = "bg-indigo-600"; statusText = "WAITING"; borderColor = "border-indigo-500/50"; highlightLine = latestState.line; opacity = 1; isWaiting = true; }
                    else { statusColor = "bg-slate-600"; statusText = "PAUSED"; opacity = 0.9; }
                }
            }
            if (isBreadcrumb && !isActive) { borderColor = 'border-indigo-500/70'; shadow = 'shadow-indigo-500/20'; opacity = 1; }
            return (
                <div className={`absolute rounded-xl shadow-2xl overflow-hidden transition-all duration-300 border-2 bg-slate-800 flex flex-col justify-between ${borderColor} ${shadow}`} style={{ left: nodeData.x, top: nodeData.y, width: NODE_WIDTH, height: NODE_HEIGHT, transform: isActive ? 'scale(1.05)' : 'scale(1)', zIndex: isActive ? 50 : (isBreadcrumb ? 40 : 10), opacity: opacity, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                    <div className="bg-slate-900/80 px-4 py-2 flex justify-between items-center border-b border-slate-700"><span className={`font-mono font-bold text-sm ${isActive || isBreadcrumb ? 'text-blue-200' : 'text-slate-400'}`}>hanoi(n={n})</span><span className={`text-[10px] font-bold px-2 py-0.5 rounded ${statusColor} text-white shadow-sm`}>{statusText}</span></div>
                    <div className="px-3 py-2 text-[11px] font-mono text-slate-400 bg-slate-900 leading-relaxed border-b border-slate-700">
                        <div className={`px-2 rounded ${highlightLine === 0 ? 'code-highlight text-yellow-200' : ''}`}>// function start</div>
                        <div className={`px-2 rounded ${highlightLine === 1 ? (isWaiting ? 'code-highlight-waiting text-indigo-300' : 'code-highlight text-yellow-200') : ''}`}>1. hanoi(n-1, {src}, {dest}, {aux});</div>
                        <div className={`px-2 rounded ${highlightLine === 2 ? 'code-highlight text-yellow-200' : ''}`}>2. move disk {n} {src}?{dest};</div>
                        <div className={`px-2 rounded ${highlightLine === 3 ? (isWaiting ? 'code-highlight-waiting text-indigo-300' : 'code-highlight text-yellow-200') : ''}`}>3. hanoi(n-1, {aux}, {src}, {dest});</div>
                        <div className={`px-2 rounded ${highlightLine === 4 ? 'code-highlight text-yellow-200' : ''}`}>// return</div>
                    </div>
                    <div className="bg-slate-800 flex-1 relative p-2 pt-3 flex flex-col justify-end">
                         <div className="w-full flex justify-center gap-5 text-[10px] text-slate-500 font-mono mb-2"><span>src=<b className="text-emerald-400">{src}</b></span><span>dest=<b className="text-blue-400">{dest}</b></span><span>aux=<b className="text-purple-400">{aux}</b></span></div>
                         <PegSystem pegs={pegsToRender} currentN={n} srcName={src} auxName={aux} destName={dest} moveDetails={moveDetails} />
                    </div>
                </div>
            );
        };
        ...
        const App = () => {
            ...
            const { history, flatNodes } = useMemo(() => {
                const { treeRoot, history } = generateSimulation(diskCount);
            ...
            const speak = (text) => {
                ...
                utterance.pitch = 1.2; utterance.rate = 1.1 * speedMultiplier;
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">Recursive Hanoi Master</h1><span className="text-xs text-slate-400 font-mono">Cinematic Simulation</span><button onClick={() => setIsSettingsOpen(true)} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors text-slate-400 hover:text-white shadow-lg"><SettingsIcon size={20}/></button></div></div>
                            <div className="flex items-center gap-2 bg-slate-800/80 p-1 pr-3 rounded-lg border border-slate-700/50"><span className="bg-slate-700/80 px-2 py-1 rounded text-slate-300 text-xs font-bold uppercase mr-1">Disks</span><select value={diskCount} onChange={(e) => { setDiskCount(Number(e.target.value)); setCurrentStep(0); setIsPlaying(false); }} className="bg-transparent font-bold outline-none text-blue-400 cursor-pointer">{[1, 2, 3, 4, 5].map(n => <option key={n} value={n}>{n}</option>)}</select></div>
                        </div>
                    </div>
                    
                    <SettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} />
                    <AlgorithmPanel algoKey="HANOI" currentLine={currentState?.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper') && !e.target.closest('.top-nav-area')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
