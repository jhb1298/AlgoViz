...
import './expression_script.js';
import './settings_manager.js';
import './algorithm_data.js';
import './chatbot.js';

/* extracted from infix.html */
        ...
        function generateSimulation(expression) {
            const history = [];
            const nums = [];
            const ops = [];
            ...
            function applyOp(algoLine) {
                ...
                history.push({ stepId: history.length, nums: [...nums, a, b], ops: [...ops, op], algoLine, status: 'CALC_START', desc: `Applying ${op}`, speech: EXPRESSION_SCRIPT.INFIX.CALC(a, b, op, res) });
                nums.push(res);
                history.push({ stepId: history.length, nums: [...nums], ops: [...ops], algoLine, status: 'CALC_END', desc: `Result ${res} pushed` });
            }

            history.push({ stepId: 0, nums: [], ops: [], currentTokenIdx: -1, algoLine: null, status: 'START', desc: "Evaluating Infix", speech: EXPRESSION_SCRIPT.INFIX.START(expression) });

            for (let i = 0; i < tokens.length; i++) {
                const token = tokens[i];
                if (!isNaN(token)) {
                    nums.push(Number(token));
                    history.push({ stepId: history.length, nums: [...nums], ops: [...ops], currentTokenIdx: i, algoLine: 2, status: 'NUM', desc: `Push number ${token}`, speech: EXPRESSION_SCRIPT.INFIX.NUMBER(token) });
                } else if (token === '(') {
                    ops.push(token);
                    history.push({ stepId: history.length, nums: [...nums], ops: [...ops], currentTokenIdx: i, algoLine: 3, status: 'OPEN', desc: "Open parenthesis", speech: EXPRESSION_SCRIPT.INFIX.OPEN_PAREN() });
                } else if (token === ')') {
                    history.push({ stepId: history.length, nums: [...nums], ops: [...ops], currentTokenIdx: i, algoLine: 4, status: 'CLOSE', desc: "Close parenthesis", speech: EXPRESSION_SCRIPT.INFIX.CLOSE_PAREN() });
                    while (ops[ops.length - 1] !== '(') {
                        applyOp(4);
                    }
                    ops.pop(); // pop '('
                    history.push({ stepId: history.length, nums: [...nums], ops: [...ops], currentTokenIdx: i, algoLine: 4, status: 'PAREN_DONE', desc: "Parenthesis solved" });
                } else {
                    while (ops.length > 0 && precedence[ops[ops.length - 1]] >= precedence[token]) {
                        applyOp(5);
                    }
                    ops.push(token);
                    history.push({ stepId: history.length, nums: [...nums], ops: [...ops], currentTokenIdx: i, algoLine: 5, status: 'OP', desc: `Push operator ${token}`, speech: EXPRESSION_SCRIPT.INFIX.OPERATOR(token) });
                }
            }
            ...
            history.push({ stepId: history.length, nums: [...nums], ops: [...ops], currentTokenIdx: tokens.length, algoLine: 7, status: 'SUCCESS', desc: "Evaluation Complete!", speech: EXPRESSION_SCRIPT.INFIX.SUCCESS(nums[0]) });
            return history;
        }
        ...
        const App = () => {
            ...
            const [input, setInput] = useState("3 * ( 5 + 2 ) - 8 / 4");
            ...
            const history = useMemo(() => generateSimulation(input), [input]);
            ...
            const state = history[currentStep] || { nums: [], ops: [], currentTokenIdx: -1 };
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">Infix Evaluation</h1><span className="text-xs text-slate-400 font-mono">Two-stack precedence processing</span><button onClick={() => setIsSettingsOpen(true)} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors text-slate-400 hover:text-white shadow-lg"><SettingsIcon size={20}/></button></div></div>
                            <div className="flex items-center gap-2 bg-slate-800/80 p-1 pr-3 rounded-lg border border-slate-700/50">
                                <span className="bg-slate-700/80 px-2 py-1 rounded text-slate-300 text-xs font-bold uppercase mr-1">Expression</span>
                                <input type="text" value={input} onChange={(e) => { setInput(e.target.value); setCurrentStep(0); setIsPlaying(false); }} className="bg-transparent font-bold outline-none text-blue-400 w-80 border-b border-blue-500/30 text-center" />
                            </div>
                        </div>
                    </div>
                    ...
                    <AlgorithmPanel algoKey="INFIX" currentLine={state?.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`absolute inset-0 flex flex-col items-center justify-center gap-12 ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            
                            {/* Expression Display */}
                            <div className="flex gap-2">
                                {tokens.map((token, i) => (
                                    <div key={i} className={`w-10 h-12 bg-slate-800 border-2 border-slate-700 rounded-lg flex items-center justify-center font-bold text-lg transition-all duration-300 ${state.currentTokenIdx === i ? 'token-active' : (i < state.currentTokenIdx ? 'opacity-30 scale-90' : '')}`}>
                                        {token}
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="flex gap-20 items-end">
                                ...
                                <div className="flex flex-col items-center gap-4">
                                    <div className="flex flex-col-reverse items-center gap-2 min-h-[300px] w-32 p-4 border-x-4 border-b-4 border-emerald-700/50 rounded-b-3xl bg-emerald-900/10">
                                        {state.nums.map((val, i) => (
                                            <div key={i} className="w-full h-10 bg-emerald-600 border-2 border-emerald-400 rounded-lg flex items-center justify-center font-black text-lg shadow-lg stack-item">
                                                {val}
                                            </div>
                                        ))}
                                    </div>
                                    <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-500">Operand Stack</span>
                                </div>

                                {/* Operator Stack */}
                                <div className="flex flex-col items-center gap-4">
                                    <div className="flex flex-col-reverse items-center gap-2 min-h-[300px] w-32 p-4 border-x-4 border-b-4 border-blue-700/50 rounded-b-3xl bg-blue-900/10">
                                        {state.ops.map((val, i) => (
                                            <div key={i} className="w-full h-10 bg-blue-600 border-2 border-blue-400 rounded-lg flex items-center justify-center font-black text-lg shadow-lg stack-item">
                                                {val}
                                            </div>
                                        ))}
                                    </div>
                                    <span className="text-[10px] font-bold uppercase tracking-widest text-blue-500">Operator Stack</span>
                                </div>
                            </div>
                        </div>
                    </div>
