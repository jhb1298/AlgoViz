...
E F`;
        ...
        function generateSimulation(graph) {
            ...
            function dfs(u, p = null) {
                ...
                history.push({ 
                    ...
                    stack: [...currentStack],
                    status: 'VISITING', desc: `Visiting ${u}`, 
                    speech: SEARCH_SCRIPT.DFS.VISITING(u), algoLine: 5
                });
                ...
                for (const v of adj[u].sort()) {
                    if (!visited.has(v)) {
                        history.push({ 
                            ...
                            stack: [...currentStack],
                            status: 'EXPLORING', desc: `Checking ${v}`, 
                            speech: SEARCH_SCRIPT.DFS.EXPLORING_EDGE(u, v), algoLine: 6
                        });
                        dfs(v, u);
                        history.push({ 
                            stepId: history.length, activeNode: u, 
                            visited: new Set(visited), 
                            stack: [...currentStack],
                            status: 'BACK', 
                            desc: `Back to ${u}`, speech: `Backtracking to ${u}`, algoLine: 3
                        });
                    }
                }
                currentStack.pop();
            ...
            history.push({ stepId: 0, activeNode: source, visited: new Set(), stack: [], status: 'START', desc: `Starting DFS at ${source}`, speech: SEARCH_SCRIPT.DFS.START(source), algoLine: 1 });
            dfs(source);
            history.push({ stepId: history.length, status: 'SUCCESS', visited: new Set(visited), stack: [], desc: "DFS Complete", speech: SEARCH_SCRIPT.DFS.SUCCESS(), algoLine: 2 });
            return history;
        }
        ...
        const NodeComponent = ({ node, isActive, isVisited }) => {
            ...
            return (
                <div 
                    className={`absolute w-20 h-20 rounded-full bg-slate-800 border-4 flex flex-col items-center justify-center transition-all duration-500 z-20 ${isActive ? 'node-active' : (isVisited ? 'node-visited' : 'border-slate-700')}`} 
                    style={{ 
                        left: node.x - 40, top: node.y - 40, 
                        fontSize: settings.fontSize + "px", 
                        borderColor: isActive ? settings.nodeColor : undefined, 
        ...
        const StackComponent = ({ stack }) => (
            <div className="flex flex-col-reverse gap-2">
                {stack.map((id, i) => (
                    <div key={i} className="flex items-center justify-center w-full h-12 bg-slate-800 border-2 border-indigo-500/50 rounded-xl font-black text-indigo-400 shadow-lg animate-in fade-in slide-in-from-bottom-2 duration-300">
                        {id}
                    </div>
                ))}
            </div>
        ...
        const App = () => {
            ...
            const state = history[currentStep] || { visited: new Set(), stack: [] };
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="font-bold text-lg leading-none tracking-tight">DFS Visualizer</h1>
                                    <span className="text-xs text-slate-400 font-mono">Exploring paths to the maximum depth</span>
                            ...
                            <div className="flex items-center gap-3">
                                <button onClick={() => setShowSidebar(!showSidebar)} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold transition-all ${showSidebar ? 'bg-slate-700 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}>
                                    {showSidebar ? "Hide Stack" : "Show Stack"}
                                </button>
                    ...
                    {isEditing && (
                        <div className="absolute inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-6 text-white pointer-events-auto">
                            <div className="bg-slate-900 border border-slate-700 rounded-3xl p-8 max-w-lg w-full shadow-2xl">
                                ...
                                <p className="text-slate-400 text-sm mb-4">Format: <code className="bg-slate-800 px-1.5 py-0.5 rounded text-blue-300">Node1 Node2</code></p>
                    ...
                    <AlgorithmPanel algoKey="DFS" currentLine={state.algoLine} />
                    ...
                    <div className="flex flex-1 overflow-hidden relative">
                        ...
                        <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab active:cursor-grabbing" onWheel={(e) => setZoom(z => Math.min(Math.max(0.1, z-e.deltaY*0.001), 2.0))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper') && !e.target.closest('.top-nav-area')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                            <div className={`node-container absolute origin-top-left ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                                ...
                                {graph.nodes.map(node => (
                                    <NodeComponent key={node.id} node={node} isActive={state.activeNode === node.id} isVisited={state.visited.has(node.id)} />
                                ))}
                            </div>
                        </div>
                        ...
                        {/* Stack Sidebar */}
                        {showSidebar && (
                            <div className="w-64 bg-slate-900/50 backdrop-blur-xl border-l border-slate-700/50 flex flex-col p-8 overflow-y-auto gap-10 z-40 relative shadow-2xl">
                                <div>
                                    <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-400 mb-6">RECURSION STACK</h3>
                                    <StackComponent stack={state.stack || []} />
                                </div>
                            </div>
                        )}
                    </div>
