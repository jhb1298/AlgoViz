...
/* extracted from prefix.html */
        ...
        function generateSimulation(expression) {
            ...
            history.push({ stepId: 0, stack: [], currentTokenIdx: tokens.length, algoLine: null, status: 'START', desc: "Scanning right to left", speech: EXPRESSION_SCRIPT.PREFIX.START(expression) });

            for (let i = tokens.length - 1; i >= 0; i--) {
                const token = tokens[i];
                if (!isNaN(token)) {
                    stack.push(Number(token));
                    history.push({ stepId: history.length, stack: [...stack], currentTokenIdx: i, algoLine: 3, status: 'PUSH', desc: `Push ${token}`, speech: EXPRESSION_SCRIPT.PREFIX.NUMBER(token) });
                } else {
                    const a = stack.pop();
                    const b = stack.pop();
                    let res = 0;
                    ...
                    history.push({ stepId: history.length, stack: [...stack, b, a], currentTokenIdx: i, algoLine: 5, status: 'OPERATOR', desc: `Operator ${token}`, speech: EXPRESSION_SCRIPT.PREFIX.OPERATOR(token) });
                    history.push({ stepId: history.length, stack: [...stack], currentTokenIdx: i, algoLine: 6, status: 'POP', desc: `Popped ${a} and ${b}`, speech: EXPRESSION_SCRIPT.PREFIX.CALC(a, b, token, res) });
                    stack.push(res);
                    history.push({ stepId: history.length, stack: [...stack], currentTokenIdx: i, algoLine: 7, status: 'PUSH_RES', desc: `Push result ${res}` });
                }
            }

            history.push({ stepId: history.length, stack: [...stack], currentTokenIdx: -1, algoLine: 8, status: 'SUCCESS', desc: "Final Result!", speech: EXPRESSION_SCRIPT.PREFIX.SUCCESS(stack[0]) });
            return history;
        }
        ...
        const App = () => {
            ...
            const [input, setInput] = useState("- * + 5 3 2 8");
            ...
            const state = history[currentStep] || { stack: [], currentTokenIdx: -1 };
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            ...
                            <div className="flex items-center gap-2 bg-slate-800/80 p-1 pr-3 rounded-lg border border-slate-700/50">
                                ...
                                <input type="text" value={input} onChange={(e) => { setInput(e.target.value); setCurrentStep(0); setIsPlaying(false); }} className="bg-transparent font-bold outline-none text-blue-400 w-64 border-b border-blue-500/30 text-center" />
                            </div>
                        </div>
                    </div>
                    ...
                    <AlgorithmPanel algoKey="PREFIX" currentLine={state?.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.5, z-e.deltaY*0.001), 2))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`absolute inset-0 flex flex-col items-center justify-center gap-12 ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            ...
                            <div className="flex gap-3">
                                {tokens.map((token, i) => (
                                    <div key={i} className={`w-12 h-14 bg-slate-800 border-2 border-slate-700 rounded-xl flex items-center justify-center font-bold text-xl transition-all duration-300 ${state.currentTokenIdx === i ? 'token-active' : (i > state.currentTokenIdx ? 'opacity-30 scale-90' : '')}`}>
                                        {token}
                                    </div>
                                ))}
                            </div>
                            ...
                            {/* Stack Visualization */}
                            <div className="flex flex-col-reverse items-center gap-2 min-h-[300px] w-48 p-4 border-x-4 border-b-4 border-slate-700 rounded-b-3xl bg-slate-800/20">
                                {state.stack.map((val, i) => (
                                    <div key={i} className="w-full h-12 bg-emerald-600 border-2 border-emerald-400 rounded-lg flex items-center justify-center font-black text-xl shadow-lg stack-item">
                                        {val}
                                    </div>
                                ))}
                                {state.stack.length === 0 && <span className="text-slate-600 font-mono text-sm mb-4">Empty Stack</span>}
                            </div>
                            <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Value Stack</span>
                        </div>
                    </div>
