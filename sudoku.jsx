...
/* extracted from sudoku.html */
        ...
        function isSafe(board, row, col, num) {
            for (let x = 0; row < 9 && x < 9; x++) if (board[row][x] === num) return { safe: false, reason: "row" };
            for (let x = 0; col < 9 && x < 9; x++) if (board[x][col] === num) return { safe: false, reason: "column" };
            let startRow = row - (row % 3), startCol = col - (col % 3);
            for (let i = 0; i < 3; i++)
                for (let j = 0; j < 3; j++)
                    if (board[i + startRow][j + startCol] === num) return { safe: false, reason: "3x3 box" };
            return { safe: true };
        }
        ...
        function generateSimulation() {
            ...
            const fixed = INITIAL_BOARD.map(row => row.map(cell => cell !== 0));
            ...
            history.push({
                ...
                algoLine: 1,
                status: 'START',
                desc: 'Let\'s begin solving the puzzle.',
                speech: SUDOKU_SCRIPT.START()
            });
                    ...
                    for (let col = 0; col < 9; col++) {
                        if (board[row][col] === 0) {
                            for (let num = 1; num <= 9; num++) {
                                const check = isSafe(board, row, col, num);
                                
                                history.push({
                                    stepId: history.length,
                                    board: board.map(r => [...r]),
                                    fixed,
                                    active: { row, col, val: num },
                                    status: check.safe ? 'SAFE' : 'CONFLICT',
                                    algoLine: check.safe ? 5 : 4,
                                    reason: check.reason,
                                    desc: check.safe ? `Placing ${num} is safe.` : `Conflict in ${check.reason}!`,
                                    speech: history.length % 100 === 0 ? SUDOKU_SCRIPT.TRYING(num, row, col) : null
                                });
                                ...
                                if (check.safe) {
                                    ...
                                    history.push({
                                        ...
                                        status: 'BACKTRACK',
                                        algoLine: 7,
                                        desc: `Backtracking from (${row+1}, ${col+1})`,
                                        speech: history.length % 150 === 0 ? SUDOKU_SCRIPT.BACKTRACK(row, col) : null
                                    });
                                }
                            }
                            return false;
        ...
        const App = () => {
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none">
                    <div className="absolute top-0 left-0 right-0 z-[60]">
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="font-bold text-lg leading-none tracking-tight">Sudoku Solver</h1>
                                    <span className="text-xs text-slate-400 font-mono">Recursive Backtracking</span>
                    ...
                    <AlgorithmPanel algoKey="SUDOKU" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden flex items-center justify-center p-20">
                        <div className="sudoku-grid grid grid-cols-9 bg-slate-800 shadow-2xl" style={{ width: '540px', height: '540px', transform: `scale(${zoom})` }}>
                            {state.board.map((row, r) => (
                                row.map((val, c) => {
                                    const isActive = state.active?.row === r && state.active?.col === c;
                                    const isFixed = state.fixed[r][c];
                                    const displayVal = isActive ? state.active.val : (val || "");
                                    
                                    return (
                                        <div key={`${r}-${c}`} className={`sudoku-cell flex items-center justify-center text-2xl font-bold
                                            ${isActive ? (state.status === 'CONFLICT' ? 'sudoku-cell-conflict sudoku-cell-active' : 'sudoku-cell-active') : ''}
                                            ${isFixed ? 'sudoku-cell-fixed' : ''}
                                            ${state.status === 'SAFE' && isActive ? 'sudoku-cell-safe' : ''}
                                            ${(c + 1) % 3 === 0 && c < 8 ? 'subgrid-border-r' : ''}
                                            ${(r + 1) % 3 === 0 && r < 8 ? 'subgrid-border-b' : ''}
                                        `}>
                                            {displayVal}
                                        </div>
                                    );
