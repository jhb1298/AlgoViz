...
/* extracted from graham_scan.html */
        ...
        function generateSimulation(points) {
            ...
            // 1. Find lowest Y point
            let minY = pts[0].y;
            let minIdx = 0;
            for (let i = 1; i < pts.length; i++) {
                let y = pts[i].y;
                if ((y > minY) || (y === minY && pts[i].x < pts[minIdx].x)) { // SVG Y is down
                    minY = y;
                    minIdx = i;
                }
            }
            ...
            history.push({
                stepId: 0, points: pts, stack: [], current: p0, 
                algoLine: 1,
                status: 'START', desc: "Finding starting point...",
                speech: GEOMETRY_SCRIPT.GRAHAM.START()
            });

            history.push({
                stepId: history.length, points: pts, stack: [], current: p0,
                algoLine: 1,
                status: 'PICK_START', desc: `Lowest point is ${p0.id}`,
                speech: GEOMETRY_SCRIPT.GRAHAM.PICK_START(p0)
            });

            // 2. Sort by polar angle
            const sorted = pts.slice(1).sort((a, b) => {
                const cp = crossProduct(p0, a, b);
                if (cp === 0) return distSq(p0, a) - distSq(p0, b);
                return cp;
            });
            pts = [p0, ...sorted];

            history.push({
                stepId: history.length, points: pts, stack: [], current: null,
                algoLine: 2,
                status: 'SORTED', desc: "Sorted points by angle",
                speech: GEOMETRY_SCRIPT.GRAHAM.SORTING()
            });
            ...
            history.push({
                stepId: history.length, points: pts, stack: [pts[0], pts[1], pts[2]], current: pts[2],
                algoLine: 3,
                status: 'PUSH', desc: `Added initial points ${pts[0].id}, ${pts[1].id}, ${pts[2].id}`,
                speech: `Starting with first three points.`
            });

            for (let i = 3; i < pts.length; i++) {
                history.push({
                    stepId: history.length, points: pts, stack: [...stack], current: pts[i],
                    algoLine: 4,
                    status: 'CHECK', desc: `Checking point ${pts[i].id}`,
                    speech: GEOMETRY_SCRIPT.GRAHAM.CHECKING(pts[i])
                });

                while (stack.length >= 2 && crossProduct(stack[stack.length - 2], stack[stack.length - 1], pts[i]) >= 0) {
                    const popped = stack.pop();
                    history.push({
                        stepId: history.length, points: pts, stack: [...stack], current: pts[i], popped,
                        algoLine: 6,
                        status: 'POP', desc: `Removing ${popped.id} (inward turn)`,
                        speech: GEOMETRY_SCRIPT.GRAHAM.BACKTRACKING(popped)
                    });
                }
                stack.push(pts[i]);
                history.push({
                    stepId: history.length, points: pts, stack: [...stack], current: pts[i],
                    algoLine: 7,
                    status: 'PUSH', desc: `Added ${pts[i].id} to hull`,
                    speech: GEOMETRY_SCRIPT.GRAHAM.PUSHING(pts[i])
                });
            }

            history.push({
                stepId: history.length, points: pts, stack: [...stack, pts[0]], current: null,
                algoLine: 8,
                status: 'SUCCESS', desc: "Hull Complete!",
                speech: GEOMETRY_SCRIPT.GRAHAM.SUCCESS()
            });
        ...
        const App = () => {
            ...
            const state = history[currentStep] || { points: [], stack: [] };

            const handleCanvasClick = (e) => {
                if (isPlaying || isDraggingCanvas) return;
                const rect = e.currentTarget.getBoundingClientRect();
                const x = (e.clientX - rect.left - manualOffset.x) / zoom;
                const y = (e.clientY - rect.top - manualOffset.y) / zoom;
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <AlgorithmPanel algoKey="GRAHAM_SCAN" currentLine={state?.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-crosshair" onClick={handleCanvasClick} onWheel={(e) => setZoom(z => Math.min(Math.max(0.2, z-e.deltaY*0.001), 2))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`absolute left-0 top-0 w-full h-full ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${manualOffset.x}px, ${manualOffset.y}px) scale(${zoom})` }}>
                            <svg className="absolute top-0 left-0 overflow-visible w-full h-full">
                                {/* Sorted Path (Angle Order) */}
                                {state.status === 'SORTED' && state.points.length > 1 && (
                                    <path d={`M ${state.points.map(p => `${p.x},${p.y}`).join(' L ')}`} fill="none" stroke="#334155" strokeWidth="1" strokeDasharray="5 5" />
                                )}

                                {/* Stack Connections */}
                                {state.stack.length > 1 && (
                                    <path d={`M ${state.stack.map(p => `${p.x},${p.y}`).join(' L ')}`} fill="rgba(99, 102, 241, 0.1)" stroke="#6366f1" strokeWidth="4" />
                                )}

                                {/* Current Check Line */}
                                {state.status === 'CHECK' && state.stack.length >= 1 && state.current && (
                                    <line x1={state.stack[state.stack.length-1].x} y1={state.stack[state.stack.length-1].y} x2={state.current.x} y2={state.current.y} stroke="#f43f5e" strokeWidth="2" className="line-scanning" />
                                )}

                                {/* Popped line visualization */}
                                {state.status === 'POP' && state.stack.length >= 1 && state.popped && state.current && (
                                    <g>
                                        <line x1={state.stack[state.stack.length-1].x} y1={state.stack[state.stack.length-1].y} x2={state.popped.x} y2={state.popped.y} className="line-bad" />
                                        <line x1={state.popped.x} y1={state.popped.y} x2={state.current.x} y2={state.current.y} className="line-bad" />
                                    </g>
                                )}

                                {/* Points */}
                                {state.points.map((p, i) => {
                                    const stackIdx = state.stack.findIndex(hp => hp.id === p.id);
                                    const isHull = stackIdx !== -1;
                                    const isActive = state.current?.id === p.id;
                                    return (
                                        <g key={p.id} className="point">
                                            ...
                                            {isHull && <text x={p.x} y={p.y + 20} textAnchor="middle" className="text-[8px] font-bold fill-emerald-400">{stackIdx}</text>}
