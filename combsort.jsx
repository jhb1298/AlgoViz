...
/* extracted from combsort.html */
        ...
        function generateSimulation(arr) {
            ...
            history.push({ stepId: 0, arr: [...data], gap, status: 'START', algoLine: 1, desc: "Starting Comb Sort", speech: SORTING_SCRIPT.COMB.START() });
            ...
            while (!sorted) {
                ...
                history.push({ stepId: history.length, arr: [...data], gap, status: 'GAP', algoLine: 3, desc: `Gap set to ${gap}`, speech: SORTING_SCRIPT.COMB.GAP(gap) });

                for (let i = 0; i + gap < n; i++) {
                    history.push({ stepId: history.length, arr: [...data], gap, compare: [i, i + gap], status: 'COMPARING', algoLine: 5, desc: `Comparing indices ${i} and ${i + gap}`, speech: SORTING_SCRIPT.COMB.COMPARING(data[i], data[i+gap]) });
                    if (data[i] > data[i + gap]) {
                        [data[i], data[i + gap]] = [data[i + gap], data[i]];
                        sorted = false;
                        history.push({ stepId: history.length, arr: [...data], gap, swap: [i, i + gap], status: 'SWAPPING', algoLine: 6, desc: `Swapped ${data[i+gap]} and ${data[i]}` });
                    }
                }
            }

            history.push({ stepId: history.length, arr: [...data], gap: 1, status: 'SUCCESS', algoLine: 6, desc: "Sort Complete!", speech: SORTING_SCRIPT.COMB.SUCCESS() });
            return history;
        }
        ...
        const App = () => {
            ...
            const [input, setInput] = useState("40,10,100,20,50,70,30,90,60,80");
            ...
            return (
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    ...
                    <AlgorithmPanel algoKey="COMBSORT" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.5, z-e.deltaY*0.001), 2))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 flex items-end justify-center gap-2 ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(calc(-50% + ${manualOffset.x}px), calc(-50% + ${manualOffset.y}px)) scale(${zoom})` }}>
                            {state.arr.map((val, i) => {
                                ...
                                if (state.compare && state.compare.includes(i)) bgColor = "bg-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.5)]";
                                if (state.swap && state.swap.includes(i)) bgColor = "bg-rose-500 shadow-[0_0_20px_rgba(244,63,94,0.6)]";
