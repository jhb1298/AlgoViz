...
import './search_script.js';
import './algorithm_data.js';
import './chatbot.js';
import './settings_manager.js';

/* extracted from binarysearch.html */
        ...
        function generateSimulation(arr, target) {
            const history = [];
            ...
            history.push({ 
                stepId: 0, arr: data, low, high, mid: -1, 
                status: 'START', desc: `Searching for ${target}`, 
                speech: SEARCH_SCRIPT.BINARY.START(target),
                algoLine: 1
            });
            
            while (low <= high) {
                history.push({ 
                    stepId: history.length, arr: data, low, high, mid: -1, 
                    status: 'WHILE', desc: `Check if range ${low}-${high} is valid`, 
                    speech: null,
                    algoLine: 2
                });
                ...
                history.push({ 
                    stepId: history.length, arr: data, low, high, mid, 
                    status: 'MID', desc: `Calculating mid index: ${mid}`, 
                    speech: SEARCH_SCRIPT.BINARY.MID(val),
                    algoLine: 3
                });
                ...
                if (val === target) {
                    history.push({ 
                        stepId: history.length, arr: data, low, high, mid, 
                        status: 'FOUND', desc: `Found match at index ${mid}!`, 
                        speech: SEARCH_SCRIPT.BINARY.FOUND(target, mid),
                        algoLine: 4
                    });
                    break;
                }
                ...
                if (val < target) {
                    history.push({ 
                        stepId: history.length, arr: data, low, high, mid, 
                        status: 'TOO_LOW', desc: `${val} < ${target}, discarding left half`, 
                        speech: SEARCH_SCRIPT.BINARY.TOO_LOW(val, target),
                        algoLine: 5
                    });
                    low = mid + 1;
                } else {
                    history.push({ 
                        stepId: history.length, arr: data, low, high, mid, 
                        status: 'TOO_HIGH', desc: `${val} > ${target}, discarding right half`, 
                        speech: SEARCH_SCRIPT.BINARY.TOO_HIGH(val, target),
                        algoLine: 6
                    });
                    high = mid - 1;
                }
            ...
            if (low > high) {
                history.push({ 
                    stepId: history.length, arr: data, low, high, mid: -1, 
                    status: 'NOT_FOUND', desc: `${target} not found in array`, 
                    speech: SEARCH_SCRIPT.BINARY.NOT_FOUND(target),
                    algoLine: 7
                });
            }
            ...
            history.push({ 
                stepId: history.length, arr: data, low, high, mid: -1, 
                status: 'SUCCESS', desc: "Search cycle completed", 
                speech: SEARCH_SCRIPT.BINARY.SUCCESS(),
                algoLine: 7
            });
            return history;
        }
        ...
        const App = () => {
            ...
            const [input, setInput] = useState("10,23,35,42,58,61,74,89,95");
            ...
            const history = useMemo(() => {
                ...
                return arr.length ? generateSimulation(arr, target) : [];
            }, [input, target]);
            ...
            const state = history[currentStep] || { arr: [], low: 0, high: -1, mid: -1 };
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">Binary Search Visualizer</h1><span className="text-xs text-slate-400 font-mono">Logarithmic Divide & Conquer</span><button onClick={() => setIsSettingsOpen(true)} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors text-slate-400 hover:text-white shadow-lg"><SettingsIcon size={20}/></button></div></div>
                            <div className="flex items-center gap-4">
                                <div className="flex items-center gap-2 bg-slate-800/80 p-1 px-3 rounded-lg border border-slate-700/50">
                                    <span className="text-slate-500 text-[10px] font-bold uppercase">Target</span>
                                    <input type="number" value={target} onChange={(e) => { setTarget(Number(e.target.value)); setCurrentStep(0); setIsPlaying(false); }} className="bg-transparent font-bold outline-none text-emerald-400 w-16 text-center" />
                                </div>
                                <div className="flex items-center gap-2 bg-slate-800/80 p-1 px-3 rounded-lg border border-slate-700/50">
                                    <span className="text-slate-500 text-[10px] font-bold uppercase">Sorted Array</span>
                                    <input type="text" value={input} onChange={(e) => { setInput(e.target.value); setCurrentStep(0); setIsPlaying(false); }} className="bg-transparent font-bold outline-none text-blue-400 w-48 text-center text-sm" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <AlgorithmPanel algoKey="BINARY_SEARCH" currentLine={state.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.5, z-e.deltaY*0.001), 2))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 flex items-end justify-center gap-2 ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(calc(-50% + ${manualOffset.x}px), calc(-50% + ${manualOffset.y}px)) scale(${zoom})` }}>
                            {state.arr.map((val, i) => {
                                const isCurrentRange = i >= state.low && i <= state.high;
                                const isMid = i === state.mid;
                                const isFound = state.status === 'FOUND' && isMid;
                                
                                let bgColor = isCurrentRange ? "bg-slate-600" : "bg-slate-800 opacity-20";
                                let shadow = "";
                                let transform = "translateY(0)";

                                if (isMid) {
                                    bgColor = "bg-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.5)]";
                                    transform = "translateY(-15px)";
                                }
                                if (isFound) {
                                    bgColor = "bg-emerald-500 shadow-[0_0_25px_rgba(16,185,129,0.6)] scale-110";
                                    transform = "translateY(-25px)";
                                }

                                return (
                                    <div key={i} className="flex flex-col items-center gap-2 relative bar-transition" style={{ width: BAR_WIDTH, transform }}>
                                        {i === state.low && isCurrentRange && <span className="absolute -top-12 marker-label text-blue-400">Low</span>}
                                        {i === state.high && isCurrentRange && <span className="absolute -top-12 marker-label text-rose-400">High</span>}
                                        {isMid && <span className="absolute -top-20 marker-label text-yellow-400 font-black text-xs">Mid</span>}
                                        
                                        <div className={`w-full rounded-t-lg rounded-b-sm border border-white/5 ${bgColor}`} style={{ height: val * 2.5 + 40 }}></div>
                                        <span className={`font-mono font-bold text-sm ${isCurrentRange ? 'text-slate-300' : 'text-slate-700'}`}>{val}</span>
                                        <span className="text-[9px] font-bold text-slate-600">idx: {i}</span>
                                    </div>
                                );
                            })}
                        </div>
