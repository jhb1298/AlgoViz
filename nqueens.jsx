...
import './nqueens_script.js';
import './settings_manager.js';
import './algorithm_data.js';
import './chatbot.js';

/* extracted from nqueens.html */
        ...
        const Crown = (props) => (
            <IconWrapper {...props} viewBox="0 0 24 24">
                <path d="m2 4 3 12h14l3-12-6 7-4-7-4 7-6-7zm3 16h14" fill="currentColor" stroke="none" />
            </IconWrapper>
        );
        ...
        function generateSimulation(n) {
            let nodeIdCounter = 0;
            const history = [];
            const initialQueens = Array(n).fill(-1);
            ...
            function solve(row, queens, parentId = null) {
                const id = nodeIdCounter++;
                const node = { id, parentId, row, queens: [...queens], children: [] };
                
                history.push({ stepId: history.length, nodeId: id, line: 0, algoLine: 1, queens: [...queens], status: 'START', desc: `Row ${row + 1}: Finding a spot`, speech: row === 0 ? NQUEENS_SCRIPT.START(n) : null });

                if (row === n) {
                    history.push({ stepId: history.length, nodeId: id, line: 4, algoLine: null, queens: [...queens], status: 'SUCCESS', desc: `Found a solution!`, speech: NQUEENS_SCRIPT.SUCCESS() });
                    return { node, success: true };
                }

                for (let col = 0; col < n; col++) {
                    history.push({ stepId: history.length, nodeId: id, line: 1, algoLine: 2, queens: [...queens], trialCol: col, status: 'TRYING', desc: `Trying column ${col + 1}`, speech: NQUEENS_SCRIPT.TRYING(row, col) });
                    
                    if (isSafe(queens, row, col)) {
                        queens[row] = col;
                        history.push({ stepId: history.length, nodeId: id, line: 2, algoLine: 3, queens: [...queens], trialCol: col, status: 'SAFE', desc: `Column ${col + 1} is safe!`, speech: NQUEENS_SCRIPT.SAFE(row, col) });
                        
                        const { node: child, success } = solve(row + 1, queens, id);
                        node.children.push(child);
                        
                        if (success) return { node, success: true };
                        
                        queens[row] = -1; // Backtrack
                    } else {
                        history.push({ stepId: history.length, nodeId: id, line: 1, algoLine: 2, queens: [...queens], trialCol: col, status: 'CONFLICT', desc: `Conflict at column ${col + 1}`, speech: NQUEENS_SCRIPT.CONFLICT(row, col) });
                    }
                }

                history.push({ stepId: history.length, nodeId: id, line: 3, algoLine: 5, queens: [...queens], status: 'BACKTRACK', desc: `Backtracking from row ${row + 1}`, speech: NQUEENS_SCRIPT.BACKTRACK(row) });
                return { node, success: false };
            }

            const { node: treeRoot } = solve(0, initialQueens);
        ...
        const Chessboard = ({ n, queens, trialCol, row, status }) => {
            return (
                <div className="flex flex-col items-center">
                    <div className="grid gap-0 border-2 border-slate-600 rounded overflow-hidden shadow-xl" style={{ gridTemplateColumns: `repeat(${n}, 1fr)` }}>
                        {Array.from({ length: n * n }).map((_, i) => {
                            const r = Math.floor(i / n);
                            const c = i % n;
                            const isBlack = (r + c) % 2 === 1;
                            const hasQueen = queens[r] === c;
                            const isTrial = row === r && trialCol === c;
                            
                            let bgColor = isBlack ? 'bg-slate-700' : 'bg-slate-300';
                            if (isTrial) bgColor = status === 'CONFLICT' ? 'bg-rose-500/60' : 'bg-yellow-400/60';
                            if (hasQueen) bgColor = 'bg-emerald-500/40';
        ...
        const NodeComponent = ({ nodeData, isActive, isBreadcrumb, latestState, n }) => {
            ...
            if (latestState) {
                ...
                if (latestState.status === 'START') { statusColor = "bg-blue-600"; statusText = "DECIDING"; borderColor="border-blue-500"; }
                if (latestState.status === 'TRYING') { statusColor = "bg-amber-600"; statusText = "CHECKING"; borderColor="border-amber-500"; }
                if (latestState.status === 'SAFE') { statusColor = "bg-emerald-600"; statusText = "PLACED"; borderColor="border-emerald-500"; }
                if (latestState.status === 'CONFLICT') { statusColor = "bg-rose-600"; statusText = "ATTACKED"; borderColor="border-rose-500"; }
                if (latestState.status === 'BACKTRACK') { statusColor = "bg-slate-600"; statusText = "STUCK"; borderColor="border-slate-600"; }
                if (latestState.status === 'SUCCESS') { statusColor = "bg-emerald-500"; statusText = "SOLVED!"; borderColor="border-emerald-400"; }
                
                if (!isActive && latestState.status === 'SAFE') {
                    statusColor = "bg-indigo-600"; statusText = "WAITING"; isWaiting = true;
                }
            }
            ...
            return (
                <div className={`absolute rounded-xl shadow-2xl overflow-hidden transition-all duration-300 border-2 bg-slate-800 flex flex-col justify-between ${borderColor}`} style={{ left: nodeData.x, top: nodeData.y, width: NODE_WIDTH, height: NODE_HEIGHT, transform: isActive ? 'scale(1.05)' : 'scale(1)', zIndex: isActive ? 50 : 10, opacity, fontSize: settings.fontSize + "px", borderColor: isActive ? settings.nodeColor : undefined, boxShadow: isActive ? `0 0 20px ${settings.nodeColor}80` : undefined }}>
                    <div className="bg-slate-900/80 px-4 py-2 flex justify-between items-center border-b border-slate-700">
                        <span className="font-mono font-bold text-xs text-blue-300">N-Queens (Row {row + 1})</span>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${statusColor} text-white shadow-sm`}>{statusText}</span>
                    </div>
                    <div className="px-3 py-2 text-[10px] font-mono text-slate-400 bg-slate-900 border-b border-slate-700 leading-tight">
                        <div className={getCodeClass(1)}>1. for col from 0 to N-1:</div>
                        <div className={getCodeClass(1)}>&nbsp;&nbsp;if isSafe(row, col):</div>
                        <div className={getCodeClass(2)}>&nbsp;&nbsp;&nbsp;&nbsp;placeQueen(row, col)</div>
                        <div className={getCodeClass(2)}>&nbsp;&nbsp;&nbsp;&nbsp;solve(row + 1)</div>
                        <div className={getCodeClass(3)}>2. backtrack()</div>
                    </div>
                    <div className="bg-slate-800 flex-1 flex flex-col items-center justify-center p-4">
                        <Chessboard n={n} queens={latestState?.queens || nodeData.queens} trialCol={latestState?.trialCol} row={row} status={latestState?.status} />
                    </div>
                </div>
            );
        };
                ...
                <div className="h-screen w-screen flex flex-col relative overflow-hidden bg-slate-900 text-white select-none" onMouseMove={(e) => { if(isDraggingPanel) setPanelOffset({x: e.clientX-panelDragStart.current.x, y: e.clientY-panelDragStart.current.y}); else if(isDraggingCanvas) setManualOffset({x: e.clientX-canvasDragStart.current.x, y: e.clientY-canvasDragStart.current.y}); }} onMouseUp={() => {setIsDraggingCanvas(false); setIsDraggingPanel(false);}}>
                    <div className="absolute top-0 left-0 right-0 z-[60] top-nav-area">
                        ...
                        <div className="relative h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 flex items-center justify-between px-6 shadow-2xl transition-transform duration-300 -translate-y-full peer-hover:translate-y-0 hover:translate-y-0 z-[60]">
                            <div className="flex items-center gap-4"><a href="index.html" className="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-400 hover:text-white"><Home size={20} /></a><div><h1 className="font-bold text-lg leading-none tracking-tight">N-Queens Solver</h1><span className="text-xs text-slate-400 font-mono">Backtracking Strategy</span><button onClick={() => setIsSettingsOpen(true)} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors text-slate-400 hover:text-white shadow-lg"><SettingsIcon size={20}/></button></div></div>
                            <div className="flex items-center gap-2 bg-slate-800/80 p-1 pr-3 rounded-lg border border-slate-700/50">
                                <span className="bg-slate-700/80 px-2 py-1 rounded text-slate-300 text-xs font-bold uppercase mr-1">Queens</span>
                                <select value={n} onChange={(e) => { setN(Number(e.target.value)); setCurrentStep(0); setIsPlaying(false); }} className="bg-transparent font-bold outline-none text-blue-400 cursor-pointer">{[4, 5, 6].map(val => <option key={val} value={val}>{val}</option>)}</select>
                            </div>
                        </div>
                    </div>
                    
                    <SettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} />
                    <AlgorithmPanel algoKey="NQUEENS" currentLine={currentState?.algoLine} />
                    <div className="flex-1 relative infinite-bg overflow-hidden cursor-grab" onWheel={(e) => setZoom(z => Math.min(Math.max(0.1, z-e.deltaY*0.001), 1.5))} onMouseDown={(e) => {if(!e.target.closest('.control-panel-wrapper') && !e.target.closest('.top-nav-area')) {setIsDraggingCanvas(true); canvasDragStart.current={x: e.clientX-manualOffset.x, y: e.clientY-manualOffset.y};}}}>
                        <div className={`node-container absolute origin-top-left ${isDraggingCanvas ? '' : 'camera-layer'}`} style={{ transform: `translate(${camTarget.x + manualOffset.x}px, ${camTarget.y + manualOffset.y}px) scale(${zoom})` }}>
                            ...
                            {flatNodes.map(node => <NodeComponent key={node.id} nodeData={node} isActive={currentState?.nodeId === node.id} isBreadcrumb={activePathIds.has(node.id)} latestState={latestNodeStates[node.id]} n={n} />)}
                        </div>
                    </div>
